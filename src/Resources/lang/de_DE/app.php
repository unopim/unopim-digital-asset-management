<?php

return [
    'admin' => [
        'components' => [
            'layouts' => [
                'sidebar' => [
                    'dam'           => 'Medienverwaltung',
                    'assets'        => 'Medienobjekte',
                    'shares'        => 'Freigegebene Links',
                    'tags'          => 'Tags',
                    'configuration' => 'Konfiguration',
                ],
            ],
            'modal' => [
                'confirm' => [
                    'message' => 'Wenn Sie dieses Verzeichnis löschen, werden auch alle Unterverzeichnisse darin gelöscht. Diese Aktion ist dauerhaft und kann nicht rückgängig gemacht werden.',
                ],
            ],
            'asset' => [
                'field' => [
                    'add-asset'     => 'Asset hinzufügen',
                    'assign-assets' => 'Assets zuweisen',
                    'preview'       => 'Vorschau',
                    'remove'        => 'Entfernen',
                    'download'      => 'Herunterladen',
                ],
            ],
        ],
        'dam' => [
            'file' => [
                'deleted'   => 'Datei gelöscht',
                'not-found' => 'Datei nicht gefunden',
            ],
            'image-edit' => [
                'cannot-decode'  => 'Bild kann nicht dekodiert werden',
                'preview-failed' => 'Vorschau fehlgeschlagen',
            ],
            'index' => [
                'title'    => 'Digitale Medienverwaltung',
                'datagrid' => [
                    'file-name'      => 'Dateiname',
                    'tags'           => 'Tags',
                    'property-name'  => 'Eigenschaftsname',
                    'property-value' => 'Eigenschaftswert',
                    'created-at'     => 'Erstellt am',
                    'updated-at'     => 'Aktualisiert am',
                    'extension'      => 'Erweiterung',
                    'path'           => 'Pfad',
                ],
                'mass-action' => [
                    'deleting'   => 'Lösche :count Assets…',
                    'processing' => 'Verarbeite :count Assets…',
                ],
                'move' => [
                    'directory' => 'Verschiebe Verzeichnis ":name"…',
                    'asset'     => 'Verschiebe Asset ":name"…',
                ],
                'directory' => [
                    'title'  => 'Verzeichnis',
                    'create' => [
                        'title'     => 'Verzeichnis erstellen',
                        'name'      => 'Name',
                        'file_name' => 'Dateiname',
                        'save-btn'  => 'Verzeichnis speichern',
                    ],
                    'rename' => [
                        'title' => 'Verzeichnis umbenennen',
                    ],
                    'search' => [
                        'placeholder'     => 'Verzeichnisse suchen...',
                        'no-matches'      => 'Keine Treffer',
                        'error'           => 'Suche fehlgeschlagen — bitte erneut versuchen',
                        'not-found-flash' => 'Verzeichnis nicht mehr verfügbar',
                        'count'           => ':shown von :total Treffer',
                        'count-total'     => ':total Treffer',
                    ],
                    'asset' => [
                        'rename' => [
                            'title'    => 'Asset umbenennen',
                            'save-btn' => 'Asset speichern',
                        ],
                    ],
                    'actions' => [
                        'delete'                    => 'Löschen',
                        'edit'                      => 'Bearbeiten',
                        'rename'                    => 'Umbenennen',
                        'copy'                      => 'Kopieren',
                        'cut'                       => 'Ausschneiden',
                        'download'                  => 'Herunterladen',
                        'download-zip'              => 'ZIP herunterladen',
                        'paste'                     => 'Einfügen',
                        'add-directory'             => 'Verzeichnis hinzufügen',
                        'upload-files'              => 'Dateien hochladen',
                        'upload-files-and-folders'  => 'Dateien und Ordner hochladen',
                        'upload-folder'             => 'Ordner hochladen',
                        'upload-drop-hint'          => 'Dateien oder Ordner hierher ziehen',
                        'upload-or'                 => 'oder durchsuchen:',
                        'copy-directory-structured' => 'Verzeichnisstruktur kopieren',
                        'get'                       => 'Get',
                        'get-by-id'                 => 'Nach ID suchen',
                        'comment'                   => 'Kommentar',
                        'share'                     => 'Verzeichnis freigeben',
                    ],
                    'linked-resources'                          => 'Verlinkte Ressourcen',
                    'not-found'                                 => 'Kein Verzeichnis gefunden',
                    'created-success'                           => 'Verzeichnis erfolgreich erstellt',
                    'creation-failed'                           => 'Verzeichniserstellung fehlgeschlagen',
                    'updated-success'                           => 'Verzeichnis erfolgreich aktualisiert',
                    'update-failed'                             => 'Verzeichnisaktualisierung fehlgeschlagen',
                    'moved-success'                             => 'Verzeichnis erfolgreich verschoben',
                    'cannot-move'                               => 'Verzeichnis kann nicht verschoben werden',
                    'cannot-copy'                               => 'Ein Ordner kann nicht in sich selbst oder in einen seiner Unterordner kopiert werden.',
                    'fetch-all-success'                         => 'Verzeichnisse erfolgreich abgerufen',
                    'can-not-deleted'                           => 'Verzeichnis kann nicht gelöscht werden, da es das Root-Verzeichnis ist.',
                    'deleting-in-progress'                      => 'Verzeichnis wird gelöscht',
                    'can-not-copy'                              => 'Verzeichnis kann nicht kopiert werden, da es das Root-Verzeichnis ist.',
                    'coping-in-progress'                        => 'Verzeichnisstruktur wird kopiert.',
                    'asset-not-found'                           => 'Kein Asset gefunden',
                    'asset-renamed-success'                     => 'Asset erfolgreich umbenannt',
                    'asset-moved-success'                       => 'Asset erfolgreich verschoben',
                    'asset-name-already-exist'                  => 'Der neue Name existiert bereits für ein anderes Asset namens :asset_name',
                    'asset-name-conflict-in-the-same-directory' => 'Der Asset-Name steht im Konflikt mit einer bestehenden Datei im selben Verzeichnis.',
                    'old-file-not-found'                        => 'Die Datei unter dem Pfad :old_path wurde nicht gefunden.',
                    'image-name-is-the-same'                    => 'Dieser Name existiert bereits. Bitte geben Sie einen anderen ein.',
                    'not-writable'                              => 'Sie dürfen eine :type an diesem Speicherort ":path" nicht :actionType.',
                    'empty-directory'                           => 'Dieses Verzeichnis ist leer.',
                    'failed-download-directory'                 => 'Fehler beim Erstellen der ZIP-Datei.',
                    'not-allowed'                               => 'Das Hochladen von Skriptdateien ist nicht erlaubt.',
                    'copy-success'                              => 'Ordner erfolgreich kopiert.',
                    'name-unique'                               => 'Der Verzeichnisname muss im selben übergeordneten Verzeichnis eindeutig sein.',
                    'folder-upload-success'                     => 'Ordner erfolgreich hochgeladen.',
                    'folder-upload-partial'                     => 'Ordner hochgeladen, :count Datei(en) aufgrund von Fehlern übersprungen.',
                    'load-more'                                 => 'Mehr laden',
                ],
                'description'                    => 'Dieses Tool hilft Ihnen, alle Ihre Medienassets an einem Ort zu organisieren, zu speichern und zu verwalten.',
                'root'                           => 'Wurzel',
                'upload'                         => 'Hochladen',
                'new'                            => 'Neu',
                'uploading'                      => 'Hochladen...',
                'cancel'                         => 'Abbrechen',
                'upload-cancelled'               => 'Upload abgebrochen.',
                'folder-upload-https-required'   => 'Das Hochladen von Ordnern erfordert eine sichere Verbindung (HTTPS).',
                'folder-upload-no-https-warning' => 'Leere Ordner werden ohne eine sichere Verbindung (HTTPS) nicht erstellt. Dateien werden trotzdem hochgeladen.',
                'folder-upload-no-files'         => 'Der/die ausgewählte(n) Ordner enthält/enthalten keine Dateien. Leere Unterordner können ohne eine sichere Verbindung (HTTPS) nicht erkannt werden.',
                'upload-in-progress-title'       => 'Upload läuft',
                'upload-in-progress-message'     => 'Das Verlassen dieser Seite bricht den laufenden Upload ab. Sind Sie sicher, dass Sie die Seite verlassen möchten?',
                'upload-leave-page'              => 'Verlassen',
                'upload-stay-page'               => 'Bleiben',
                'drop-zone-hint'                 => 'Dateien oder Ordner zum Hochladen ziehen',
                'select-all'                     => 'Alle auswählen',
                'select-directory-to-upload'     => 'Wählen Sie ein Verzeichnis für den Upload aus.',
                'upload-session'                 => [
                    'uploading'            => 'Lade :total Datei(en) hoch…',
                    'paused'               => 'Pausiert — :done von :total hochgeladen',
                    'interrupted'          => ':done von :total hochgeladen · :interrupted unterbrochen',
                    'failed'               => ':done hochgeladen, :failed fehlgeschlagen',
                    'completed'            => ':done von :total hochgeladen',
                    'create-folder-failed' => 'Ordner konnte nicht erstellt werden',
                ],
                'drop-zone-no-permission' => 'Sie haben keine Berechtigung, hier hochzuladen.',
                'upload-complete'         => 'Upload abgeschlossen',
                'item-upload-complete'    => 'Upload abgeschlossen',
                'copy'                    => [
                    'directory' => 'Kopiere Verzeichnis ":name"…',
                ],
                'delete' => [
                    'directory' => 'Lösche Verzeichnis ":name"…',
                ],
            ],
            'asset' => [
                'properties' => [
                    'index' => [
                        'title'      => 'Asset-Eigenschaften',
                        'create-btn' => 'Eigenschaft erstellen',
                        'datagrid'   => [
                            'name'     => 'Name',
                            'type'     => 'Typ',
                            'language' => 'Sprache',
                            'value'    => 'Wert',
                            'edit'     => 'Bearbeiten',
                            'delete'   => 'Löschen',
                        ],
                        'create' => [
                            'title'                  => 'Eigenschaft erstellen',
                            'name'                   => 'Name',
                            'type'                   => 'Typ',
                            'language'               => 'Sprache',
                            'value'                  => 'Wert',
                            'add-to-filter'          => 'Zum Filter hinzufügen',
                            'add-to-filter-help'     => 'Wenn aktiviert, erscheint diese Eigenschaft als Filter in der Seitenleiste der Asset-Liste.',
                            'sort-order'             => 'Sortierreihenfolge',
                            'sort-order-help'        => 'Kleinere Zahlen erscheinen zuerst in der Filter-Seitenleiste. Eigenschaften mit derselben Zahl werden alphabetisch sortiert.',
                            'sort-order-placeholder' => 'z.B. 10',
                            'save-btn'               => 'Speichern',
                        ],
                        'edit' => [
                            'title' => 'Eigenschaft bearbeiten',
                        ],
                        'delete-success' => 'Asset-Eigenschaft erfolgreich gelöscht',
                        'delete-failed'  => 'Asset-Eigenschaft konnte nicht gelöscht werden',
                        'create-success' => 'Asset-Eigenschaft erfolgreich erstellt',
                        'update-success' => 'Asset-Eigenschaft erfolgreich aktualisiert',
                        'update-failure' => 'Asset-Eigenschaft konnte nicht aktualisiert werden',
                        'not-found'      => 'Eigenschaft nicht gefunden',
                        'found-success'  => 'Eigenschaft erfolgreich gefunden',
                    ],
                ],
                'comments' => [
                    'create' => [
                        'create-success' => 'Kommentar wurde erfolgreich hinzugefügt',
                        'create-failure' => 'Kommentar konnte nicht erstellt werden',
                    ],
                    'post-comment'    => 'Kommentar posten',
                    'post-reply'      => 'Antwort posten',
                    'reply'           => 'Antwort',
                    'edit-btn'        => 'Bearbeiten',
                    'delete-btn'      => 'Löschen',
                    'save-btn'        => 'Speichern',
                    'cancel-btn'      => 'Abbrechen',
                    'add-reply'       => 'Antwort hinzufügen',
                    'add-comment'     => 'Kommentar hinzufügen',
                    'no-comments'     => 'Noch keine Kommentare',
                    'not-found'       => 'Kommentare nicht gefunden',
                    'updated-success' => 'Kommentar erfolgreich aktualisiert',
                    'update-failed'   => 'Kommentar konnte nicht aktualisiert werden',
                    'delete-success'  => 'Asset-Tags wurden erfolgreich entfernt',
                    'delete-failed'   => 'Asset-Kommentar konnte nicht gelöscht werden',
                    'deleted-user'    => 'Gelöschter Benutzer',
                ],
                'edit' => [
                    'title'                 => 'Asset bearbeiten',
                    'back'                  => 'Zurück',
                    'previous'              => 'Zurück',
                    'next'                  => 'Weiter',
                    'name'                  => 'Name',
                    'value'                 => 'Wert',
                    'save-btn'              => 'Speichern',
                    'file-name'             => 'Dateiname',
                    'file-info'             => 'Dateiinformationen',
                    'type'                  => 'Typ',
                    'size'                  => 'Größe',
                    'dimensions'            => 'Abmessungen',
                    'path'                  => 'Pfad',
                    'created-at'            => 'Erstellt',
                    'updated-at'            => 'Aktualisiert',
                    'embedded_meta_info'    => 'Eingebettete Metadaten',
                    'no-metadata-available' => 'Keine Metadaten verfügbar',
                    'tags'                  => 'Tags',
                    'select-tags'           => 'Tag wählen oder erstellen',
                    'tag-attach-success'    => 'Tag erfolgreich hinzugefügt.',
                    'tag-detach-success'    => 'Tag erfolgreich vom Medienobjekt entfernt.',
                    'directory-path'        => 'Verzeichnispfad',
                    'no-directory'          => 'Kein Verzeichnis zugewiesen',
                    'details'               => 'Details',
                    'breadcrumb-assets'     => 'Medienobjekte',
                    'tab'                   => [
                        'preview'          => 'Vorschau',
                        'properties'       => 'Eigenschaften',
                        'comments'         => 'Kommentare',
                        'linked_resources' => 'Verknüpfte Ressourcen',
                        'history'          => 'Verlauf',
                    ],
                    'button' => [
                        'download'            => 'Herunterladen',
                        'custom_download'     => 'Benutzerdefinierter Download',
                        'rename'              => 'Umbenennen',
                        're_upload'           => 'Erneut hochladen',
                        're_uploading'        => 'Erneut hochladen...',
                        'cancel'              => 'Abbrechen',
                        're-upload-cancelled' => 'Erneuter Upload abgebrochen.',
                        'delete'              => 'Löschen',
                        'share'               => 'Freigeben',
                    ],
                    'share' => [
                        'title'      => 'Asset freigeben',
                        'link'       => 'Freigabe-Link',
                        'copy'       => 'Kopieren',
                        'copied'     => 'Link in die Zwischenablage kopiert',
                        'expiry'     => 'Link läuft ab nach',
                        'expiry-1d'  => '1 Tag',
                        'expiry-7d'  => '7 Tage',
                        'expiry-30d' => '30 Tage',
                        'expires-on' => 'Läuft ab am',
                        'failed'     => 'Freigabe-Link konnte nicht generiert werden.',
                    ],
                    'preview-modal' => [
                        'not-available' => 'Vorschau für diesen Dateityp nicht verfügbar.',
                        'download-file' => 'Datei herunterladen',
                        'download-zip'  => 'Als ZIP herunterladen',
                        'close'         => 'Vorschau schließen',
                        'mime'          => 'MIME',
                        'card'          => [
                            'click-for-details' => 'Klicken für vollständige Details',
                            'preview'           => 'Vorschau',
                            'edit-image'        => 'Bild bearbeiten',
                        ],
                        'image-viewer' => [
                            'rotate-left'   => 'Links drehen (L)',
                            'rotate-right'  => 'Rechts drehen (R)',
                            'zoom-out'      => 'Verkleinern (-)',
                            'zoom-in'       => 'Vergrößern (+)',
                            'fit-to-screen' => 'An Bildschirm anpassen',
                            'actual-size'   => 'Tatsächliche Größe',
                            'reset-all'     => 'Alles zurücksetzen (0)',
                        ],
                        'video-player' => [
                            'back-10s'           => '10 s zurück',
                            'forward-10s'        => '10 s vor',
                            '10s'                => '10 s',
                            'speed'              => 'Geschwindigkeit',
                            'loop'               => 'Wiederholen (L)',
                            'mute'               => 'Stummschalten (M)',
                            'picture-in-picture' => 'Bild im Bild',
                            'fullscreen'         => 'Vollbild (F)',
                            'more-actions'       => 'Weitere Aktionen',
                            'copy-link'          => 'Link kopieren',
                            'link-copied'        => 'Kopiert!',
                            'open-in-new-tab'    => 'In neuem Tab öffnen',
                        ],
                    ],
                    'custom-download' => [
                        'title'              => 'Benutzerdefinierter Download',
                        'format'             => 'Format',
                        'width'              => 'Breite (px)',
                        'width-placeholder'  => '200',
                        'height'             => 'Höhe (px)',
                        'height-placeholder' => '200',
                        'download-btn'       => 'Herunterladen',
                        'extension-types'    => [
                            'jpg'      => 'JPG',
                            'png'      => 'PNG',
                            'jpeg'     => 'JPEG',
                            'webp'     => 'WEBP',
                            'original' => 'Original',
                        ],
                    ],
                    'tag-already-exists'        => 'Tag existiert bereits',
                    'image-source-not-readable' => 'Bildquelle nicht lesbar',
                    'failed-to-read'            => 'Metadaten des Bildes konnten nicht gelesen werden: :exception',
                    'file-re-upload-success'    => 'Dateien wurden erfolgreich erneut hochgeladen.',
                    'image-processing-failed'   => 'Bildverarbeitung fehlgeschlagen: :message',
                    'image-editor'              => [
                        'tools'                    => 'Werkzeuge',
                        'close'                    => 'Editor schließen',
                        'crop'                     => 'Zuschneiden & Skalieren',
                        'crop-sub'                 => 'Bild zuschneiden oder skalieren',
                        'adjust'                   => 'Helligkeit & Kontrast',
                        'adjust-sub'               => 'Licht und Ton anpassen',
                        'rotate'                   => 'Drehen & Spiegeln',
                        'rotate-sub'               => 'Ausrichtung transformieren',
                        'selection'                => 'Auswahl',
                        'drag-handles'             => 'Ziehpunkte',
                        'scale-after-crop'         => 'Nach dem Zuschneiden skalieren',
                        'optional'                 => 'optional',
                        'width-px'                 => 'Breite (px)',
                        'height-px'                => 'Höhe (px)',
                        'blank-keep-dims'          => 'Leer lassen, um die zugeschnittenen Abmessungen beizubehalten.',
                        'auto'                     => 'auto',
                        'adjustments'              => 'Anpassungen',
                        'brightness'               => 'Helligkeit',
                        'contrast'                 => 'Kontrast',
                        'rotation'                 => 'Drehung',
                        'flip'                     => 'Spiegeln',
                        'horizontal'               => 'Horizontal',
                        'vertical'                 => 'Vertikal',
                        'platform'                 => 'Plattform',
                        'platform-loading'         => 'Wird geladen…',
                        'no-platforms'             => 'Keine Plattformen',
                        'color-none'               => 'Keine',
                        'color-white'              => 'Weiß',
                        'color-silver'             => 'Silber',
                        'color-light-gray'         => 'Hellgrau',
                        'color-gray'               => 'Grau',
                        'color-slate'              => 'Schiefergrau',
                        'color-dark-gray'          => 'Dunkelgrau',
                        'color-charcoal'           => 'Anthrazit',
                        'color-black'              => 'Schwarz',
                        'color-light-red'          => 'Hellrot',
                        'color-red'                => 'Rot',
                        'color-light-yellow'       => 'Hellgelb',
                        'color-yellow'             => 'Gelb',
                        'color-light-green'        => 'Hellgrün',
                        'color-green'              => 'Grün',
                        'color-light-cyan'         => 'Hellcyan',
                        'color-cyan'               => 'Türkis',
                        'color-light-blue'         => 'Hellblau',
                        'color-blue'               => 'Blau',
                        'color-light-purple'       => 'Helllila',
                        'color-purple'             => 'Lila',
                        'color-light-pink'         => 'Hellrosa',
                        'color-pink'               => 'Rosa',
                        'color-light-rose'         => 'Helles Rosa',
                        'color-rose'               => 'Rosé',
                        'model'                    => 'Modell',
                        'no-models'                => 'Keine Modelle',
                        'prompt'                   => 'Eingabe',
                        'apply'                    => 'Anwenden',
                        'applying'                 => 'Wird angewendet…',
                        'error-platforms'          => 'KI-Plattformen konnten nicht geladen werden.',
                        'error-select-color'       => 'Bitte eine Hintergrundfarbe auswählen.',
                        'error-operation'          => 'Vorgang fehlgeschlagen. Bitte versuchen Sie es erneut.',
                        'success-updated'          => 'Bild erfolgreich aktualisiert.',
                        'success-adjusted'         => 'Bild erfolgreich angepasst.',
                        'success-transformed'      => 'Bild erfolgreich transformiert.',
                        'error-no-ai-image'        => 'KI hat kein Bild zurückgegeben.',
                        'error-provide-dims'       => 'Geben Sie mindestens einen Zuschneidebereich oder Ausgabemaße an.',
                        'edit-bg'                  => 'Hintergrund bearbeiten',
                        'edit-bg-sub'              => 'Farbe, Upload oder Eingabeaufforderung',
                        'bg-tab-color'             => 'Farbe',
                        'bg-tab-upload'            => 'Hochladen',
                        'bg-tab-ai'                => 'Eingabeaufforderung',
                        'bg-color-label'           => 'Hintergrundfarbe',
                        'bg-color-mode-normal'     => 'Normal',
                        'bg-color-mode-ai'         => 'AI',
                        'bg-custom-color'          => 'Benutzerdefiniert',
                        'bg-upload-hint'           => 'Klicken um ein Hintergrundbild hochzuladen',
                        'bg-upload-change'         => 'Klicken zum Ändern',
                        'bg-ai-prompt-placeholder' => 'Beschreiben Sie den gewünschten Hintergrund…',
                        'sharpen'                  => 'Schärfen',
                        'blur'                     => 'Unschärfe',
                        'filters'                  => 'Filter',
                        'filters-sub'              => 'Graustufen und invertieren',
                        'greyscale'                => 'Graustufen',
                        'invert'                   => 'Invertieren',
                        'error-no-filter'          => 'Wählen Sie mindestens einen Filter aus.',
                        'error-provider-no-images' => 'Dieser KI-Anbieter unterstützt keine Bildgenerierung.',
                        'error-no-image-platform'  => 'Keine KI-Plattformen mit Bildgenerierungsunterstützung gefunden. Konfigurieren Sie eine OpenAI-, Gemini- oder xAI-Plattform.',
                    ],
                ],
                'linked-resources' => [
                    'index' => [
                        'datagrid' => [
                            'product'       => 'Produkt',
                            'category'      => 'Kategorie',
                            'product-sku'   => 'Produkt-SKU: ',
                            'category code' => 'Kategorie-Code: ',
                            'resource-type' => 'Ressourcentyp',
                            'resource'      => 'Ressource',
                            'resource-view' => 'Ressourcenansicht',
                        ],
                    ],
                    'found-success' => 'Ressource erfolgreich gefunden',
                    'not-found'     => 'Ressource nicht gefunden',
                ],
                'tags' => [
                    'create' => [
                        'create-success' => 'Tags wurden erfolgreich hinzugefügt',
                    ],
                    'found-success'  => 'Tag erfolgreich gefunden',
                    'not-found'      => 'Tags nicht gefunden',
                    'delete-success' => 'Asset-Tags erfolgreich gelöscht',
                ],
                'delete-success'                          => 'Asset erfolgreich gelöscht',
                'delete-failed-due-to-attached-resources' => 'Asset wird verwendet. Vor dem Löschen bitte trennen',
                'datagrid'                                => [
                    'mass-delete-success'                 => 'Massenlöschung erfolgreich.',
                    'mass-update-success'                 => 'Massenaktualisierung erfolgreich.',
                    'files-upload-success'                => 'Dateien erfolgreich hochgeladen.',
                    'file-upload-success'                 => 'Datei erfolgreich hochgeladen.',
                    'not-found'                           => 'Datei nicht gefunden',
                    'not-found-to-show'                   => 'Datei zur Anzeige nicht gefunden',
                    'not-found-to-download'               => 'Datei zum Herunterladen nicht gefunden',
                    'edit-success'                        => 'Datei erfolgreich hochgeladen',
                    'no-file-name'                        => 'kein Dateiname',
                    'show-success'                        => 'Datei erfolgreich gefunden',
                    'update-success'                      => 'Datei erfolgreich aktualisiert',
                    'not-found-to-update'                 => 'Datei existiert nicht',
                    'not-found-to-destroy'                => 'Datei existiert nicht',
                    'files-upload-failed'                 => 'Fehler beim Hochladen der Dateien.',
                    'file-upload-failed'                  => 'Datei konnte nicht hochgeladen werden',
                    'invalid-file'                        => 'Ungültige Datei bereitgestellt',
                    'invalid-file-format-or-not-provided' => 'Keine Dateien bereitgestellt oder ungültiger Format.',
                    'file-process-failed'                 => 'Einige Dateien konnten nicht verarbeitet werden',
                    'file-forbidden-type'                 => 'Die Datei hat einen verbotenen Typ oder eine verbotene Erweiterung.',
                    'file-too-large'                      => 'Die Datei ist zu groß. Die maximal zulässige Größe beträgt :size.',
                    'storage-upload-failed'               => 'Das Hochladen von ":name" auf den Speicher ist fehlgeschlagen. Überprüfen Sie die Anmeldeinformationen und Berechtigungen.',
                    'not-found-or-no-file'                => 'Asset nicht gefunden oder Datei existiert nicht.',
                    'download-link-ready'                 => 'Asset gefunden. Sie können die Datei über den bereitgestellten Link herunterladen.',
                ],
            ],
            'share' => [
                'target-not-found' => 'Das freigegebene Asset oder Verzeichnis konnte nicht gefunden werden.',
                'not-found'        => 'Freigabe-Link nicht gefunden.',
                'revoked'          => 'Freigabe-Link widerrufen.',
                'deleted'          => 'Freigabe-Link gelöscht.',
                'already-revoked'  => 'Dieser Freigabe-Link wurde bereits widerrufen.',
                'reauthorized'     => 'Freigabelink erneut autorisiert — die ursprüngliche URL ist wieder aktiv.',
                'not-revoked'      => 'Dieser Freigabelink ist nicht widerrufen.',
                'updated'          => 'Freigabelink aktualisiert.',
                'update-failed'    => 'Freigabelink konnte nicht aktualisiert werden.',
                'delete-failed'    => 'Freigabe-Link konnte nicht gelöscht werden.',
                'modal'            => [
                    'title-asset'        => 'Asset freigeben',
                    'title-directory'    => 'Verzeichnis freigeben',
                    'edit-title'         => 'Freigabelink bearbeiten',
                    'edit-name'          => 'Freigabe umbenennen',
                    'unnamed-share'      => '(kein benutzerdefinierter Name)',
                    'name-label'         => 'Benutzerdefinierter Name',
                    'name-hint'          => 'Leer lassen, um den ursprünglichen Datei-/Verzeichnisnamen als Bezeichnung beizubehalten.',
                    'save'               => 'Änderungen speichern',
                    'saving'             => 'Wird gespeichert…',
                    'cancel'             => 'Abbrechen',
                    'create-new'         => 'Neuen Freigabe-Link erstellen',
                    'expiry'             => 'Link läuft ab nach',
                    'expiry-1d'          => '1 Tag',
                    'expiry-7d'          => '7 Tage',
                    'expiry-30d'         => '30 Tage',
                    'expiry-365d'        => '1 Jahr',
                    'no-expiry'          => 'Kein Ablaufdatum',
                    'create'             => 'Link generieren',
                    'creating'           => 'Wird generiert…',
                    'active-links'       => 'Aktive Links',
                    'loading'            => 'Wird geladen…',
                    'no-active-links'    => 'Noch keine aktiven Freigabe-Links für dieses Element.',
                    'copy'               => 'Kopieren',
                    'copied'             => 'Link in die Zwischenablage kopiert',
                    'revoke'             => 'Widerrufen',
                    'delete'             => 'Delete',
                    'revoked'            => 'Freigabe-Link widerrufen',
                    'revoke-failed'      => 'Freigabe-Link konnte nicht widerrufen werden.',
                    'expires-on'         => 'Läuft ab am',
                    'never-expires'      => 'Läuft nie ab',
                    'views'              => 'Aufrufe',
                    'downloads'          => 'Downloads',
                    'created'            => 'Freigabe-Link erstellt.',
                    'create-failed'      => 'Freigabe-Link konnte nicht erstellt werden.',
                    'load-failed'        => 'Vorhandene Freigabe-Links konnten nicht geladen werden.',
                    'advanced'           => 'Erweitert',
                    'revoked-status'     => 'Link widerrufen',
                    'reauthorize'        => 'Erneut autorisieren',
                    'reauthorizing'      => 'Wird erneut autorisiert…',
                    'reauthorize-failed' => 'Freigabelink konnte nicht erneut autorisiert werden.',
                    'revoked-notice'     => 'Dieser Link ist widerrufen. Erneut autorisieren, um die ursprüngliche URL wieder zu aktivieren.',
                ],
                'index' => [
                    'title'       => 'Freigegebene Links',
                    'description' => 'Alle aktiven und historischen Freigabe-Links für Assets und Verzeichnisse im DAM.',
                ],
                'datagrid' => [
                    'type'           => 'Typ',
                    'asset'          => 'Asset',
                    'directory'      => 'Verzeichnis',
                    'target'         => 'Ziel',
                    'custom-name'    => 'Benutzerdefinierter Name',
                    'created-by'     => 'Erstellt von',
                    'expires-at'     => 'Läuft ab am',
                    'never'          => 'Nie',
                    'status'         => 'Status',
                    'status-active'  => 'Aktiv',
                    'status-expired' => 'Abgelaufen',
                    'status-revoked' => 'Widerrufen',
                    'views'          => 'Aufrufe',
                    'downloads'      => 'Downloads',
                    'created-at'     => 'Erstellt am',
                    'edit'           => 'Bearbeiten',
                    'copy-link'      => 'Link kopieren',
                    'revoke'         => 'Widerrufen',
                    'delete'         => 'Löschen',
                ],
            ],
            'upload' => [
                'pause'     => 'Pausieren',
                'resume'    => 'Fortsetzen',
                'retry'     => 'Erneut versuchen',
                'cancel'    => 'Abbrechen',
                'paused'    => 'Upload pausiert. Vorhandene Assets bleiben unverändert — jederzeit fortsetzbar.',
                'resumed'   => 'Upload fortgesetzt.',
                'cancelled' => 'Upload abgebrochen. Bereits hochgeladene Assets wurden beibehalten.',
                'retried'   => 'Fehlgeschlagene Uploads werden im Hintergrund erneut versucht.',
            ],
            'tag' => [
                'not-found'           => 'Tag nicht gefunden.',
                'create-success'      => 'Tag erfolgreich erstellt.',
                'update-success'      => 'Tag erfolgreich aktualisiert.',
                'delete-success'      => 'Tag erfolgreich gelöscht.',
                'mass-delete-success' => 'Ausgewählte Tags erfolgreich gelöscht.',
                'index'               => [
                    'title'       => 'Tags',
                    'description' => 'Verwalten Sie die Tags, mit denen Assets im DAM gekennzeichnet und organisiert werden.',
                    'create'      => 'Tag erstellen',
                ],
                'datagrid' => [
                    'name'         => 'Name',
                    'assets-count' => 'Assets',
                    'created-at'   => 'Erstellt am',
                    'edit'         => 'Bearbeiten',
                    'delete'       => 'Löschen',
                ],
                'modal' => [
                    'create-title'     => 'Tag erstellen',
                    'edit-title'       => 'Tag bearbeiten',
                    'name-label'       => 'Tag-Name',
                    'name-placeholder' => 'z. B. sommer-kampagne',
                    'save'             => 'Speichern',
                    'saving'           => 'Wird gespeichert…',
                    'cancel'           => 'Abbrechen',
                ],
                'mass-action' => [
                    'assign-tags'              => 'Tags zuweisen',
                    'modal-title'              => 'Tags zuweisen',
                    'modal-subtitle'           => 'Tags zu :count ausgewählten Asset(s) hinzufügen. Bestehende Tags bleiben erhalten.',
                    'modal-subtitle-recursive' => 'Tags zu den ausgewählten Assets und allen Assets in den ausgewählten Ordnern hinzufügen (rekursiv). Bestehende Tags bleiben erhalten.',
                    'tags-label'               => 'Tags',
                    'tags-placeholder'         => 'Tags suchen oder eingeben, um neue zu erstellen…',
                    'add-tag'                  => 'Eingabetaste zum Hinzufügen drücken',
                    'assign'                   => 'Zuweisen',
                    'assigning'                => 'Wird zugewiesen…',
                    'cancel'                   => 'Abbrechen',
                    'assign-success'           => 'Tags :count Asset(s) zugewiesen.',
                    'assign-failed'            => 'Tags konnten nicht zugewiesen werden.',
                    'no-assets'                => 'Keine Assets ausgewählt.',
                    'no-items'                 => 'Wählen Sie mindestens ein Asset oder einen Ordner aus.',
                    'no-tags'                  => 'Bitte wählen oder erstellen Sie mindestens ein Tag.',
                ],
            ],
        ],
        'catalog' => [
            'attributes' => [
                'type' => [
                    'asset' => 'Asset',
                ],
            ],
            'category-fields' => [
                'type' => [
                    'asset' => 'Asset',
                ],
            ],
        ],
        'acl' => [
            'menu'             => 'Medienverwaltung',
            'asset'            => 'Asset',
            'property'         => 'Eigenschaft',
            'comment'          => 'Kommentar',
            'linked_resources' => 'Verknüpfte Ressourcen',
            'meta_data'        => 'Eingebettete Meta-Informationen',
            'directory'        => 'Verzeichnis',
            'tag'              => 'Tag',
            'create'           => 'Erstellen',
            'edit'             => 'Bearbeiten',
            'update'           => 'Aktualisieren',
            'delete'           => 'Delete',
            'list'             => 'Auflisten',
            'view'             => 'Ansehen',
            'upload'           => 'Hochladen',
            're_upload'        => 'Erneut hochladen',
            'mass_update'      => 'Massenaktualisierung',
            'mass_delete'      => 'Massenlöschung',
            'download'         => 'Herunterladen',
            'custom_download'  => 'Benutzerdefinierter Download',
            'rename'           => 'Umbenennen',
            'move'             => 'Verschieben',
            'copy'             => 'Kopieren',
            'copy-structure'   => 'Verzeichnisstruktur kopieren',
            'download-zip'     => 'ZIP herunterladen',
            'asset-assign'     => 'Asset zuweisen',
            'share'            => 'Freigeben',
            'assets'           => 'Medienobjekte',
            'shares'           => 'Freigegebene Links',
            'revoke'           => 'Widerrufen',
            'tags'             => 'Tags',
            'configuration'    => 'Konfiguration',
        ],
        'permissions' => [
            'title'                 => 'DAM-Verzeichnisberechtigungen',
            'tab-subtitle'          => 'Verzeichnisse, auf die diese Rolle zugreifen kann.',
            'no-actions'            => 'Nur Anzeige. Öffnen Sie einen Unterordner, auf den Sie Zugriff haben.',
            'unauthorized'          => 'Sie sind nicht berechtigt, DAM-Verzeichnisberechtigungen zu verwalten.',
            'all-directories'       => 'Alle Verzeichnisse — Zugriff auf alle Verzeichnisse jetzt und zukünftig gewähren',
            'inherit-children'      => 'Unterverzeichnisse erben — automatisch Zugriff auf alle Unterverzeichnisse in gewährten Verzeichnissen',
            'loading'               => 'Verzeichnisse werden geladen…',
            'empty'                 => 'Keine Verzeichnisse gefunden.',
            'error'                 => 'Verzeichnisse konnten nicht geladen werden. Bitte versuchen Sie es erneut.',
            'retry'                 => 'Erneut versuchen',
            'grants-load-warning'   => 'Einige Verzeichnisberechtigungen konnten nicht geladen werden. Bitte überprüfen Sie Ihre Auswahl vor dem Speichern.',
            'inherit-children-note' => 'Unterverzeichnisse werden beim Speichern automatisch eingeschlossen.',
        ],
        'validation' => [
            'asset' => [
                'required' => 'Das Feld :attribute ist erforderlich.',
            ],
            'comment' => [
                'required' => 'Das Kommentarfeld ist erforderlich.',
            ],
            'property' => [
                'name' => [
                    'required' => 'Das Namensfeld ist erforderlich.',
                    'unique'   => 'Der Name wurde bereits vergeben.',
                ],
                'language' => [
                    'not-found' => 'Die ausgewählte Sprache wurde nicht gefunden oder ist derzeit deaktiviert.',
                ],
                'sort-order' => [
                    'required' => 'Das Feld Sortierreihenfolge ist erforderlich, wenn „Zum Filter hinzufügen“ aktiviert ist.',
                ],
            ],
        ],
        'errors' => [
            '401' => 'Diese Aktion ist nicht autorisiert.',
        ],
        'configuration' => [
            'save-btn'    => 'Speichern',
            'title'       => 'Konfiguration',
            'saved'       => 'Konfiguration erfolgreich gespeichert.',
            'description' => 'Konfigurieren Sie die Einstellungen der digitalen Medienverwaltung (DAM) für Ihre Anwendung.',
            'general'     => [
                'title'            => 'Allgemeine Einstellungen',
                'description'      => 'Diese Einstellungen steuern das gesamte DAM-Browsing-Erlebnis.',
                'explorer-enabled' => [
                    'label' => 'Explorer-Ansicht aktivieren',
                    'hint'  => 'Verwenden Sie die Explorer-ähnliche Oberfläche zum Durchsuchen und Verwalten von Assets.',
                ],
                'bookmarks-enabled' => [
                    'label' => 'Lesezeichen-Panel anzeigen',
                    'hint'  => 'Zeigt eine eigene Seitenleiste mit Ihren als Lesezeichen gespeicherten Ordnern und Assets an.',
                ],
            ],
            'directory' => [
                'title'       => 'Verzeichniseinstellungen',
                'description' => 'Diese Einstellungen steuern, wie Ordner und Assets im Verzeichnis-Panel angezeigt werden.',
                'show-tree'   => [
                    'label'       => 'Verzeichnisbaum anzeigen',
                    'hint'        => 'Zeigt die Ordnerhierarchie in der linken Seitenleiste für eine einfachere Navigation an.',
                    'locked-hint' => 'Immer aktiv, solange die Explorer-Ansicht deaktiviert ist.',
                ],
                'tree-show-assets' => [
                    'label' => 'Assets im Verzeichnisbaum anzeigen',
                    'hint'  => 'Zeigt Assets zusammen mit Ordnern im Verzeichnisbaum an, anstatt nur Ordner anzuzeigen.',
                ],
            ],
            'ai-tagging' => [
                'title'       => 'KI-Verschlagwortung',
                'description' => 'Schlägt automatisch Tags für hochgeladene Bilder anhand der konfigurierten Magic-AI-Plattform vor.',
                'enabled'     => [
                    'label' => 'Assets automatisch mit KI verschlagworten',
                    'hint'  => 'Verwendet Ihre konfigurierte Magic-AI-Plattform, um beim Hochladen automatisch Tags für Bilder vorzuschlagen.',
                ],
                'platform' => [
                    'label'                    => 'Plattform',
                    'use-default'              => 'Standardplattform verwenden',
                    'none-configured'          => 'Keine KI-Plattform konfiguriert',
                    'none-configured-hint'     => 'Fügen Sie zunächst eine bildfähige Magic-AI-Plattform hinzu — siehe Einstellungen → Magic AI → Plattformen.',
                ],
                'max-tags' => [
                    'label' => 'Max. Tags',
                    'hint'  => 'Maximale Anzahl an vorgeschlagenen Tags pro Bild (1-20).',
                ],
            ],
        ],
        'explorer' => [
            'not-found'         => 'Verzeichnis nicht gefunden.',
            'access-denied'     => 'Sie haben keinen Zugriff auf dieses Verzeichnis.',
            'empty'             => 'Keine Verzeichnisse gefunden.',
            'action-completed'  => 'Aktion erfolgreich abgeschlossen.',
            'foreign-selection' => [
                'notice' => ':count Element(e) in einem anderen Tab ausgewählt.',
                'action' => 'Auch hier auswählen',
            ],
            'bookmarks' => [
                'title'     => 'Lesezeichen',
                'drag-hint' => 'Ordner hierher ziehen, um ihn als Lesezeichen zu speichern',
                'max'       => 'Maximale Anzahl von 20 Lesezeichen erreicht.',
                'remove'    => 'Lesezeichen entfernen',
                'empty'     => 'Noch keine Lesezeichen.',
            ],
            'tab' => [
                'new'   => 'Neuer Tab',
                'close' => 'Tab schließen',
            ],
            'sections' => [
                'folders' => 'Ordner',
                'files'   => 'Dateien',
                'folder'  => 'Ordner',
            ],
            'clipboard' => [
                'ready'   => 'Bereit zum Einfügen',
                'dismiss' => 'Löschen',
            ],
            'context' => [
                'open'                 => 'Öffnen',
                'open-new-tab'         => 'In neuem Tab öffnen',
                'bookmark'             => 'Verzeichnis mit Lesezeichen',
                'folder-upload'        => 'Ordner hochladen',
                'copy'                 => 'Kopieren',
                'paste'                => 'Einfügen',
                'copy-progress'        => 'Kopiervorgang in Warteschlange.',
                'copy-done'            => 'Kopiert.',
                'paste-file-started'   => 'Datei wird eingefügt...',
                'paste-folder-started' => 'Ordner wird eingefügt...',
                'paste-done'           => 'Erfolgreich eingefügt.',
                'move-asset-started'   => 'Datei wird verschoben...',
                'move-folder-started'  => 'Ordner wird verschoben...',
                'move-done'            => 'Ordner erfolgreich verschoben.',
                'move-asset-done'      => 'Datei erfolgreich verschoben.',
                'delete-done'          => 'Verzeichnis erfolgreich gelöscht.',
            ],
            'folder' => [
                'deleted' => 'Dieser Ordner wurde gelöscht — zurück zum Stammverzeichnis.',
            ],
            'search' => [
                'placeholder' => 'Dateien und Ordner suchen…',
            ],
            'list' => [
                'header' => [
                    'name'     => 'Name',
                    'type'     => 'Typ',
                    'size'     => 'Größe',
                    'modified' => 'Geändert',
                    'actions'  => 'Aktionen',
                ],
                'items-count' => ':count Elemente',
            ],
            'dialog' => [
                'rename-asset' => [
                    'title'       => 'Datei umbenennen',
                    'placeholder' => 'Neuer Name',
                ],
                'save'   => 'Speichern',
                'cancel' => 'Abbrechen',
            ],
            'view' => [
                'grid' => 'Rasteransicht',
                'list' => 'Listenansicht',
            ],
            'mass-actions' => [
                'select-count'   => ':count ausgewählt',
                'select-action'  => 'Aktion auswählen',
                'delete'         => 'Löschen',
                'confirm'        => ':count Element(e) löschen? Ordner werden rekursiv gelöscht und können nicht wiederhergestellt werden.',
                'deleting'       => ':count Element(e) wird/werden gelöscht…',
                'deleted-assets' => ':count Asset(s) gelöscht.',
                'deleted-dirs'   => ':count Ordner zur Löschung in die Warteschlange eingereiht.',
                'move-to'        => 'Verschieben nach…',
                'copy-to'        => 'Kopieren nach…',
                'moving'         => 'Wird verschoben…',
                'copying'        => 'Wird kopiert…',
                'moved-assets'   => ':count Datei(en) verschoben.',
                'copied-assets'  => ':count Datei(en) kopiert.',
                'moved-dirs'     => ':count Ordner verschoben.',
                'copied-dirs'    => ':count Ordner kopiert.',
                'pick-dest'      => 'Zielordner auswählen',
                'select-here'    => 'Hier auswählen',
                'new-folder'     => 'Neuer Ordner',
                'folder-name'    => 'Ordnername',
                'create-error'   => 'Ordner konnte nicht erstellt werden.',
                'move-done'      => ':count Dateien verschoben.',
                'copy-done'      => ':count Dateien kopiert.',
                'still-running'  => 'Der Vorgang läuft noch im Hintergrund',
                'progress'       => 'Fortschritt',
                'total-files'    => ':count Datei(en)',
            ],
            'toolbar' => [
                'toggle-tree'    => 'Verzeichnisbaum ein-/ausblenden',
                'toggle-sidebar' => 'Seitenleiste ein-/ausblenden',
            ],
            'upload' => [
                'queued'              => 'In Warteschlange',
                'uploading'           => 'Wird hochgeladen',
                'creating'            => 'Ordner wird erstellt',
                'done'                => 'Fertig',
                'failed'              => 'Fehlgeschlagen',
                'interrupted'         => 'Unterbrochen — zum Wiederholen erneut hinzufügen',
                'resuming'            => 'Wird fortgesetzt…',
                'overall'             => ':done von :total hochgeladen',
                'too-large-to-resume' => 'Stapel zu groß für automatische Fortsetzung nach dem Neuladen',
            ],
        ],
    ],
    'share' => [
        'public' => [
            'app-name'              => 'DAM',
            'download'              => 'Herunterladen',
            'back-to-gallery'       => 'Zurück zur Galerie',
            'back'                  => 'Zurück',
            'file-name'             => 'Dateiname',
            'file-type'             => 'Dateityp',
            'file-size'             => 'Dateigröße',
            'video-not-supported'   => 'Ihr Browser unterstützt das Video-Tag nicht.',
            'audio-not-supported'   => 'Ihr Browser unterstützt das Audio-Tag nicht.',
            'preview-not-available' => 'Für diese Datei ist keine Vorschau verfügbar. Verwenden Sie die Schaltfläche Herunterladen oben.',
            'files-count'           => ':count Datei(en)',
            'empty-directory'       => 'Dieses freigegebene Verzeichnis enthält noch keine Dateien.',
            'expires-on'            => 'Dieser Link läuft ab am',
            'powered-by-dam'        => 'Bereitgestellt von Unopim DAM',
            'expired-title'         => 'Link abgelaufen',
            'expired-message'       => 'Dieser Freigabe-Link ist abgelaufen und nicht mehr zugänglich. Bitte fordern Sie einen neuen Link beim Eigentümer an.',
            'revoked-title'         => 'Link widerrufen',
            'revoked-message'       => 'Dieser Freigabe-Link wurde vom Eigentümer widerrufen und ist nicht mehr zugänglich.',
            'not-found-title'       => 'Link nicht gefunden',
            'not-found-message'     => 'Der von Ihnen geöffnete Freigabe-Link existiert nicht. Er wurde möglicherweise falsch eingegeben oder entfernt.',
            'download-zip'          => 'Alles als ZIP herunterladen',
            'per-page'              => 'Pro Seite',
        ],
    ],
    'data-transfer' => [
        'bundle' => [
            'invalid-archive'            => 'The uploaded archive could not be opened.',
            'invalid-zip'                => 'The uploaded archive is damaged and cannot be read.',
            'no-data-file'               => 'The archive contains no CSV or Excel file to import.',
            'zip-too-many-entries'       => 'The archive contains :count entries, which is above the limit of :limit.',
            'zip-contents-too-large'     => 'The archive expands to more than :limit MB.',
            'zip-compression-suspicious' => 'The entry ":entry" is compressed far beyond a normal ratio and was rejected.',
            'asset-not-found'            => 'No asset was found at path ":path".',
            'asset-tree-misnamed'        => 'Das Archiv enthält einen Ordner namens ":name" statt "assets", daher konnte keines seiner Assets importiert werden. Benennen Sie ihn in "assets" um und stellen Sie sicher, dass die Ordner darunter den Pfaden in Ihrer Datendatei entsprechen, z. B. "assets/Root/Seed/Tech Sheets/Cotton/". macOS speichert ein in einen Ordnernamen eingegebenes "/" als ":", was die übliche Ursache ist.',
        ],
    ],
];
