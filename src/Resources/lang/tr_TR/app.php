<?php

return [
    'admin' => [
        'components' => [
            'layouts' => [
                'sidebar' => [
                    'dam'           => 'DAM',
                    'assets'        => 'Varlıklar',
                    'shares'        => 'Paylaşılan Bağlantılar',
                    'tags'          => 'Etiketler',
                    'configuration' => 'Yapılandırma',
                ],
            ],
            'modal' => [
                'confirm' => [
                    'message' => 'Bu dizini silmek, içindeki tüm alt dizinleri de silecektir. Bu işlem kalıcıdır ve geri alınamaz.',
                ],
            ],
            'asset' => [
                'field' => [
                    'add-asset'     => 'Varlık ekle',
                    'assign-assets' => 'Varlıkları ata',
                    'preview'       => 'Önizleme',
                    'remove'        => 'Kaldır',
                    'download'      => 'İndir',
                ],
            ],
        ],
        'dam' => [
            'file' => [
                'deleted'   => 'Dosya silindi',
                'not-found' => 'Dosya bulunamadı',
            ],
            'image-edit' => [
                'cannot-decode'  => 'Görüntü çözümlenemiyor',
                'preview-failed' => 'Önizleme başarısız',
            ],
            'index' => [
                'title'    => 'DAM',
                'datagrid' => [
                    'file-name'      => 'Dosya adı',
                    'tags'           => 'Etiketler',
                    'property-name'  => 'Özellik adı',
                    'property-value' => 'Özellik değeri',
                    'created-at'     => 'Oluşturulma tarihi',
                    'updated-at'     => 'Güncellenme tarihi',
                    'extension'      => 'Uzantı',
                    'path'           => 'Yol',
                ],
                'mass-action' => [
                    'deleting'   => ':count varlık siliniyor…',
                    'processing' => ':count varlık işleniyor…',
                ],
                'move' => [
                    'directory' => '":name" dizini taşınıyor…',
                    'asset'     => '":name" varlığı taşınıyor…',
                ],
                'directory' => [
                    'title'  => 'Dizin',
                    'create' => [
                        'title'     => 'Dizin oluştur',
                        'name'      => 'Ad',
                        'file_name' => 'Dosya Adı',
                        'save-btn'  => 'Dizini kaydet',
                    ],
                    'rename' => [
                        'title' => 'Dizini yeniden adlandır',
                    ],
                    'search' => [
                        'placeholder'     => 'Dizinlerde ara...',
                        'no-matches'      => 'Eşleşme yok',
                        'error'           => 'Arama başarısız — tekrar deneyin',
                        'not-found-flash' => 'Dizine artık erişilemiyor',
                        'count'           => ':total eşleşmeden :shown',
                        'count-total'     => ':total eşleşme',
                    ],
                    'asset' => [
                        'rename' => [
                            'title'    => 'Varlığı yeniden adlandır',
                            'save-btn' => 'Varlığı kaydet',
                        ],
                    ],
                    'actions' => [
                        'delete'                    => 'Sil',
                        'edit'                      => 'Düzenle',
                        'rename'                    => 'Yeniden adlandır',
                        'copy'                      => 'Kopyala',
                        'cut'                       => 'Kes',
                        'download'                  => 'İndir',
                        'download-zip'              => 'Zip olarak indir',
                        'paste'                     => 'Yapıştır',
                        'add-directory'             => 'Dizin ekle',
                        'upload-files'              => 'Dosya yükle',
                        'upload-files-and-folders'  => 'Dosya ve Klasör Yükle',
                        'upload-folder'             => 'Klasör Yükle',
                        'upload-drop-hint'          => 'Dosya veya klasörleri buraya sürükleyin',
                        'upload-or'                 => 'veya gözat:',
                        'copy-directory-structured' => 'Dizin yapısını kopyala',
                        'get-by-id'                 => 'Kimliğe göre al',
                        'comment'                   => 'Yorum',
                        'share'                     => 'Dizini paylaş',
                    ],
                    'linked-resources'                          => 'Bağlı kaynaklar',
                    'not-found'                                 => 'Dizin bulunamadı',
                    'created-success'                           => 'Dizin başarıyla oluşturuldu',
                    'creation-failed'                           => 'Dizin oluşturma başarısız',
                    'updated-success'                           => 'Dizin başarıyla güncellendi',
                    'update-failed'                             => 'Dizin güncelleme başarısız',
                    'moved-success'                             => 'Dizin başarıyla taşındı',
                    'cannot-move'                               => 'Dizin taşınamıyor',
                    'fetch-all-success'                         => 'Dizinler başarıyla alındı',
                    'can-not-deleted'                           => 'Dizin kök dizin olduğu için silinemez.',
                    'deleting-in-progress'                      => 'Dizin silme işlemi devam ediyor',
                    'can-not-copy'                              => 'Dizin kök dizin olduğu için kopyalanamaz.',
                    'coping-in-progress'                        => 'Dizin yapısının kopyalanması devam ediyor.',
                    'asset-not-found'                           => 'Varlık bulunamadı',
                    'asset-renamed-success'                     => 'Varlık başarıyla yeniden adlandırıldı',
                    'asset-moved-success'                       => 'Varlık başarıyla taşındı',
                    'asset-name-already-exist'                  => 'Yeni ad, :asset_name adlı başka bir varlıkta zaten mevcut',
                    'asset-name-conflict-in-the-same-directory' => 'Varlık adı, aynı dizindeki mevcut bir dosya ile çakışıyor.',
                    'old-file-not-found'                        => ':old_path yolundaki dosya bulunamadı.',
                    'image-name-is-the-same'                    => 'Bu ad zaten mevcut. Lütfen farklı bir ad girin.',
                    'not-writable'                              => 'Bu konumda ":path" bir :type üzerinde :actionType işlemine izniniz yok.',
                    'empty-directory'                           => 'Bu dizin boş.',
                    'failed-download-directory'                 => 'Zip dosyası oluşturulamadı.',
                    'not-allowed'                               => 'Betik dosyaları yüklemeye izin verilmiyor.',
                    'copy-success'                              => 'Klasör başarıyla kopyalandı.',
                    'name-unique'                               => 'Dizin adı, aynı üst dizin içinde benzersiz olmalıdır.',
                    'folder-upload-success'                     => 'Klasör başarıyla yüklendi.',
                    'folder-upload-partial'                     => 'Klasör yüklendi, :count dosya hata nedeniyle atlandı.',
                    'load-more'                                 => 'Daha fazla yükle',
                ],
                'description'                    => 'Tüm medya varlıklarınızı tek bir yerde düzenlemenize, saklamanıza ve yönetmenize yardımcı olan araç',
                'root'                           => 'Kök',
                'upload'                         => 'Yükle',
                'uploading'                      => 'Yükleniyor...',
                'cancel'                         => 'İptal',
                'upload-cancelled'               => 'Yükleme iptal edildi.',
                'folder-upload-https-required'   => 'Klasör yükleme güvenli bir bağlantı (HTTPS) gerektirir.',
                'folder-upload-no-https-warning' => 'Güvenli bağlantı (HTTPS) olmadan boş klasörler oluşturulmayacak. Dosyalar yine de yüklenecek.',
                'folder-upload-no-files'         => 'Seçilen klasör(ler) hiç dosya içermiyor. Güvenli bağlantı (HTTPS) olmadan boş alt klasörler tespit edilemiyor.',
                'upload-in-progress-title'       => 'Yükleme Devam Ediyor',
                'upload-in-progress-message'     => 'Bu sayfadan ayrılmak aktif yüklemeyi iptal edecek. Ayrılmak istediğinizden emin misiniz?',
                'upload-leave-page'              => 'Ayrıl',
                'upload-stay-page'               => 'Kal',
                'drop-zone-hint'                 => 'Yüklemek için dosya veya klasörleri sürükleyin',
                'drop-zone-no-permission'        => 'Bu konuma yükleme yapma izniniz yok.',
                'upload-complete'                => 'Yükleme tamamlandı',
                'item-upload-complete'           => 'Yükleme tamamlandı',
                'copy'                           => [
                    'directory' => '":name" dizini kopyalanıyor…',
                ],
                'delete' => [
                    'directory' => '":name" dizini siliniyor…',
                ],
            ],
            'asset' => [
                'properties' => [
                    'index' => [
                        'title'      => 'Varlık özellikleri',
                        'create-btn' => 'Özellik oluştur',
                        'datagrid'   => [
                            'name'     => 'Ad',
                            'type'     => 'Tür',
                            'language' => 'Dil',
                            'value'    => 'Değer',
                            'edit'     => 'Düzenle',
                            'delete'   => 'Sil',
                        ],
                        'create' => [
                            'title'                  => 'Özellik oluştur',
                            'name'                   => 'Ad',
                            'type'                   => 'Tür',
                            'language'               => 'Dil',
                            'value'                  => 'Değer',
                            'add-to-filter'          => 'Filtreye Ekle',
                            'add-to-filter-help'     => 'Etkinleştirildiğinde bu özellik, varlık listesi kenar çubuğunda filtre olarak görünür.',
                            'sort-order'             => 'Sıralama düzeni',
                            'sort-order-help'        => 'Daha küçük sayılar filtre kenar çubuğunda önce görünür. Aynı sayıya sahip özellikler alfabetik olarak sıralanır.',
                            'sort-order-placeholder' => 'örn. 10',
                            'save-btn'               => 'Kaydet',
                        ],
                        'edit' => [
                            'title' => 'Özelliği düzenle',
                        ],
                        'delete-success' => 'Varlık özelliği başarıyla silindi',
                        'delete-failed'  => 'Varlık özelliği silinemedi',
                        'create-success' => 'Varlık özelliği başarıyla oluşturuldu',
                        'update-success' => 'Varlık özelliği başarıyla güncellendi',
                        'update-failure' => 'Varlık özelliği güncellenemedi',
                        'not-found'      => 'Özellik bulunamadı',
                        'found-success'  => 'Özellik başarıyla bulundu',
                    ],
                ],
                'comments' => [
                    'create' => [
                        'create-success' => 'Yorum başarıyla eklendi',
                        'create-failure' => 'Yorum oluşturulamadı',
                    ],
                    'post-comment'    => 'Yorumu gönder',
                    'post-reply'      => 'Yanıtı gönder',
                    'reply'           => 'Yanıtla',
                    'edit-btn'        => 'Düzenle',
                    'delete-btn'      => 'Sil',
                    'save-btn'        => 'Kaydet',
                    'cancel-btn'      => 'İptal',
                    'add-reply'       => 'Yanıt ekle',
                    'add-comment'     => 'Yorum ekle',
                    'no-comments'     => 'Henüz yorum yok',
                    'not-found'       => 'Yorumlar bulunamadı',
                    'updated-success' => 'Yorum başarıyla güncellendi',
                    'update-failed'   => 'Yorum güncellenemedi',
                    'delete-success'  => 'Varlık yorumu başarıyla silindi',
                    'delete-failed'   => 'Varlık yorumu silinemedi',
                    'deleted-user'    => 'Silinen kullanıcı',
                ],
                'edit' => [
                    'title'                 => 'Varlığı düzenle',
                    'back'                  => 'Geri',
                    'previous'              => 'Önceki',
                    'next'                  => 'Sonraki',
                    'name'                  => 'Ad',
                    'value'                 => 'Değer',
                    'save-btn'              => 'Kaydet',
                    'file-name'             => 'Dosya Adı',
                    'file-info'             => 'Dosya Bilgisi',
                    'type'                  => 'Tür',
                    'size'                  => 'Boyut',
                    'dimensions'            => 'Boyutlar',
                    'path'                  => 'Yol',
                    'created-at'            => 'Oluşturuldu',
                    'updated-at'            => 'Güncellendi',
                    'embedded_meta_info'    => 'Gömülü meta bilgisi',
                    'no-metadata-available' => 'Kullanılabilir meta veri yok',
                    'tags'                  => 'Etiketler',
                    'select-tags'           => 'Bir etiket seçin veya oluşturun',
                    'directory-path'        => 'Dizin Yolu',
                    'no-directory'          => 'Dizin atanmadı',
                    'details'               => 'Detaylar',
                    'breadcrumb-assets'     => 'Varlıklar',
                    'tab'                   => [
                        'preview'          => 'Önizleme',
                        'properties'       => 'Özellikler',
                        'comments'         => 'Yorumlar',
                        'linked_resources' => 'Bağlı kaynaklar',
                        'history'          => 'Geçmiş',
                    ],
                    'button' => [
                        'download'            => 'İndir',
                        'custom_download'     => 'Özel indirme',
                        'rename'              => 'Yeniden adlandır',
                        're_upload'           => 'Yeniden yükle',
                        're_uploading'        => 'Yeniden yükleniyor...',
                        'cancel'              => 'İptal',
                        're-upload-cancelled' => 'Yeniden yükleme iptal edildi.',
                        'delete'              => 'Sil',
                        'share'               => 'Paylaş',
                    ],
                    'share' => [
                        'title'      => 'Varlığı paylaş',
                        'link'       => 'Paylaşılabilir bağlantı',
                        'copy'       => 'Kopyala',
                        'copied'     => 'Bağlantı panoya kopyalandı',
                        'expiry'     => 'Bağlantı şu süre sonra sona erer',
                        'expiry-1d'  => '1 gün',
                        'expiry-7d'  => '7 gün',
                        'expiry-30d' => '30 gün',
                        'expires-on' => 'Son kullanma tarihi',
                        'failed'     => 'Paylaşım bağlantısı oluşturulamadı.',
                    ],
                    'preview-modal' => [
                        'not-available' => 'Bu dosya türü için önizleme mevcut değil.',
                        'download-file' => 'Dosyayı İndir',
                        'download-zip'  => 'ZIP olarak indir',
                        'close'         => 'Önizlemeyi kapat',
                        'mime'          => 'MIME',
                        'card'          => [
                            'click-for-details' => 'Tüm detaylar için tıklayın',
                            'preview'           => 'Önizleme',
                            'edit-image'        => 'Görüntüyü düzenle',
                        ],
                        'image-viewer' => [
                            'rotate-left'   => 'Sola döndür (L)',
                            'rotate-right'  => 'Sağa döndür (R)',
                            'zoom-out'      => 'Uzaklaştır (-)',
                            'zoom-in'       => 'Yaklaştır (+)',
                            'fit-to-screen' => 'Ekrana sığdır',
                            'actual-size'   => 'Gerçek boyut',
                            'reset-all'     => 'Tümünü sıfırla (0)',
                        ],
                        'video-player' => [
                            'back-10s'           => '10 s geri',
                            'forward-10s'        => '10 s ileri',
                            '10s'                => '10 s',
                            'speed'              => 'Hız',
                            'loop'               => 'Döngü (L)',
                            'mute'               => 'Sessiz (M)',
                            'picture-in-picture' => 'Resim içinde resim',
                            'fullscreen'         => 'Tam ekran (F)',
                            'more-actions'       => 'Daha fazla eylem',
                            'copy-link'          => 'Bağlantıyı kopyala',
                            'link-copied'        => 'Kopyalandı!',
                            'open-in-new-tab'    => 'Yeni sekmede aç',
                        ],
                    ],
                    'custom-download' => [
                        'title'              => 'Özel indirme',
                        'format'             => 'Biçim',
                        'width'              => 'Genişlik (px)',
                        'width-placeholder'  => '200',
                        'height'             => 'Yükseklik (px)',
                        'height-placeholder' => '200',
                        'download-btn'       => 'İndir',
                        'extension-types'    => [
                            'jpg'      => 'JPG',
                            'png'      => 'PNG',
                            'jpeg'     => 'JPEG',
                            'webp'     => 'WEBP',
                            'original' => 'Orijinal',
                        ],
                    ],
                    'tag-already-exists'        => 'Etiket zaten mevcut',
                    'image-source-not-readable' => 'Görüntü kaynağı okunamıyor',
                    'failed-to-read'            => 'Görüntü meta verileri okunamadı :exception',
                    'file-re-upload-success'    => 'Dosyalar başarıyla yeniden yüklendi.',
                    'image-processing-failed'   => 'Görüntü işleme başarısız: :message',
                    'image-editor'              => [
                        'tools'                    => 'Araçlar',
                        'close'                    => 'Düzenleyiciyi kapat',
                        'crop'                     => 'Kırp ve Yeniden Boyutlandır',
                        'crop-sub'                 => 'Görseli kırp veya ölçekle',
                        'adjust'                   => 'Parlaklık ve Kontrast',
                        'adjust-sub'               => 'Işık ve tonu ayarla',
                        'rotate'                   => 'Döndür ve Çevir',
                        'rotate-sub'               => 'Yönelimi dönüştür',
                        'selection'                => 'Seçim',
                        'drag-handles'             => 'sürükleme tutamaçları',
                        'scale-after-crop'         => 'Kırpma sonrası ölçekle',
                        'optional'                 => 'isteğe bağlı',
                        'width-px'                 => 'Genişlik (px)',
                        'height-px'                => 'Yükseklik (px)',
                        'blank-keep-dims'          => 'Kırpılmış boyutları korumak için boş bırakın.',
                        'auto'                     => 'otomatik',
                        'adjustments'              => 'Ayarlamalar',
                        'brightness'               => 'Parlaklık',
                        'contrast'                 => 'Kontrast',
                        'rotation'                 => 'Döndürme',
                        'flip'                     => 'Çevir',
                        'horizontal'               => 'Yatay',
                        'vertical'                 => 'Dikey',
                        'platform'                 => 'Platform',
                        'platform-loading'         => 'Yükleniyor…',
                        'no-platforms'             => 'Platform yok',
                        'color-none'               => 'Yok',
                        'color-white'              => 'Beyaz',
                        'color-silver'             => 'Gümüş',
                        'color-light-gray'         => 'Açık gri',
                        'color-gray'               => 'Gri',
                        'color-slate'              => 'Arduvaz grisi',
                        'color-dark-gray'          => 'Koyu gri',
                        'color-charcoal'           => 'Kömür',
                        'color-black'              => 'Siyah',
                        'color-light-red'          => 'Açık kırmızı',
                        'color-red'                => 'Kırmızı',
                        'color-light-yellow'       => 'Açık sarı',
                        'color-yellow'             => 'Sarı',
                        'color-light-green'        => 'Açık yeşil',
                        'color-green'              => 'Yeşil',
                        'color-light-cyan'         => 'Açık camgöbeği',
                        'color-cyan'               => 'Camgöbeği',
                        'color-light-blue'         => 'Açık mavi',
                        'color-blue'               => 'Mavi',
                        'color-light-purple'       => 'Açık mor',
                        'color-purple'             => 'Mor',
                        'color-light-pink'         => 'Açık pembe',
                        'color-pink'               => 'Pembe',
                        'color-light-rose'         => 'Açık gül',
                        'color-rose'               => 'Gül',
                        'model'                    => 'Model',
                        'no-models'                => 'Model yok',
                        'prompt'                   => 'İstem',
                        'apply'                    => 'Uygula',
                        'applying'                 => 'Uygulanıyor…',
                        'error-platforms'          => 'YZ platformları yüklenemedi.',
                        'error-select-color'       => 'Lütfen bir arka plan rengi seçin.',
                        'error-operation'          => 'İşlem başarısız oldu. Lütfen tekrar deneyin.',
                        'success-updated'          => 'Görsel başarıyla güncellendi.',
                        'success-adjusted'         => 'Görsel başarıyla ayarlandı.',
                        'success-transformed'      => 'Görsel başarıyla dönüştürüldü.',
                        'error-no-ai-image'        => 'YZ görsel döndürmedi.',
                        'error-provide-dims'       => 'En az bir kırpma bölgesi veya çıktı boyutları belirtin.',
                        'edit-bg'                  => 'Arka planı düzenle',
                        'edit-bg-sub'              => 'Renk, yükleme veya istem',
                        'bg-tab-color'             => 'Renk',
                        'bg-tab-upload'            => 'Yükle',
                        'bg-tab-ai'                => 'İstem',
                        'bg-color-label'           => 'Arka plan rengi',
                        'bg-color-mode-normal'     => 'Normal',
                        'bg-color-mode-ai'         => 'AI',
                        'bg-custom-color'          => 'Özel',
                        'bg-upload-hint'           => 'Arka plan resmi yüklemek için tıklayın',
                        'bg-upload-change'         => 'Değiştirmek için tıklayın',
                        'bg-ai-prompt-placeholder' => 'İstediğiniz arka planı açıklayın…',
                        'sharpen'                  => 'Keskinleştir',
                        'blur'                     => 'Bulanıklaştır',
                        'filters'                  => 'Filtreler',
                        'filters-sub'              => 'Gri tonlama ve tersine çevirme',
                        'greyscale'                => 'Gri tonlama',
                        'invert'                   => 'Tersine çevir',
                        'error-no-filter'          => 'Uygulanacak en az bir filtre seçin.',
                        'error-provider-no-images' => 'Bu AI sağlayıcısı görüntü oluşturmayı desteklemiyor.',
                        'error-no-image-platform'  => 'Görüntü oluşturma desteği olan AI platformu bulunamadı. Bir OpenAI, Gemini veya xAI platformu yapılandırın.',
                    ],
                ],
                'linked-resources' => [
                    'index' => [
                        'datagrid' => [
                            'product'       => 'Ürün',
                            'category'      => 'Kategori',
                            'product-sku'   => 'Ürün Sku: ',
                            'category code' => 'Kategori kodu: ',
                            'resource-type' => 'Kaynak türü',
                            'resource'      => 'Kaynak',
                            'resource-view' => 'Kaynak görünümü',
                        ],
                    ],
                    'found-success' => 'Kaynak başarıyla bulundu',
                    'not-found'     => 'Kaynak bulunamadı',
                ],
                'tags' => [
                    'create' => [
                        'create-success' => 'Etiketler başarıyla eklendi',
                    ],
                    'found-success'  => 'Etiket başarıyla bulundu',
                    'not-found'      => 'Etiketler bulunamadı',
                    'delete-success' => 'Varlık etiketleri başarıyla kaldırıldı',
                ],
                'delete-success'                          => 'Varlık başarıyla silindi',
                'delete-failed-due-to-attached-resources' => 'Varlık kullanımda. Silmeden önce bağlantıyı kaldırın',
                'datagrid'                                => [
                    'mass-delete-success'                 => 'Toplu silme başarıyla tamamlandı.',
                    'mass-update-success'                 => 'Toplu güncelleme başarılı.',
                    'files-upload-success'                => 'Dosyalar başarıyla yüklendi.',
                    'file-upload-success'                 => 'Dosya başarıyla yüklendi.',
                    'not-found'                           => 'Dosya bulunamadı',
                    'not-found-to-show'                   => 'Görüntüleme için dosya bulunamadı',
                    'not-found-to-download'               => 'İndirme için dosya bulunamadı',
                    'edit-success'                        => 'Dosya başarıyla yüklendi',
                    'show-success'                        => 'Dosya başarıyla bulundu',
                    'update-success'                      => 'Dosya başarıyla güncellendi',
                    'not-found-to-update'                 => 'Dosya mevcut değil',
                    'not-found-to-destroy'                => 'Dosya mevcut değil',
                    'files-upload-failed'                 => 'Dosyalar yüklenemedi.',
                    'file-upload-failed'                  => 'Dosya yüklenemedi',
                    'invalid-file'                        => 'Geçersiz dosya sağlandı',
                    'invalid-file-format-or-not-provided' => 'Dosya sağlanmadı veya geçersiz biçim.',
                    'file-process-failed'                 => 'Bazı dosyalar işlenemedi',
                    'file-forbidden-type'                 => 'Dosyanın türü veya uzantısı yasaklı.',
                    'file-too-large'                      => 'Dosya çok büyük. İzin verilen maksimum boyut: :size.',
                    'storage-upload-failed'               => '":name" depolamaya yüklenemedi. Disk kimlik bilgilerini ve izinleri kontrol edin.',
                    'not-found-or-no-file'                => 'Varlık bulunamadı veya dosya mevcut değil.',
                    'download-link-ready'                 => 'Varlık bulundu. Dosyayı verilen bağlantıdan indirebilirsiniz.',
                ],
            ],
            'share' => [
                'target-not-found' => 'Paylaşılacak varlık veya dizin bulunamadı.',
                'not-found'        => 'Paylaşım bağlantısı bulunamadı.',
                'revoked'          => 'Paylaşım bağlantısı iptal edildi.',
                'already-revoked'  => 'Bu paylaşım bağlantısı zaten iptal edilmiş.',
                'deleted'          => 'Paylaşım bağlantısı silindi.',
                'reauthorized'     => 'Paylaşım bağlantısı yeniden yetkilendirildi — orijinal URL tekrar aktif.',
                'not-revoked'      => 'Bu paylaşım bağlantısı iptal edilmemiş.',
                'updated'          => 'Paylaşım bağlantısı güncellendi.',
                'update-failed'    => 'Paylaşım bağlantısı güncellenemedi.',
                'delete-failed'    => 'Paylaşım bağlantısı silinemedi.',
                'modal'            => [
                    'title-asset'        => 'Varlığı paylaş',
                    'title-directory'    => 'Dizini paylaş',
                    'edit-title'         => 'Paylaşım bağlantısını düzenle',
                    'edit-name'          => 'Paylaşımı yeniden adlandır',
                    'unnamed-share'      => '(özel ad yok)',
                    'name-label'         => 'Özel ad',
                    'name-hint'          => 'Etiket olarak orijinal dosya / dizin adını korumak için boş bırakın.',
                    'save'               => 'Değişiklikleri kaydet',
                    'saving'             => 'Kaydediliyor…',
                    'cancel'             => 'İptal',
                    'create-new'         => 'Yeni paylaşım bağlantısı oluştur',
                    'expiry'             => 'Bağlantı şu süre sonra sona erer',
                    'expiry-1d'          => '1 gün',
                    'expiry-7d'          => '7 gün',
                    'expiry-30d'         => '30 gün',
                    'expiry-365d'        => '1 yıl',
                    'no-expiry'          => 'Son kullanma tarihi yok',
                    'create'             => 'Bağlantı oluştur',
                    'creating'           => 'Oluşturuluyor…',
                    'active-links'       => 'Aktif bağlantılar',
                    'loading'            => 'Yükleniyor…',
                    'no-active-links'    => 'Bu öğe için henüz aktif paylaşım bağlantısı yok.',
                    'copy'               => 'Kopyala',
                    'copied'             => 'Bağlantı panoya kopyalandı',
                    'revoke'             => 'İptal et',
                    'revoked'            => 'Paylaşım bağlantısı iptal edildi',
                    'revoke-failed'      => 'Paylaşım bağlantısı iptal edilemedi.',
                    'expires-on'         => 'Son kullanma tarihi',
                    'never-expires'      => 'Hiçbir zaman sona ermez',
                    'views'              => 'Görüntülemeler',
                    'downloads'          => 'İndirmeler',
                    'created'            => 'Paylaşım bağlantısı oluşturuldu.',
                    'create-failed'      => 'Paylaşım bağlantısı oluşturulamadı.',
                    'load-failed'        => 'Mevcut paylaşım bağlantıları yüklenemedi.',
                    'advanced'           => 'Gelişmiş',
                    'delete'             => 'Sil',
                    'reauthorize'        => 'Yeniden Yetkilendir',
                    'reauthorize-failed' => 'Paylaşım bağlantısı yeniden yetkilendirilemedi.',
                    'reauthorizing'      => 'Yeniden yetkilendiriliyor…',
                    'revoked-status'     => 'Bağlantı iptal edildi',
                    'revoked-notice'     => 'Bu bağlantı iptal edildi. Orijinal URL\'yi tekrar aktif hale getirmek için yeniden yetkilendirin.',
                ],
                'index' => [
                    'title'       => 'Paylaşılan Bağlantılar',
                    'description' => 'DAM\'daki varlıklar ve dizinler için tüm aktif ve geçmiş paylaşım bağlantıları.',
                ],
                'datagrid' => [
                    'type'           => 'Tür',
                    'asset'          => 'Varlık',
                    'directory'      => 'Dizin',
                    'target'         => 'Hedef',
                    'created-by'     => 'Oluşturan',
                    'expires-at'     => 'Son kullanma',
                    'never'          => 'Hiçbir zaman',
                    'status'         => 'Durum',
                    'status-active'  => 'Aktif',
                    'status-expired' => 'Süresi dolmuş',
                    'status-revoked' => 'İptal edildi',
                    'views'          => 'Görüntülemeler',
                    'downloads'      => 'İndirmeler',
                    'created-at'     => 'Oluşturuldu',
                    'edit'           => 'Düzenle',
                    'copy-link'      => 'Bağlantıyı kopyala',
                    'revoke'         => 'İptal et',
                    'custom-name'    => 'Özel Ad',
                    'delete'         => 'Sil',
                ],
            ],
            'upload' => [
                'pause'     => 'Duraklat',
                'resume'    => 'Devam Et',
                'retry'     => 'Yeniden Dene',
                'cancel'    => 'İptal',
                'paused'    => 'Yükleme duraklatıldı. Mevcut varlıklar etkilenmedi — istediğiniz zaman devam edin.',
                'resumed'   => 'Yükleme devam ettirildi.',
                'cancelled' => 'Yükleme iptal edildi. Daha önce yüklenen varlıklar korundu.',
                'retried'   => 'Başarısız yüklemeler arka planda yeniden deneniyor.',
            ],
            'tag' => [
                'not-found'           => 'Etiket bulunamadı.',
                'create-success'      => 'Etiket başarıyla oluşturuldu.',
                'update-success'      => 'Etiket başarıyla güncellendi.',
                'delete-success'      => 'Etiket başarıyla silindi.',
                'mass-delete-success' => 'Seçili etiketler başarıyla silindi.',
                'index'               => [
                    'title'       => 'Etiketler',
                    'description' => 'DAM genelinde varlıkları etiketlemek ve düzenlemek için kullanılan etiketleri yönetin.',
                    'create'      => 'Etiket Oluştur',
                ],
                'datagrid' => [
                    'name'         => 'Ad',
                    'assets-count' => 'Varlıklar',
                    'created-at'   => 'Oluşturulma tarihi',
                    'edit'         => 'Düzenle',
                    'delete'       => 'Sil',
                ],
                'modal' => [
                    'create-title'     => 'Etiket Oluştur',
                    'edit-title'       => 'Etiketi Düzenle',
                    'name-label'       => 'Etiket adı',
                    'name-placeholder' => 'ör. yaz-kampanyası',
                    'save'             => 'Kaydet',
                    'saving'           => 'Kaydediliyor…',
                    'cancel'           => 'İptal',
                ],
                'mass-action' => [
                    'assign-tags'              => 'Etiket Ata',
                    'modal-title'              => 'Etiket Ata',
                    'modal-subtitle'           => 'Seçili :count varlığa etiket ekleyin. Mevcut etiketler korunur.',
                    'modal-subtitle-recursive' => 'Seçili varlıklara ve seçili klasörlerin içindeki her varlığa, özyinelemeli olarak etiket ekleyin. Mevcut etiketler korunur.',
                    'tags-label'               => 'Etiketler',
                    'tags-placeholder'         => 'Etiket ara veya oluşturmak için yazın…',
                    'add-tag'                  => 'Eklemek için Enter\'a basın',
                    'assign'                   => 'Ata',
                    'assigning'                => 'Atanıyor…',
                    'cancel'                   => 'İptal',
                    'assign-success'           => 'Etiketler :count varlığa atandı.',
                    'assign-failed'            => 'Etiketler atanamadı.',
                    'no-assets'                => 'Hiçbir varlık seçilmedi.',
                    'no-items'                 => 'En az bir varlık veya klasör seçin.',
                    'no-tags'                  => 'Lütfen en az bir etiket seçin veya ekleyin.',
                ],
            ],
        ],
        'catalog' => [
            'attributes' => [
                'type' => [
                    'asset' => 'Varlık',
                ],
            ],
            'category-fields' => [
                'type' => [
                    'asset' => 'Varlık',
                ],
            ],
        ],
        'acl' => [
            'menu'             => 'DAM',
            'asset'            => 'Varlık',
            'property'         => 'Özellik',
            'comment'          => 'Yorum',
            'linked_resources' => 'Bağlı kaynaklar',
            'meta_data'        => 'Gömülü Meta Bilgisi',
            'directory'        => 'Dizin',
            'tag'              => 'Etiket',
            'create'           => 'Oluştur',
            'edit'             => 'Düzenle',
            'update'           => 'Güncelle',
            'delete'           => 'Delete',
            'list'             => 'Liste',
            'view'             => 'Görüntüle',
            'upload'           => 'Yükle',
            're_upload'        => 'Yeniden yükle',
            'mass_update'      => 'Toplu güncelleme',
            'mass_delete'      => 'Toplu silme',
            'download'         => 'İndir',
            'custom_download'  => 'Özel indirme',
            'rename'           => 'Yeniden adlandır',
            'move'             => 'Taşı',
            'copy'             => 'Kopyala',
            'copy-structure'   => 'Dizin yapısını kopyala',
            'download-zip'     => 'Zip olarak indir',
            'asset-assign'     => 'Varlık ata',
            'share'            => 'Paylaş',
            'assets'           => 'Varlıklar',
            'shares'           => 'Paylaşılan Bağlantılar',
            'revoke'           => 'İptal et',
            'tags'             => 'Etiketler',
            'configuration'    => 'Yapılandırma',
        ],
        'permissions' => [
            'title'                 => 'DAM Dizin İzinleri',
            'tab-subtitle'          => 'Bu rolün erişebileceği dizinler.',
            'no-actions'            => 'Yalnızca görüntüleme. Erişiminiz olan bir alt klasörü açın.',
            'unauthorized'          => 'DAM dizin izinlerini yönetme yetkiniz yok.',
            'all-directories'       => 'Tüm Dizinler — şimdi ve gelecekte tüm dizinlere erişim ver',
            'inherit-children'      => 'Alt dizinleri devral — verilen dizinlerdeki tüm alt dizinlere otomatik olarak eriş',
            'loading'               => 'Dizinler yükleniyor…',
            'empty'                 => 'Hiçbir dizin bulunamadı.',
            'error'                 => 'Dizinler yüklenemedi. Lütfen tekrar deneyin.',
            'retry'                 => 'Yeniden Dene',
            'grants-load-warning'   => 'Bazı dizin izinleri yüklenemedi. Kaydetmeden önce seçimlerinizi doğrulayın.',
            'inherit-children-note' => 'Alt dizinler kayıt sırasında otomatik olarak dahil edilecektir.',
        ],
        'validation' => [
            'asset' => [
                'required' => ':attribute alanı zorunludur.',
            ],
            'comment' => [
                'required' => 'Yorum mesajı zorunludur.',
            ],
            'property' => [
                'name' => [
                    'required' => 'Ad alanı zorunludur.',
                    'unique'   => 'Bu ad zaten alınmış.',
                ],
                'language' => [
                    'not-found' => 'Seçilen dil bulunamadı veya şu anda devre dışı.',
                ],
            ],
        ],
        'errors' => [
            '401' => 'Bu işlem için yetkiniz yok.',
        ],
        'configuration' => [
            'save-btn' => 'Kaydet',
            'title'    => 'DAM Yapılandırması',
            'saved'    => 'Yapılandırma başarıyla kaydedildi.',
            'general'  => [
                'title'            => 'Genel Ayarlar',
                'description'      => 'DAM medya kütüphanesi için dizin ağacı görünürlüğünü, gezgin görünümünü ve yer işaretleri panelini yapılandırın.',
                'tree-show-assets' => [
                    'label' => 'Dizin Ağacında Varlıkları Göster',
                    'hint'  => 'Etkinleştirildiğinde, varlık dosyaları dizin ağacında yaprak düğümler olarak görünür.',
                ],
                'explorer-enabled' => [
                    'label' => 'Explorer Görünümünü Etkinleştir',
                    'hint'  => 'Varsayılan varlık ızgarasını çok sekmeli klasör gezginiyle değiştirir.',
                ],
                'bookmarks-enabled' => [
                    'label' => 'Yer İşaretleri Panelini Etkinleştir',
                    'hint'  => 'Hızlı gezinme için dizin ağacının altında bir yer işaretleri paneli gösterir.',
                ],
                'show-tree' => [
                    'label' => 'Dizin ağacını göster',
                    'hint'  => 'Etkinleştirildiğinde, dizin ağacı kenar çubuğu gezgininin yanında görüntülenir.',
                ],
            ],
        ],
        'explorer' => [
            'not-found'        => 'Dizin bulunamadı.',
            'access-denied'    => 'Bu dizine erişim izniniz yok.',
            'empty'            => 'Dizin bulunamadı.',
            'action-completed' => 'İşlem başarıyla tamamlandı.',
            'bookmarks'        => [
                'title'     => 'Yer İşaretleri',
                'drag-hint' => 'Yer işaretine eklemek için bir klasörü buraya sürükleyin',
                'max'       => 'Maksimum 20 yer işaretine ulaşıldı.',
                'remove'    => 'Yer imini kaldır',
                'empty'     => 'Henüz yer imi yok.',
            ],
            'tab' => [
                'new'   => 'Yeni Sekme',
                'close' => 'Sekmeyi Kapat',
            ],
            'sections' => [
                'folders' => 'Klasörler',
                'files'   => 'Dosyalar',
                'folder'  => 'Klasör',
            ],
            'clipboard' => [
                'ready'   => 'Yapıştırmaya hazır',
                'dismiss' => 'Temizle',
            ],
            'context' => [
                'open'                 => 'Aç',
                'open-new-tab'         => 'Yeni sekmede aç',
                'bookmark'             => 'Dizini Yer İşaretine Ekle',
                'folder-upload'        => 'Klasör Yükle',
                'copy'                 => 'Kopyala',
                'paste'                => 'Yapıştır',
                'copy-progress'        => 'Kopyalama kuyruğa alındı.',
                'copy-done'            => 'Kopyalandı.',
                'paste-file-started'   => 'Dosya yapıştırılıyor...',
                'paste-folder-started' => 'Klasör yapıştırılıyor...',
                'paste-done'           => 'Başarıyla yapıştırıldı.',
                'move-asset-started'   => 'Dosya taşınıyor...',
                'move-folder-started'  => 'Klasör taşınıyor...',
                'move-done'            => 'Klasör başarıyla taşındı.',
                'move-asset-done'      => 'Dosya başarıyla taşındı.',
                'delete-done'          => 'Dizin başarıyla silindi.',
            ],
            'folder' => [
                'deleted' => 'Bu klasör silindi — Kök dizine dönüldü.',
            ],
            'search' => [
                'placeholder' => 'Dosya ve klasör ara…',
            ],
            'list' => [
                'header' => [
                    'name'     => 'Ad',
                    'type'     => 'Tür',
                    'size'     => 'Boyut',
                    'modified' => 'Değiştirilme',
                    'actions'  => 'İşlemler',
                ],
                'items-count' => ':count öğe',
            ],
            'dialog' => [
                'rename-asset' => [
                    'title'       => 'Dosyayı Yeniden Adlandır',
                    'placeholder' => 'Yeni ad',
                ],
                'save'   => 'Kaydet',
                'cancel' => 'İptal',
            ],
            'view' => [
                'grid' => 'Izgara Görünümü',
                'list' => 'Liste Görünümü',
            ],
            'mass-actions' => [
                'select-count'   => ':count seçildi',
                'select-action'  => 'Eylem Seçin',
                'delete'         => 'Sil',
                'confirm'        => ':count öğe silinsin mi? Klasörler yinelemeli olarak silinir ve bu işlem geri alınamaz.',
                'deleting'       => ':count öğe siliniyor...',
                'deleted-assets' => ':count varlık silindi.',
                'deleted-dirs'   => ':count klasör silme kuyruğuna alındı.',
                'move-to'        => 'Taşı…',
                'copy-to'        => 'Kopyala…',
                'moving'         => 'Taşınıyor…',
                'copying'        => 'Kopyalanıyor…',
                'moved-assets'   => ':count dosya taşındı.',
                'copied-assets'  => ':count dosya kopyalandı.',
                'moved-dirs'     => ':count klasör taşındı.',
                'copied-dirs'    => ':count klasör kopyalandı.',
                'pick-dest'      => 'Hedef seç',
                'select-here'    => 'Buraya seç',
                'new-folder'     => 'Yeni klasör',
                'folder-name'    => 'Klasör adı',
                'create-error'   => 'Klasör oluşturulamadı.',
                'move-done'      => ':count Dosya taşındı.',
                'copy-done'      => ':count Dosya kopyalandı.',
                'still-running'  => 'İşlem arka planda hâlâ devam ediyor',
                'progress'       => 'İlerleme',
                'total-files'    => ':count dosya',
            ],
            'toolbar' => [
                'toggle-tree'    => 'Dizin ağacını aç/kapat',
                'toggle-sidebar' => 'Kenar çubuğunu aç/kapat',
            ],
            'upload' => [
                'queued'              => 'Sırada',
                'uploading'           => 'Yükleniyor',
                'creating'            => 'Klasör oluşturuluyor',
                'done'                => 'Tamamlandı',
                'failed'              => 'Başarısız',
                'interrupted'         => 'Kesildi — yeniden denemek için tekrar ekleyin',
                'resuming'            => 'Devam ediyor…',
                'overall'             => ':total öğeden :done yüklendi',
                'too-large-to-resume' => 'Toplu iş, yenilemeden sonra otomatik devam ettirilemeyecek kadar büyük',
            ],
        ],
    ],
    'share' => [
        'public' => [
            'app-name'              => 'DAM',
            'download'              => 'İndir',
            'back-to-gallery'       => 'Galeriye geri dön',
            'back'                  => 'Geri',
            'file-name'             => 'Dosya adı',
            'file-type'             => 'Dosya türü',
            'file-size'             => 'Dosya boyutu',
            'video-not-supported'   => 'Tarayıcınız video etiketini desteklemiyor.',
            'audio-not-supported'   => 'Tarayıcınız audio etiketini desteklemiyor.',
            'preview-not-available' => 'Bu dosya için önizleme mevcut değil. Yukarıdaki İndir düğmesini kullanın.',
            'files-count'           => ':count dosya',
            'empty-directory'       => 'Bu paylaşılan dizinde henüz dosya yok.',
            'expires-on'            => 'Bu bağlantının son kullanma tarihi',
            'powered-by-dam'        => 'Unopim DAM tarafından desteklenmektedir',
            'expired-title'         => 'Bağlantı süresi doldu',
            'expired-message'       => 'Bu paylaşım bağlantısının süresi dolmuş ve artık erişilemiyor. Lütfen sahipten yeni bir bağlantı isteyin.',
            'revoked-title'         => 'Bağlantı iptal edildi',
            'revoked-message'       => 'Bu paylaşım bağlantısı sahibi tarafından iptal edilmiş ve artık erişilemiyor.',
            'not-found-title'       => 'Bağlantı bulunamadı',
            'not-found-message'     => 'Açtığınız paylaşım bağlantısı mevcut değil. Yanlış yazılmış veya kaldırılmış olabilir.',
            'download-zip'          => 'Tümünü ZIP olarak indir',
            'per-page'              => 'Sayfa başına',
        ],
    ],
];
