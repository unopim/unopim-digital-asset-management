<?php

return [
    'admin' => [
        'components' => [
            'layouts' => [
                'sidebar' => [
                    'dam'           => 'Aktivstyring',
                    'assets'        => 'Aktiver',
                    'shares'        => 'Delte links',
                    'tags'          => 'Tags',
                    'configuration' => 'Konfiguration',
                ],
            ],
            'modal' => [
                'confirm' => [
                    'message' => 'Hvis du sletter denne mappe, slettes også alle undermapper i den. Denne handling er permanent og kan ikke fortrydes.',
                ],
            ],
            'asset' => [
                'field' => [
                    'add-asset'     => 'Tilføj aktiv',
                    'assign-assets' => 'Tildel aktiver',
                    'preview'       => 'Forhåndsvisning',
                    'remove'        => 'Fjern',
                    'download'      => 'Download',
                ],
            ],
        ],
        'dam' => [
            'file' => [
                'deleted'   => 'Fil slettet',
                'not-found' => 'Fil ikke fundet',
            ],
            'image-edit' => [
                'cannot-decode'  => 'Kan ikke afkode billede',
                'preview-failed' => 'Forhåndsvisning mislykkedes',
            ],
            'index' => [
                'title'    => 'Digital aktivstyring',
                'datagrid' => [
                    'file-name'      => 'Filnavn',
                    'tags'           => 'Tags',
                    'property-name'  => 'Egenskabsnavn',
                    'property-value' => 'Egenskabsværdi',
                    'created-at'     => 'Oprettet den',
                    'updated-at'     => 'Opdateret den',
                    'extension'      => 'Filendelse',
                    'path'           => 'Sti',
                ],
                'mass-action' => [
                    'deleting'   => 'Sletter :count aktiver…',
                    'processing' => 'Behandler :count aktiver…',
                ],
                'move' => [
                    'directory' => 'Flytter mappen ":name"…',
                    'asset'     => 'Flytter aktivet ":name"…',
                ],
                'directory' => [
                    'title'  => 'Mappe',
                    'create' => [
                        'title'     => 'Opret mappe',
                        'name'      => 'Navn',
                        'file_name' => 'Filnavn',
                        'save-btn'  => 'Gem mappe',
                    ],
                    'rename' => [
                        'title' => 'Omdøb mappe',
                    ],
                    'search' => [
                        'placeholder'     => 'Søg i mapper...',
                        'no-matches'      => 'Ingen resultater',
                        'error'           => 'Søgning mislykkedes — prøv igen',
                        'not-found-flash' => 'Mappen er ikke længere tilgængelig',
                        'count'           => ':shown af :total match',
                        'count-total'     => ':total match',
                    ],
                    'asset' => [
                        'rename' => [
                            'title'    => 'Omdøb aktiv',
                            'save-btn' => 'Gem aktiv',
                        ],
                    ],
                    'actions' => [
                        'delete'                    => 'Slet',
                        'edit'                      => 'Rediger',
                        'rename'                    => 'Omdøb',
                        'copy'                      => 'Kopiér',
                        'cut'                       => 'Klip',
                        'download'                  => 'Download',
                        'download-zip'              => 'Download zip',
                        'paste'                     => 'Indsæt',
                        'add-directory'             => 'Tilføj mappe',
                        'upload-files'              => 'Upload filer',
                        'upload-files-and-folders'  => 'Upload filer og mapper',
                        'upload-folder'             => 'Upload mappe',
                        'upload-drop-hint'          => 'Træk filer eller mapper hertil',
                        'upload-or'                 => 'eller gennemse:',
                        'copy-directory-structured' => 'Kopiér mappestruktur',
                        'get'                       => 'Get',
                        'get-by-id'                 => 'Hent efter id',
                        'comment'                   => 'Kommentar',
                        'share'                     => 'Del mappe',
                    ],
                    'linked-resources'                          => 'Tilknyttede ressourcer',
                    'not-found'                                 => 'Ingen mappe fundet',
                    'created-success'                           => 'Mappe oprettet',
                    'creation-failed'                           => 'Oprettelse af mappe mislykkedes',
                    'updated-success'                           => 'Mappe opdateret',
                    'update-failed'                             => 'Opdatering af mappe mislykkedes',
                    'moved-success'                             => 'Mappe flyttet',
                    'cannot-move'                               => 'Kan ikke flytte mappe',
                    'cannot-copy'                               => 'En mappe kan ikke kopieres ind i sig selv eller i en af sine egne undermapper.',
                    'fetch-all-success'                         => 'Mapper hentet',
                    'can-not-deleted'                           => 'Mappen kan ikke slettes, da det er rodmappen.',
                    'deleting-in-progress'                      => 'Sletning af mappe er i gang',
                    'can-not-copy'                              => 'Mappen kan ikke kopieres, da det er rodmappen.',
                    'coping-in-progress'                        => 'Kopiering af mappestruktur er i gang.',
                    'asset-not-found'                           => 'Intet aktiv fundet',
                    'asset-renamed-success'                     => 'Aktiv omdøbt',
                    'asset-moved-success'                       => 'Aktiv flyttet',
                    'asset-name-already-exist'                  => 'Det nye navn findes allerede hos et andet aktiv ved navn :asset_name',
                    'asset-name-conflict-in-the-same-directory' => 'Aktivets navn er i konflikt med en eksisterende fil i samme mappe.',
                    'old-file-not-found'                        => 'Filen ved stien :old_path blev ikke fundet.',
                    'image-name-is-the-same'                    => 'Dette navn findes allerede. Indtast venligst et andet.',
                    'not-writable'                              => 'Du har ikke tilladelse til at :actionType en :type på denne placering ":path".',
                    'empty-directory'                           => 'Denne mappe er tom.',
                    'failed-download-directory'                 => 'Kunne ikke oprette zip-filen.',
                    'not-allowed'                               => 'Det er ikke tilladt at uploade scriptfiler.',
                    'copy-success'                              => 'Mappen er kopieret.',
                    'name-unique'                               => 'Mappenavnet skal være unikt i samme overordnede mappe.',
                    'folder-upload-success'                     => 'Mappe uploadet.',
                    'folder-upload-partial'                     => 'Mappe uploadet med :count fil(er) sprunget over på grund af fejl.',
                    'load-more'                                 => 'Indlæs flere',
                ],
                'description'                    => 'Værktøjet hjælper dig med at organisere, opbevare og administrere alle dine medieaktiver ét sted',
                'root'                           => 'Rod',
                'upload'                         => 'Upload',
                'new'                            => 'Ny',
                'uploading'                      => 'Uploader...',
                'cancel'                         => 'Annuller',
                'upload-cancelled'               => 'Upload annulleret.',
                'folder-upload-https-required'   => 'Mappeupload kræver en sikker forbindelse (HTTPS).',
                'folder-upload-no-https-warning' => 'Tomme mapper oprettes ikke uden en sikker forbindelse (HTTPS). Filer uploades stadig.',
                'folder-upload-no-files'         => 'Den/de valgte mappe(r) indeholder ingen filer. Tomme undermapper kan ikke registreres uden en sikker forbindelse (HTTPS).',
                'upload-in-progress-title'       => 'Upload i gang',
                'upload-in-progress-message'     => 'At forlade denne side vil annullere den aktive upload. Er du sikker på, at du vil forlade?',
                'upload-leave-page'              => 'Forlad',
                'upload-stay-page'               => 'Bliv',
                'drop-zone-hint'                 => 'Træk filer eller mapper for at uploade',
                'select-all'                     => 'Vælg alle',
                'select-directory-to-upload'     => 'Vælg en mappe at uploade til.',
                'upload-session'                 => [
                    'uploading'            => 'Uploader :total fil(er)…',
                    'paused'               => 'Sat på pause — :done af :total uploadet',
                    'interrupted'          => ':done af :total uploadet · :interrupted afbrudt',
                    'failed'               => ':done uploadet, :failed mislykkedes',
                    'completed'            => ':done af :total uploadet',
                    'create-folder-failed' => 'Kunne ikke oprette mappen',
                ],
                'drop-zone-no-permission'        => 'Du har ikke tilladelse til at uploade her.',
                'upload-complete'                => 'Upload fuldført',
                'item-upload-complete'           => 'Upload fuldført',
                'copy'                           => [
                    'directory' => 'Kopierer mappen ":name"…',
                ],
                'delete' => [
                    'directory' => 'Sletter mappen ":name"…',
                ],
            ],
            'asset' => [
                'properties' => [
                    'index' => [
                        'title'      => 'Aktivegenskaber',
                        'create-btn' => 'Opret egenskab',
                        'datagrid'   => [
                            'name'     => 'Navn',
                            'type'     => 'Type',
                            'language' => 'Sprog',
                            'value'    => 'Værdi',
                            'edit'     => 'Rediger',
                            'delete'   => 'Slet',
                        ],
                        'create' => [
                            'title'                  => 'Opret egenskab',
                            'name'                   => 'Navn',
                            'type'                   => 'Type',
                            'language'               => 'Sprog',
                            'value'                  => 'Værdi',
                            'add-to-filter'          => 'Tilføj til filter',
                            'add-to-filter-help'     => 'Når aktiveret, vises denne egenskab som et filter i sidebjælken for aktivlisten.',
                            'sort-order'             => 'Sorteringsrækkefølge',
                            'sort-order-help'        => 'Lavere tal vises først i filtreringssidebjælken. Egenskaber med samme tal sorteres alfabetisk.',
                            'sort-order-placeholder' => 'f.eks. 10',
                            'save-btn'               => 'Gem',
                        ],
                        'edit' => [
                            'title' => 'Rediger egenskab',
                        ],
                        'delete-success' => 'Aktivegenskab slettet',
                        'delete-failed'  => 'Sletning af aktivegenskab mislykkedes',
                        'create-success' => 'Aktivegenskab oprettet',
                        'update-success' => 'Aktivegenskab opdateret',
                        'update-failure' => 'Opdatering af aktivegenskab mislykkedes',
                        'not-found'      => 'Egenskab ikke fundet',
                        'found-success'  => 'Egenskab fundet',
                    ],
                ],
                'comments' => [
                    'create' => [
                        'create-success' => 'Kommentaren er tilføjet',
                        'create-failure' => 'Kommentaren kunne ikke oprettes',
                    ],
                    'post-comment'    => 'Send kommentar',
                    'post-reply'      => 'Send svar',
                    'reply'           => 'Svar',
                    'edit-btn'        => 'Rediger',
                    'delete-btn'      => 'Slet',
                    'save-btn'        => 'Gem',
                    'cancel-btn'      => 'Annuller',
                    'add-reply'       => 'Tilføj svar',
                    'add-comment'     => 'Tilføj kommentar',
                    'no-comments'     => 'Ingen kommentarer endnu',
                    'not-found'       => 'Kommentarer ikke fundet',
                    'updated-success' => 'Kommentar opdateret',
                    'update-failed'   => 'Kommentaren kunne ikke opdateres',
                    'delete-success'  => 'Aktivkommentar slettet',
                    'delete-failed'   => 'Aktivkommentaren kunne ikke slettes',
                    'deleted-user'    => 'Slettet bruger',
                ],
                'edit' => [
                    'title'                 => 'Rediger aktiv',
                    'back'                  => 'Tilbage',
                    'previous'              => 'Forrige',
                    'next'                  => 'Næste',
                    'name'                  => 'Navn',
                    'value'                 => 'Værdi',
                    'save-btn'              => 'Gem',
                    'file-name'             => 'Filnavn',
                    'file-info'             => 'Filoplysninger',
                    'type'                  => 'Type',
                    'size'                  => 'Størrelse',
                    'dimensions'            => 'Dimensioner',
                    'path'                  => 'Sti',
                    'created-at'            => 'Oprettet',
                    'updated-at'            => 'Opdateret',
                    'embedded_meta_info'    => 'Indlejrede metadata',
                    'no-metadata-available' => 'Ingen metadata tilgængelig',
                    'tags'                  => 'Tags',
                    'select-tags'           => 'Vælg eller opret et tag',
                    'tag-attach-success'    => 'Tag tilføjet.',
                    'tag-detach-success'    => 'Tag fjernet fra aktivet.',
                    'directory-path'        => 'Mappesti',
                    'no-directory'          => 'Ingen mappe tildelt',
                    'details'               => 'Detaljer',
                    'breadcrumb-assets'     => 'Aktiver',
                    'tab'                   => [
                        'preview'          => 'Forhåndsvisning',
                        'properties'       => 'Egenskaber',
                        'comments'         => 'Kommentarer',
                        'linked_resources' => 'Tilknyttede ressourcer',
                        'history'          => 'Historik',
                    ],
                    'button' => [
                        'download'            => 'Download',
                        'custom_download'     => 'Brugerdefineret download',
                        'rename'              => 'Omdøb',
                        're_upload'           => 'Upload igen',
                        're_uploading'        => 'Uploader igen...',
                        'cancel'              => 'Annuller',
                        're-upload-cancelled' => 'Genupload annulleret.',
                        'delete'              => 'Slet',
                        'share'               => 'Del',
                    ],
                    'share' => [
                        'title'      => 'Del aktiv',
                        'link'       => 'Delbart link',
                        'copy'       => 'Kopiér',
                        'copied'     => 'Link kopieret til udklipsholder',
                        'expiry'     => 'Link udløber efter',
                        'expiry-1d'  => '1 dag',
                        'expiry-7d'  => '7 dage',
                        'expiry-30d' => '30 dage',
                        'expires-on' => 'Udløber den',
                        'failed'     => 'Kunne ikke generere delingslink.',
                    ],
                    'preview-modal' => [
                        'not-available' => 'Forhåndsvisning er ikke tilgængelig for denne filtype.',
                        'download-file' => 'Download Fil',
                        'download-zip'  => 'Download som ZIP',
                        'close'         => 'Luk forhåndsvisning',
                        'mime'          => 'MIME',
                        'card'          => [
                            'click-for-details' => 'Klik for fulde detaljer',
                            'preview'           => 'Forhåndsvisning',
                            'edit-image'        => 'Rediger billede',
                        ],
                        'image-viewer' => [
                            'rotate-left'   => 'Roter til venstre (L)',
                            'rotate-right'  => 'Roter til højre (R)',
                            'zoom-out'      => 'Zoom ud (-)',
                            'zoom-in'       => 'Zoom ind (+)',
                            'fit-to-screen' => 'Tilpas til skærm',
                            'actual-size'   => 'Faktisk størrelse',
                            'reset-all'     => 'Nulstil alt (0)',
                        ],
                        'video-player' => [
                            'back-10s'           => '10 s tilbage',
                            'forward-10s'        => '10 s frem',
                            '10s'                => '10 s',
                            'speed'              => 'Hastighed',
                            'loop'               => 'Gentag (L)',
                            'mute'               => 'Lydløs (M)',
                            'picture-in-picture' => 'Billede i billede',
                            'fullscreen'         => 'Fuld skærm (F)',
                            'more-actions'       => 'Flere handlinger',
                            'copy-link'          => 'Kopiér link',
                            'link-copied'        => 'Kopieret!',
                            'open-in-new-tab'    => 'Åbn i ny fane',
                        ],
                    ],
                    'custom-download' => [
                        'title'              => 'Brugerdefineret download',
                        'format'             => 'Format',
                        'width'              => 'Bredde (px)',
                        'width-placeholder'  => '200',
                        'height'             => 'Højde (px)',
                        'height-placeholder' => '200',
                        'download-btn'       => 'Download',
                        'extension-types'    => [
                            'jpg'      => 'JPG',
                            'png'      => 'PNG',
                            'jpeg'     => 'JPEG',
                            'webp'     => 'WEBP',
                            'original' => 'Original',
                        ],
                    ],
                    'tag-already-exists'        => 'Tagget findes allerede',
                    'image-source-not-readable' => 'Billedkilden kan ikke læses',
                    'failed-to-read'            => 'Kunne ikke læse billedets metadata :exception',
                    'file-re-upload-success'    => 'Filer er blevet uploadet igen.',
                    'image-processing-failed'   => 'Billedbehandling mislykkedes: :message',
                    'image-editor'              => [
                        'tools'                    => 'Værktøjer',
                        'close'                    => 'Luk redaktør',
                        'crop'                     => 'Beskær og tilpas størrelse',
                        'crop-sub'                 => 'Trim eller skalér billede',
                        'adjust'                   => 'Lysstyrke og kontrast',
                        'adjust-sub'               => 'Juster lys og tone',
                        'rotate'                   => 'Roter og spejlvend',
                        'rotate-sub'               => 'Transformér orientering',
                        'selection'                => 'Valg',
                        'drag-handles'             => 'trækgreb',
                        'scale-after-crop'         => 'Skalér efter beskæring',
                        'optional'                 => 'valgfri',
                        'width-px'                 => 'Bredde (px)',
                        'height-px'                => 'Højde (px)',
                        'blank-keep-dims'          => 'Lad feltet stå tomt for at bevare beskårne dimensioner.',
                        'auto'                     => 'auto',
                        'adjustments'              => 'Justeringer',
                        'brightness'               => 'Lysstyrke',
                        'contrast'                 => 'Kontrast',
                        'rotation'                 => 'Rotation',
                        'flip'                     => 'Spejlvend',
                        'horizontal'               => 'Vandret',
                        'vertical'                 => 'Lodret',
                        'platform'                 => 'Platform',
                        'platform-loading'         => 'Indlæser…',
                        'no-platforms'             => 'Ingen platforme',
                        'color-none'               => 'Ingen',
                        'color-white'              => 'Hvid',
                        'color-silver'             => 'Sølv',
                        'color-light-gray'         => 'Lysegrå',
                        'color-gray'               => 'Grå',
                        'color-slate'              => 'Skiffergrå',
                        'color-dark-gray'          => 'Mørkegrå',
                        'color-charcoal'           => 'Kul',
                        'color-black'              => 'Sort',
                        'color-light-red'          => 'Lysrød',
                        'color-red'                => 'Rød',
                        'color-light-yellow'       => 'Lysgul',
                        'color-yellow'             => 'Gul',
                        'color-light-green'        => 'Lysgrøn',
                        'color-green'              => 'Grøn',
                        'color-light-cyan'         => 'Lyscyan',
                        'color-cyan'               => 'Cyan',
                        'color-light-blue'         => 'Lysblå',
                        'color-blue'               => 'Blå',
                        'color-light-purple'       => 'Lyslilla',
                        'color-purple'             => 'Lilla',
                        'color-light-pink'         => 'Lyserød',
                        'color-pink'               => 'Lyserød',
                        'color-light-rose'         => 'Lys rose',
                        'color-rose'               => 'Rosé',
                        'model'                    => 'Model',
                        'no-models'                => 'Ingen modeller',
                        'prompt'                   => 'Prompt',
                        'apply'                    => 'Anvend',
                        'applying'                 => 'Anvender…',
                        'error-platforms'          => 'Kunne ikke indlæse AI-platforme.',
                        'error-select-color'       => 'Vælg en baggrundsfarve.',
                        'error-operation'          => 'Handlingen mislykkedes. Prøv igen.',
                        'success-updated'          => 'Billede opdateret.',
                        'success-adjusted'         => 'Billede justeret.',
                        'success-transformed'      => 'Billede transformeret.',
                        'error-no-ai-image'        => 'AI returnerede intet billede.',
                        'error-provide-dims'       => 'Angiv mindst en beskæringsregion eller outputdimensioner.',
                        'edit-bg'                  => 'Rediger baggrund',
                        'edit-bg-sub'              => 'Farve, upload eller prompt',
                        'bg-tab-color'             => 'Farve',
                        'bg-tab-upload'            => 'Upload',
                        'bg-tab-ai'                => 'Prompt',
                        'bg-color-label'           => 'Baggrundsfarve',
                        'bg-color-mode-normal'     => 'Normal',
                        'bg-color-mode-ai'         => 'AI',
                        'bg-custom-color'          => 'Tilpasset',
                        'bg-upload-hint'           => 'Klik for at uploade et baggrundsbillede',
                        'bg-upload-change'         => 'Klik for at ændre',
                        'bg-ai-prompt-placeholder' => 'Beskriv den baggrund du ønsker…',
                        'sharpen'                  => 'Skarphed',
                        'blur'                     => 'Sløring',
                        'filters'                  => 'Filtre',
                        'filters-sub'              => 'Gråtoner og invertering',
                        'greyscale'                => 'Gråtoner',
                        'invert'                   => 'Inverter',
                        'error-no-filter'          => 'Vælg mindst ét filter at anvende.',
                        'error-provider-no-images' => 'Denne AI-udbyder understøtter ikke billedgenerering.',
                        'error-no-image-platform'  => 'Ingen AI-platforme med understøttelse af billedgenerering fundet. Konfigurer en OpenAI-, Gemini- eller xAI-platform.',
                    ],
                ],
                'linked-resources' => [
                    'index' => [
                        'datagrid' => [
                            'product'       => 'Produkt',
                            'category'      => 'Kategori',
                            'product-sku'   => 'Produkt-Sku: ',
                            'category code' => 'Kategorikode: ',
                            'resource-type' => 'Ressourcetype',
                            'resource'      => 'Ressource',
                            'resource-view' => 'Ressourcevisning',
                        ],
                    ],
                    'found-success' => 'Ressource fundet',
                    'not-found'     => 'Ressource ikke fundet',
                ],
                'tags' => [
                    'create' => [
                        'create-success' => 'Tags er blevet tilføjet',
                    ],
                    'found-success'  => 'Tag fundet',
                    'not-found'      => 'Tags ikke fundet',
                    'delete-success' => 'Aktivtags fjernet',
                ],
                'delete-success'                          => 'Aktiv slettet',
                'delete-failed-due-to-attached-resources' => 'Aktivet er i brug. Fjern tilknytningen før sletning',
                'datagrid'                                => [
                    'mass-delete-success'                 => 'Masseslettet.',
                    'mass-update-success'                 => 'Masseopdatering fuldført.',
                    'files-upload-success'                => 'Filer uploadet.',
                    'file-upload-success'                 => 'Fil uploadet.',
                    'not-found'                           => 'Fil ikke fundet',
                    'not-found-to-show'                   => 'Filen blev ikke fundet til visning',
                    'not-found-to-download'               => 'Filen blev ikke fundet til download',
                    'edit-success'                        => 'Fil uploadet',
                    'no-file-name'                        => 'intet filnavn',
                    'show-success'                        => 'Fil fundet',
                    'update-success'                      => 'Fil opdateret',
                    'not-found-to-update'                 => 'Filen findes ikke',
                    'not-found-to-destroy'                => 'Filen findes ikke',
                    'files-upload-failed'                 => 'Filer kunne ikke uploades.',
                    'file-upload-failed'                  => 'Filen kunne ikke uploades',
                    'invalid-file'                        => 'Ugyldig fil angivet',
                    'invalid-file-format-or-not-provided' => 'Ingen filer angivet eller ugyldigt format.',
                    'file-process-failed'                 => 'Nogle filer kunne ikke behandles',
                    'file-forbidden-type'                 => 'Filen har en forbudt type eller filendelse.',
                    'file-too-large'                      => 'Filen er for stor. Den maksimalt tilladte størrelse er :size.',
                    'storage-upload-failed'               => 'Upload af ":name" til lageret mislykkedes. Kontrollér diskoplysninger og tilladelser.',
                    'not-found-or-no-file'                => 'Aktiv ikke fundet eller fil eksisterer ikke.',
                    'download-link-ready'                 => 'Aktiv fundet. Du kan downloade filen fra det angivne link.',
                ],
            ],
            'share' => [
                'target-not-found' => 'Det delte aktiv eller den delte mappe kunne ikke findes.',
                'not-found'        => 'Delingslink ikke fundet.',
                'revoked'          => 'Delingslink tilbagekaldt.',
                'deleted'          => 'Delt link slettet.',
                'already-revoked'  => 'Dette delingslink er allerede tilbagekaldt.',
                'reauthorized'     => 'Delingslink genauthoriseret — den originale URL er aktiv igen.',
                'not-revoked'      => 'Dette delingslink er ikke tilbagekaldt.',
                'updated'          => 'Delingslink opdateret.',
                'update-failed'    => 'Kunne ikke opdatere delingslinket.',
                'delete-failed'    => 'Sletning af delt link mislykkedes.',
                'modal'            => [
                    'title-asset'        => 'Del aktiv',
                    'title-directory'    => 'Del mappe',
                    'edit-title'         => 'Rediger delingslink',
                    'edit-name'          => 'Omdøb deling',
                    'unnamed-share'      => '(intet brugerdefineret navn)',
                    'name-label'         => 'Brugerdefineret navn',
                    'name-hint'          => 'Lad være tom for at beholde det oprindelige fil-/mappenavn som etiket.',
                    'save'               => 'Gem ændringer',
                    'saving'             => 'Gemmer…',
                    'cancel'             => 'Annuller',
                    'create-new'         => 'Opret et nyt delingslink',
                    'expiry'             => 'Link udløber efter',
                    'expiry-1d'          => '1 dag',
                    'expiry-7d'          => '7 dage',
                    'expiry-30d'         => '30 dage',
                    'expiry-365d'        => '1 år',
                    'no-expiry'          => 'Ingen udløbsdato',
                    'create'             => 'Generér link',
                    'creating'           => 'Genererer…',
                    'active-links'       => 'Aktive links',
                    'loading'            => 'Indlæser…',
                    'no-active-links'    => 'Endnu ingen aktive delingslinks for dette element.',
                    'copy'               => 'Kopiér',
                    'copied'             => 'Link kopieret til udklipsholder',
                    'revoke'             => 'Tilbagekald',
                    'delete'             => 'Delete',
                    'revoked'            => 'Delingslink tilbagekaldt',
                    'revoke-failed'      => 'Kunne ikke tilbagekalde delingslink.',
                    'expires-on'         => 'Udløber den',
                    'never-expires'      => 'Udløber aldrig',
                    'views'              => 'Visninger',
                    'downloads'          => 'Downloads',
                    'created'            => 'Delingslink oprettet.',
                    'create-failed'      => 'Kunne ikke oprette delingslink.',
                    'load-failed'        => 'Kunne ikke indlæse eksisterende delingslinks.',
                    'advanced'           => 'Avanceret',
                    'revoked-status'     => 'Link tilbagekaldt',
                    'reauthorize'        => 'Genauthoriser',
                    'reauthorizing'      => 'Genauthoriserer…',
                    'reauthorize-failed' => 'Kunne ikke genauthorisere delingslinket.',
                    'revoked-notice'     => 'Dette link er tilbagekaldt. Genauthoriser for at gøre den originale URL aktiv igen.',
                ],
                'index' => [
                    'title'       => 'Delte links',
                    'description' => 'Alle aktive og historiske delingslinks for aktiver og mapper i DAM.',
                ],
                'datagrid' => [
                    'type'           => 'Type',
                    'asset'          => 'Aktiv',
                    'directory'      => 'Mappe',
                    'target'         => 'Mål',
                    'custom-name'    => 'Brugerdefineret navn',
                    'created-by'     => 'Oprettet af',
                    'expires-at'     => 'Udløber den',
                    'never'          => 'Aldrig',
                    'status'         => 'Status',
                    'status-active'  => 'Aktiv',
                    'status-expired' => 'Udløbet',
                    'status-revoked' => 'Tilbagekaldt',
                    'views'          => 'Visninger',
                    'downloads'      => 'Downloads',
                    'created-at'     => 'Oprettet den',
                    'edit'           => 'Rediger',
                    'copy-link'      => 'Kopier link',
                    'revoke'         => 'Tilbagekald',
                    'delete'         => 'Slet',
                ],
            ],
            'upload' => [
                'pause'     => 'Pause',
                'resume'    => 'Genoptag',
                'retry'     => 'Prøv igen',
                'cancel'    => 'Annuller',
                'paused'    => 'Upload sat på pause. Eksisterende aktiver er ikke berørt — genoptag når som helst.',
                'resumed'   => 'Upload genoptaget.',
                'cancelled' => 'Upload annulleret. Allerede uploadede aktiver blev bevaret.',
                'retried'   => 'Prøver mislykkede uploads igen i baggrunden.',
            ],
            'tag' => [
                'not-found'           => 'Tag ikke fundet.',
                'create-success'      => 'Tag oprettet.',
                'update-success'      => 'Tag opdateret.',
                'delete-success'      => 'Tag slettet.',
                'mass-delete-success' => 'Valgte tags slettet.',
                'index'               => [
                    'title'       => 'Tags',
                    'description' => 'Administrer de tags, der bruges til at mærke og organisere aktiver på tværs af DAM.',
                    'create'      => 'Opret tag',
                ],
                'datagrid' => [
                    'name'         => 'Navn',
                    'assets-count' => 'Aktiver',
                    'created-at'   => 'Oprettet',
                    'edit'         => 'Rediger',
                    'delete'       => 'Slet',
                ],
                'modal' => [
                    'create-title'     => 'Opret tag',
                    'edit-title'       => 'Rediger tag',
                    'name-label'       => 'Tagnavn',
                    'name-placeholder' => 'fx sommerkampagne',
                    'save'             => 'Gem',
                    'saving'           => 'Gemmer…',
                    'cancel'           => 'Annuller',
                ],
                'mass-action' => [
                    'assign-tags'              => 'Tildel tags',
                    'modal-title'              => 'Tildel tags',
                    'modal-subtitle'           => 'Tilføj tags til :count valgte aktiv(er). Eksisterende tags bevares.',
                    'modal-subtitle-recursive' => 'Tilføj tags til de valgte aktiver og alle aktiver inde i de valgte mapper, rekursivt. Eksisterende tags bevares.',
                    'tags-label'               => 'Tags',
                    'tags-placeholder'         => 'Søg efter tags, eller skriv for at oprette…',
                    'add-tag'                  => 'Tryk Enter for at tilføje',
                    'assign'                   => 'Tildel',
                    'assigning'                => 'Tildeler…',
                    'cancel'                   => 'Annuller',
                    'assign-success'           => 'Tags tildelt :count aktiv(er).',
                    'assign-failed'            => 'Det lykkedes ikke at tildele tags.',
                    'no-assets'                => 'Ingen aktiver valgt.',
                    'no-items'                 => 'Vælg mindst ét aktiv eller én mappe.',
                    'no-tags'                  => 'Vælg eller tilføj mindst ét tag.',
                ],
            ],
        ],
        'catalog' => [
            'attributes' => [
                'type' => [
                    'asset' => 'Aktiv',
                ],
            ],
            'category-fields' => [
                'type' => [
                    'asset' => 'Aktiv',
                ],
            ],
        ],
        'acl' => [
            'menu'             => 'Aktivstyring',
            'asset'            => 'Aktiv',
            'property'         => 'Egenskab',
            'comment'          => 'Kommentar',
            'linked_resources' => 'Tilknyttede ressourcer',
            'meta_data'        => 'Indlejret metainformation',
            'directory'        => 'Mappe',
            'tag'              => 'Tag',
            'create'           => 'Opret',
            'edit'             => 'Rediger',
            'update'           => 'Opdater',
            'delete'           => 'Delete',
            'list'             => 'Liste',
            'view'             => 'Vis',
            'upload'           => 'Upload',
            're_upload'        => 'Upload igen',
            'mass_update'      => 'Masseopdatering',
            'mass_delete'      => 'Masseslet',
            'download'         => 'Download',
            'custom_download'  => 'Brugerdefineret download',
            'rename'           => 'Omdøb',
            'move'             => 'Flyt',
            'copy'             => 'Kopiér',
            'copy-structure'   => 'Kopiér mappestruktur',
            'download-zip'     => 'Download zip',
            'asset-assign'     => 'Tildel aktiv',
            'share'            => 'Del',
            'assets'           => 'Aktiver',
            'shares'           => 'Delte links',
            'revoke'           => 'Tilbagekald',
            'tags'             => 'Tags',
            'configuration'    => 'Konfiguration',
        ],
        'permissions' => [
            'title'                 => 'DAM-mapperettigheder',
            'tab-subtitle'          => 'Mapper denne rolle har adgang til.',
            'no-actions'            => 'Kun visning. Åbn en undermappe, du har adgang til.',
            'unauthorized'          => 'Du har ikke tilladelse til at administrere DAM-mapperettigheder.',
            'all-directories'       => 'Alle mapper — giv adgang til alle mapper nu og i fremtiden',
            'inherit-children'      => 'Nedarv undermapper — få automatisk adgang til alle undermapper i de tildelte mapper',
            'loading'               => 'Indlæser mapper…',
            'empty'                 => 'Ingen mapper fundet.',
            'error'                 => 'Kunne ikke indlæse mapper. Prøv igen.',
            'retry'                 => 'Prøv igen',
            'grants-load-warning'   => 'Nogle mapperettigheder kunne ikke indlæses. Kontrollér dine valg, før du gemmer.',
            'inherit-children-note' => 'Undermapper inkluderes automatisk ved lagring.',
        ],
        'validation' => [
            'asset' => [
                'required' => 'Feltet :attribute er påkrævet.',
            ],
            'comment' => [
                'required' => 'Kommentarteksten er påkrævet.',
            ],
            'property' => [
                'name' => [
                    'required' => 'Navnefeltet er påkrævet.',
                    'unique'   => 'Navnet er allerede taget.',
                ],
                'language' => [
                    'not-found' => 'Det valgte sprog kunne ikke findes eller er deaktiveret.',
                ],
                'sort-order' => [
                    'required' => 'Feltet Sorteringsrækkefølge er påkrævet, når Tilføj til filter er aktiveret.',
                ],
            ],
        ],
        'errors' => [
            '401' => 'Denne handling er ikke autoriseret.',
        ],
        'configuration' => [
            'save-btn'    => 'Gem',
            'title'       => 'Konfiguration',
            'saved'       => 'Konfiguration gemt.',
            'description' => 'Konfigurer indstillingerne for digital aktivstyring (DAM) i din applikation.',
            'general'     => [
                'title'            => 'Generelle indstillinger',
                'description'      => 'Disse indstillinger styrer den overordnede DAM-browsingoplevelse.',
                'explorer-enabled' => [
                    'label' => 'Aktivér udforskningsvisning',
                    'hint'  => 'Brug den Explorer-lignende grænseflade til at gennemse og administrere aktiver.',
                ],
                'bookmarks-enabled' => [
                    'label' => 'Vis bogmærkepanel',
                    'hint'  => 'Vis en dedikeret sidebjælke med dine bogmærkede mapper og aktiver.',
                ],
            ],
            'directory' => [
                'title'       => 'Mappeindstillinger',
                'description' => 'Disse indstillinger styrer, hvordan mapper og aktiver vises i mappepanelet.',
                'show-tree'   => [
                    'label'       => 'Vis mappetræ',
                    'hint'        => 'Vis mappehierarkiet i venstre sidebjælke for lettere navigation.',
                    'locked-hint' => 'Altid slået til, mens udforskningsvisning er deaktiveret.',
                ],
                'tree-show-assets' => [
                    'label' => 'Vis aktiver i mappetræet',
                    'hint'  => 'Vis aktiver sammen med mapper i mappetræet i stedet for kun at vise mapper.',
                ],
            ],
        ],
        'explorer' => [
            'not-found'         => 'Mappen blev ikke fundet.',
            'access-denied'     => 'Du har ikke adgang til denne mappe.',
            'empty'             => 'Ingen mapper fundet.',
            'action-completed'  => 'Handlingen blev fuldført.',
            'foreign-selection' => [
                'notice' => ':count element(er) valgt i en anden fane.',
                'action' => 'Vælg dem her også',
            ],
            'bookmarks' => [
                'title'     => 'Bogmærker',
                'drag-hint' => 'Træk en mappe hertil for at bogmærke den',
                'max'       => 'Maksimalt 20 bogmærker nået.',
                'remove'    => 'Fjern bogmærke',
                'empty'     => 'Ingen bogmærker endnu.',
            ],
            'tab' => [
                'new'   => 'Ny fane',
                'close' => 'Luk fane',
            ],
            'sections' => [
                'folders' => 'Mapper',
                'files'   => 'Filer',
                'folder'  => 'Mappe',
            ],
            'clipboard' => [
                'ready'   => 'Klar til at indsætte',
                'dismiss' => 'Ryd',
            ],
            'context' => [
                'open'                 => 'Åbn',
                'open-new-tab'         => 'Åbn i ny fane',
                'bookmark'             => 'Sæt mappe som bogmærke',
                'folder-upload'        => 'Upload mappe',
                'copy'                 => 'Kopiér',
                'paste'                => 'Indsæt',
                'copy-progress'        => 'Kopiering sat i kø.',
                'copy-done'            => 'Kopieret.',
                'paste-file-started'   => 'Indsætter fil...',
                'paste-folder-started' => 'Indsætter mappe...',
                'paste-done'           => 'Indsat korrekt.',
                'move-asset-started'   => 'Flytter fil...',
                'move-folder-started'  => 'Flytter mappe...',
                'move-done'            => 'Mappen er flyttet.',
                'move-asset-done'      => 'Filen er flyttet.',
                'delete-done'          => 'Mappe slettet.',
            ],
            'folder' => [
                'deleted' => 'Denne mappe blev slettet — returneret til roden.',
            ],
            'search' => [
                'placeholder' => 'Søg efter filer og mapper…',
            ],
            'list' => [
                'header' => [
                    'name'     => 'Navn',
                    'type'     => 'Type',
                    'size'     => 'Størrelse',
                    'modified' => 'Ændret',
                    'actions'  => 'Handlinger',
                ],
                'items-count' => ':count elementer',
            ],
            'dialog' => [
                'rename-asset' => [
                    'title'       => 'Omdøb fil',
                    'placeholder' => 'Nyt navn',
                ],
                'save'   => 'Gem',
                'cancel' => 'Annuller',
            ],
            'view' => [
                'grid' => 'Gittervisning',
                'list' => 'Listevisning',
            ],
            'mass-actions' => [
                'select-count'   => ':count valgt',
                'select-action'  => 'Vælg handling',
                'delete'         => 'Slet',
                'confirm'        => 'Slet :count element(er)? Mapper slettes rekursivt og kan ikke fortrydes.',
                'deleting'       => 'Sletter :count element(er)…',
                'deleted-assets' => ':count aktiv(er) slettet.',
                'deleted-dirs'   => ':count mappe(r) sat i kø til sletning.',
                'move-to'        => 'Flyt til…',
                'copy-to'        => 'Kopiér til…',
                'moving'         => 'Flytter…',
                'copying'        => 'Kopierer…',
                'moved-assets'   => ':count fil(er) flyttet.',
                'copied-assets'  => ':count fil(er) kopieret.',
                'moved-dirs'     => ':count mappe(r) flyttet.',
                'copied-dirs'    => ':count mappe(r) kopieret.',
                'pick-dest'      => 'Vælg destination',
                'select-here'    => 'Vælg her',
                'new-folder'     => 'Ny mappe',
                'folder-name'    => 'Mappenavn',
                'create-error'   => 'Mappen kunne ikke oprettes.',
                'move-done'      => ':count Filer flyttet.',
                'copy-done'      => ':count Filer kopieret.',
                'still-running'  => 'Handlingen kører stadig i baggrunden',
                'progress'       => 'Fremskridt',
                'total-files'    => ':count fil(er)',
            ],
            'toolbar' => [
                'toggle-tree'    => 'Skift mappetræ',
                'toggle-sidebar' => 'Skift sidebjælke',
            ],
            'upload' => [
                'queued'              => 'I kø',
                'uploading'           => 'Uploader',
                'creating'            => 'Opretter mappe',
                'done'                => 'Færdig',
                'failed'              => 'Mislykkedes',
                'interrupted'         => 'Afbrudt — tilføj igen for at prøve igen',
                'resuming'            => 'Genoptager…',
                'overall'             => ':done af :total uploadet',
                'too-large-to-resume' => 'Batchet er for stort til automatisk genoptagelse efter genindlæsning',
            ],
        ],
    ],
    'share' => [
        'public' => [
            'app-name'              => 'DAM',
            'download'              => 'Download',
            'back-to-gallery'       => 'Tilbage til galleri',
            'back'                  => 'Tilbage',
            'file-name'             => 'Filnavn',
            'file-type'             => 'Filtype',
            'file-size'             => 'Filstørrelse',
            'video-not-supported'   => 'Din browser understøtter ikke video-elementet.',
            'audio-not-supported'   => 'Din browser understøtter ikke audio-elementet.',
            'preview-not-available' => 'Forhåndsvisning er ikke tilgængelig for denne fil. Brug knappen Download ovenfor.',
            'files-count'           => ':count fil(er)',
            'empty-directory'       => 'Denne delte mappe indeholder endnu ingen filer.',
            'expires-on'            => 'Dette link udløber den',
            'powered-by-dam'        => 'Drevet af Unopim DAM',
            'expired-title'         => 'Link udløbet',
            'expired-message'       => 'Dette delingslink er udløbet og er ikke længere tilgængeligt. Anmod om et nyt link fra ejeren.',
            'revoked-title'         => 'Link tilbagekaldt',
            'revoked-message'       => 'Dette delingslink er tilbagekaldt af ejeren og er ikke længere tilgængeligt.',
            'not-found-title'       => 'Link ikke fundet',
            'not-found-message'     => 'Det delingslink, du åbnede, eksisterer ikke. Det kan være tastet forkert eller fjernet.',
            'download-zip'          => 'Download alle som ZIP',
            'per-page'              => 'Per side',
        ],
    ],
];
