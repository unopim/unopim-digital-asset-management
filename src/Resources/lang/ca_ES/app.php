<?php

return [
    'admin' => [
        'components' => [
            'layouts' => [
                'sidebar' => [
                    'dam'           => 'Gestió d\'actius',
                    'assets'        => 'Recursos',
                    'shares'        => 'Enllaços compartits',
                    'tags'          => 'Etiquetes',
                    'configuration' => 'Configuració',
                ],
            ],
            'modal' => [
                'confirm' => [
                    'message' => 'Eliminar aquest directori també eliminarà tots els subdirectoris que conté. Aquesta acció és permanent i no es pot desfer.',
                ],
            ],
            'asset' => [
                'field' => [
                    'add-asset'     => 'Afegir Actiu',
                    'assign-assets' => 'Assignar Actius',
                    'preview'       => 'Previsualitzar',
                    'remove'        => 'Eliminar',
                    'download'      => 'Descarregar',
                ],
            ],
        ],
        'dam' => [
            'file' => [
                'deleted'   => 'Fitxer eliminat',
                'not-found' => 'Fitxer no trobat',
            ],
            'image-edit' => [
                'cannot-decode'  => 'No es pot descodificar la imatge',
                'preview-failed' => 'La previsualització ha fallat',
            ],
            'index' => [
                'title'    => 'Gestió d\'actius digitals',
                'datagrid' => [
                    'file-name'      => 'Nom del Fitxer',
                    'tags'           => 'Etiquetes',
                    'property-name'  => 'Nom de la Propietat',
                    'property-value' => 'Valor de la Propietat',
                    'created-at'     => 'Creat el',
                    'updated-at'     => 'Actualitzat el',
                    'extension'      => 'Extensió',
                    'path'           => 'Ruta',
                ],
                'mass-action' => [
                    'deleting'   => 'S\'estan eliminant :count actius…',
                    'processing' => 'S\'estan processant :count actius…',
                ],
                'move' => [
                    'directory' => 'Movent el directori ":name"…',
                    'asset'     => 'Movent l\'actiu ":name"…',
                ],
                'directory' => [
                    'title'  => 'Directori',
                    'create' => [
                        'title'     => 'Crear Directori',
                        'name'      => 'Nom',
                        'file_name' => 'Nom del fitxer',
                        'save-btn'  => 'Desar Directori',
                    ],
                    'rename' => [
                        'title' => 'Canviar el nom del Directori',
                    ],
                    'search' => [
                        'placeholder'     => 'Cerca directoris...',
                        'no-matches'      => 'Sense coincidències',
                        'error'           => 'Cerca fallida — torna-ho a provar',
                        'not-found-flash' => 'El directori ja no és accessible',
                        'count'           => ':shown de :total coincidències',
                        'count-total'     => ':total coincidències',
                    ],
                    'asset' => [
                        'rename' => [
                            'title'    => 'Canviar el nom de l\'Actiu',
                            'save-btn' => 'Desar Actiu',
                        ],
                    ],
                    'actions' => [
                        'delete'                    => 'Eliminar',
                        'edit'                      => 'Edita',
                        'rename'                    => 'Canviar el nom',
                        'copy'                      => 'Copiar',
                        'cut'                       => 'Tallar',
                        'download'                  => 'Descarregar',
                        'download-zip'              => 'Descarregar Zip',
                        'paste'                     => 'Enganxar',
                        'add-directory'             => 'Afegir Directori',
                        'upload-files'              => 'Pujar Fitxers',
                        'upload-files-and-folders'  => 'Pujar fitxers i carpetes',
                        'upload-folder'             => 'Pujar carpeta',
                        'upload-drop-hint'          => 'Arrossegueu fitxers o carpetes aquí',
                        'upload-or'                 => 'o navegueu:',
                        'copy-directory-structured' => 'Copiar Estructura del Directori',
                        'get'                       => 'Get',
                        'get-by-id'                 => 'Obtenir per Id',
                        'comment'                   => 'Comentari',
                        'share'                     => 'Compartir directori',
                    ],
                    'linked-resources'                          => 'Recursos Vinculats',
                    'not-found'                                 => 'No s\'ha trobat cap directori',
                    'created-success'                           => 'Directori creat correctament',
                    'creation-failed'                           => 'Error en crear el directori',
                    'updated-success'                           => 'Directori actualitzat correctament',
                    'update-failed'                             => 'Error en actualitzar el directori',
                    'moved-success'                             => 'Directori mogut correctament',
                    'cannot-move'                               => 'No es pot moure el directori',
                    'cannot-copy'                               => 'No es pot copiar una carpeta dins d’ella mateixa ni dins d’una subcarpeta seva.',
                    'fetch-all-success'                         => 'Directoris obtinguts correctament',
                    'can-not-deleted'                           => 'No es pot eliminar el directori perquè és el Directori Arrel.',
                    'deleting-in-progress'                      => 'Eliminació del directori en curs',
                    'can-not-copy'                              => 'No es pot copiar el directori perquè és el Directori Arrel.',
                    'coping-in-progress'                        => 'Còpia de l\'estructura del directori en curs.',
                    'asset-not-found'                           => 'No s\'ha trobat cap actiu',
                    'asset-renamed-success'                     => 'Actiu reanomenat correctament',
                    'asset-moved-success'                       => 'Actiu mogut correctament',
                    'asset-name-already-exist'                  => 'El nou nom ja existeix amb un altre actiu anomenat :asset_name',
                    'asset-name-conflict-in-the-same-directory' => 'El nom de l\'actiu entra en conflicte amb un fitxer existent al mateix directori.',
                    'old-file-not-found'                        => 'El fitxer sol·licitat a la ruta :old_path no s\'ha trobat.',
                    'image-name-is-the-same'                    => 'Aquest nom ja existeix. Si us plau, introduïu-ne un altre.',
                    'not-writable'                              => 'No teniu permís per :actionType un :type en aquesta ubicació ":path".',
                    'empty-directory'                           => 'Aquest directori és buit.',
                    'failed-download-directory'                 => 'No s\'ha pogut crear el fitxer zip.',
                    'not-allowed'                               => 'No es permet la pujada de fitxers d\'script.',
                    'copy-success'                              => 'Carpeta copiada correctament.',
                    'name-unique'                               => 'El nom del directori ha de ser únic dins del mateix directori pare.',
                    'folder-upload-success'                     => 'Carpeta pujada correctament.',
                    'folder-upload-partial'                     => 'Carpeta pujada amb :count fitxer(s) omès(os) a causa d\'errors.',
                    'load-more'                                 => 'Carrega\'n més',
                ],
                'description'                    => 'L\'eina us pot ajudar a organitzar, emmagatzemar i gestionar tots els vostres actius multimèdia en un sol lloc',
                'root'                           => 'Arrel',
                'upload'                         => 'Pujar',
                'new'                            => 'Nou',
                'uploading'                      => 'Pujant...',
                'cancel'                         => 'Cancel·lar',
                'upload-cancelled'               => 'Càrrega cancel·lada.',
                'folder-upload-https-required'   => 'La càrrega de carpetes requereix una connexió segura (HTTPS).',
                'folder-upload-no-https-warning' => 'Les carpetes buides no es crearan sense una connexió segura (HTTPS). Els fitxers es continuaran pujant.',
                'folder-upload-no-files'         => 'La(les) carpeta(es) seleccionada(es) no conté(nen) fitxers. Les subcarpetes buides no es poden detectar sense una connexió segura (HTTPS).',
                'upload-in-progress-title'       => 'Càrrega en curs',
                'upload-in-progress-message'     => 'Sortir d\'aquesta pàgina cancel·larà la càrrega activa. Esteu segur que voleu sortir?',
                'upload-leave-page'              => 'Sortir',
                'upload-stay-page'               => 'Quedar-se',
                'drop-zone-hint'                 => 'Arrossegueu fitxers o carpetes per pujar',
                'select-all'                     => 'Selecciona-ho tot',
                'select-directory-to-upload'     => 'Selecciona un directori on pujar.',
                'upload-session'                 => [
                    'uploading'            => 'Pujant :total fitxer(s)…',
                    'paused'               => 'En pausa — :done de :total pujats',
                    'interrupted'          => ':done de :total pujats · :interrupted interromputs',
                    'failed'               => ':done pujats, :failed fallits',
                    'completed'            => ':done de :total pujats',
                    'create-folder-failed' => 'No s\'ha pogut crear la carpeta',
                ],
                'drop-zone-no-permission' => 'No teniu permís per pujar aquí.',
                'upload-complete'         => 'Càrrega completa',
                'item-upload-complete'    => 'Càrrega completa',
                'copy'                    => [
                    'directory' => 'Copiant el directori ":name"…',
                ],
                'delete' => [
                    'directory' => 'Eliminant el directori ":name"…',
                ],
            ],
            'asset' => [
                'properties' => [
                    'index' => [
                        'title'      => 'Propietats de l\'Actiu',
                        'create-btn' => 'Crear Propietat',
                        'datagrid'   => [
                            'name'     => 'Nom',
                            'type'     => 'Tipus',
                            'language' => 'Idioma',
                            'value'    => 'Valor',
                            'edit'     => 'Editar',
                            'delete'   => 'Eliminar',
                        ],
                        'create' => [
                            'title'                  => 'Crear Propietat',
                            'name'                   => 'Nom',
                            'type'                   => 'Tipus',
                            'language'               => 'Idioma',
                            'value'                  => 'Valor',
                            'add-to-filter'          => 'Afegir al filtre',
                            'add-to-filter-help'     => 'Quan està activat, aquesta propietat apareix com a filtre a la barra lateral del llistat d\'actius.',
                            'sort-order'             => 'Ordre de classificació',
                            'sort-order-help'        => 'Els números més baixos apareixen primer a la barra lateral del filtre. Les propietats amb el mateix número s\'ordenen alfabèticament.',
                            'sort-order-placeholder' => 'p. ex. 10',
                            'save-btn'               => 'Desar',
                        ],
                        'edit' => [
                            'title' => 'Editar Propietat',
                        ],
                        'delete-success' => 'Propietat de l\'Actiu eliminada correctament',
                        'delete-failed'  => 'Error en eliminar la propietat de l\'actiu',
                        'create-success' => 'Propietat de l\'Actiu creada correctament',
                        'update-success' => 'Propietat de l\'Actiu actualitzada correctament',
                        'update-failure' => 'Error en actualitzar la propietat de l\'actiu',
                        'not-found'      => 'Propietat no trobada',
                        'found-success'  => 'Propietat trobada correctament',
                    ],
                ],
                'comments' => [
                    'create' => [
                        'create-success' => 'El comentari s\'ha afegit correctament',
                        'create-failure' => 'No s\'ha pogut crear el comentari',
                    ],
                    'post-comment'    => 'Publicar Comentari',
                    'post-reply'      => 'Publicar Resposta',
                    'reply'           => 'Respondre',
                    'edit-btn'        => 'Editar',
                    'delete-btn'      => 'Eliminar',
                    'save-btn'        => 'Desar',
                    'cancel-btn'      => 'Cancel·lar',
                    'add-reply'       => 'Afegir Resposta',
                    'add-comment'     => 'Afegir Comentari',
                    'no-comments'     => 'Encara no hi ha comentaris',
                    'not-found'       => 'Comentaris no trobats',
                    'updated-success' => 'Comentari actualitzat correctament',
                    'update-failed'   => 'No s\'ha pogut actualitzar el comentari',
                    'delete-success'  => 'Comentari de l\'Actiu eliminat correctament',
                    'delete-failed'   => 'No s\'ha pogut eliminar el comentari de l\'Actiu',
                    'deleted-user'    => 'Usuari eliminat',
                ],
                'edit' => [
                    'title'                 => 'Editar Actiu',
                    'back'                  => 'Enrere',
                    'previous'              => 'Anterior',
                    'next'                  => 'Següent',
                    'name'                  => 'Nom',
                    'value'                 => 'Valor',
                    'save-btn'              => 'Desar',
                    'file-name'             => 'Nom del fitxer',
                    'file-info'             => 'Informació del fitxer',
                    'type'                  => 'Tipus',
                    'size'                  => 'Mida',
                    'dimensions'            => 'Dimensions',
                    'path'                  => 'Ruta',
                    'created-at'            => 'Creat',
                    'updated-at'            => 'Actualitzat',
                    'embedded_meta_info'    => 'Informació Meta Incrustada',
                    'no-metadata-available' => 'No hi ha metadades disponibles',
                    'tags'                  => 'Etiquetes',
                    'select-tags'           => 'Escollir o Crear una Etiqueta',
                    'tag-attach-success'    => 'Etiqueta afegida correctament.',
                    'tag-detach-success'    => 'Etiqueta eliminada de l\'actiu correctament.',
                    'directory-path'        => 'Ruta del directori',
                    'no-directory'          => 'Cap directori assignat',
                    'details'               => 'Detalls',
                    'breadcrumb-assets'     => 'Actius',
                    'tab'                   => [
                        'preview'          => 'Vista prèvia',
                        'properties'       => 'Propietats',
                        'comments'         => 'Comentaris',
                        'linked_resources' => 'Recursos Vinculats',
                        'history'          => 'Historial',
                    ],
                    'button' => [
                        'download'            => 'Descarregar',
                        'custom_download'     => 'Descàrrega Personalitzada',
                        'rename'              => 'Canviar el nom',
                        're_upload'           => 'Tornar a Pujar',
                        're_uploading'        => 'Tornant a pujar...',
                        'cancel'              => 'Cancel·lar',
                        're-upload-cancelled' => 'Càrrega cancel·lada.',
                        'delete'              => 'Eliminar',
                        'share'               => 'Compartir',
                    ],
                    'share' => [
                        'title'      => 'Compartir recurs',
                        'link'       => 'Enllaç per compartir',
                        'copy'       => 'Copiar',
                        'copied'     => 'Enllaç copiat al porta-retalls',
                        'expiry'     => 'L\'enllaç expira després de',
                        'expiry-1d'  => '1 dia',
                        'expiry-7d'  => '7 dies',
                        'expiry-30d' => '30 dies',
                        'expires-on' => 'Expira el',
                        'failed'     => 'No s\'ha pogut generar l\'enllaç de compartició.',
                    ],
                    'preview-modal' => [
                        'not-available' => 'La vista prèvia no està disponible per a aquest tipus de fitxer.',
                        'download-file' => 'Descarregar Fitxer',
                        'download-zip'  => 'Descarrega com a ZIP',
                        'close'         => 'Tancar la previsualització',
                        'mime'          => 'MIME',
                        'card'          => [
                            'click-for-details' => 'Feu clic per a tots els detalls',
                            'preview'           => 'Previsualització',
                            'edit-image'        => 'Edita la imatge',
                        ],
                        'image-viewer' => [
                            'rotate-left'   => 'Gira a l\'esquerra (L)',
                            'rotate-right'  => 'Gira a la dreta (R)',
                            'zoom-out'      => 'Allunyar (-)',
                            'zoom-in'       => 'Apropar (+)',
                            'fit-to-screen' => 'Ajustar a la pantalla',
                            'actual-size'   => 'Mida real',
                            'reset-all'     => 'Restablir tot (0)',
                        ],
                        'video-player' => [
                            'back-10s'           => 'Enrere 10 s',
                            'forward-10s'        => 'Endavant 10 s',
                            '10s'                => '10 s',
                            'speed'              => 'Velocitat',
                            'loop'               => 'Bucle (L)',
                            'mute'               => 'Silenci (M)',
                            'picture-in-picture' => 'Imatge en imatge',
                            'fullscreen'         => 'Pantalla completa (F)',
                            'more-actions'       => 'Més accions',
                            'copy-link'          => 'Copia l\'enllaç',
                            'link-copied'        => 'Copiat!',
                            'open-in-new-tab'    => 'Obre en una pestanya nova',
                        ],
                    ],
                    'custom-download' => [
                        'title'              => 'Descàrrega Personalitzada',
                        'format'             => 'Format',
                        'width'              => 'Amplada (px)',
                        'width-placeholder'  => '200',
                        'height'             => 'Alçada (px)',
                        'height-placeholder' => '200',
                        'download-btn'       => 'Descarregar',
                        'extension-types'    => [
                            'jpg'      => 'JPG',
                            'png'      => 'PNG',
                            'jpeg'     => 'JPEG',
                            'webp'     => 'WEBP',
                            'original' => 'Original',
                        ],
                    ],
                    'tag-already-exists'        => 'L\'etiqueta ja existeix',
                    'image-source-not-readable' => 'Font de la imatge no llegible',
                    'failed-to-read'            => 'No s\'han pogut llegir les metadades de la imatge :exception',
                    'file-re-upload-success'    => 'Fitxers tornats a pujar correctament.',
                    'image-processing-failed'   => 'El processament de la imatge ha fallat: :message',
                    'image-editor'              => [
                        'tools'                    => 'Eines',
                        'close'                    => 'Tancar l\'editor',
                        'crop'                     => 'Retallar i redimensionar',
                        'crop-sub'                 => 'Retallar o escalar la imatge',
                        'adjust'                   => 'Brillantor i contrast',
                        'adjust-sub'               => 'Ajustar la llum i el to',
                        'rotate'                   => 'Girar i invertir',
                        'rotate-sub'               => 'Transformar l\'orientació',
                        'selection'                => 'Selecció',
                        'drag-handles'             => 'controladors d\'arrossegament',
                        'scale-after-crop'         => 'Escalar després de retallar',
                        'optional'                 => 'opcional',
                        'width-px'                 => 'Amplada (px)',
                        'height-px'                => 'Alçada (px)',
                        'blank-keep-dims'          => 'Deixeu en blanc per mantenir les dimensions retallades.',
                        'auto'                     => 'automàtic',
                        'adjustments'              => 'Ajustaments',
                        'brightness'               => 'Brillantor',
                        'contrast'                 => 'Contrast',
                        'rotation'                 => 'Rotació',
                        'flip'                     => 'Invertir',
                        'horizontal'               => 'Horitzontal',
                        'vertical'                 => 'Vertical',
                        'platform'                 => 'Plataforma',
                        'platform-loading'         => 'Carregant…',
                        'no-platforms'             => 'Cap plataforma',
                        'color-none'               => 'Cap',
                        'color-white'              => 'Blanc',
                        'color-silver'             => 'Plata',
                        'color-light-gray'         => 'Gris clar',
                        'color-gray'               => 'Gris',
                        'color-slate'              => 'Gris pissarra',
                        'color-dark-gray'          => 'Gris fosc',
                        'color-charcoal'           => 'Carbó',
                        'color-black'              => 'Negre',
                        'color-light-red'          => 'Vermell clar',
                        'color-red'                => 'Vermell',
                        'color-light-yellow'       => 'Groc clar',
                        'color-yellow'             => 'Groc',
                        'color-light-green'        => 'Verd clar',
                        'color-green'              => 'Verd',
                        'color-light-cyan'         => 'Cian clar',
                        'color-cyan'               => 'Cian',
                        'color-light-blue'         => 'Blau clar',
                        'color-blue'               => 'Blau',
                        'color-light-purple'       => 'Porpra clar',
                        'color-purple'             => 'Porpra',
                        'color-light-pink'         => 'Rosa clar',
                        'color-pink'               => 'Rosa',
                        'color-light-rose'         => 'Rosa pàl·lid',
                        'color-rose'               => 'Rosa',
                        'model'                    => 'Model',
                        'no-models'                => 'Cap model',
                        'prompt'                   => 'Indicació',
                        'apply'                    => 'Aplicar',
                        'applying'                 => 'Aplicant…',
                        'error-platforms'          => 'No s\'han pogut carregar les plataformes d\'IA.',
                        'error-select-color'       => 'Seleccioneu un color de fons.',
                        'error-operation'          => 'L\'operació ha fallat. Torneu-ho a provar.',
                        'success-updated'          => 'Imatge actualitzada correctament.',
                        'success-adjusted'         => 'Imatge ajustada correctament.',
                        'success-transformed'      => 'Imatge transformada correctament.',
                        'error-no-ai-image'        => 'La IA no ha retornat cap imatge.',
                        'error-provide-dims'       => 'Proporcioneu almenys una regió de retall o dimensions de sortida.',
                        'edit-bg'                  => 'Editar fons',
                        'edit-bg-sub'              => 'Color, càrrega o indicació',
                        'bg-tab-color'             => 'Color',
                        'bg-tab-upload'            => 'Carregar',
                        'bg-tab-ai'                => 'Indicació',
                        'bg-color-label'           => 'Color de fons',
                        'bg-color-mode-normal'     => 'Normal',
                        'bg-color-mode-ai'         => 'AI',
                        'bg-custom-color'          => 'Personalitzat',
                        'bg-upload-hint'           => 'Feu clic per carregar una imatge de fons',
                        'bg-upload-change'         => 'Feu clic per canviar',
                        'bg-ai-prompt-placeholder' => 'Descriviu el fons que voleu…',
                        'sharpen'                  => 'Enfocament',
                        'blur'                     => 'Desenfocament',
                        'filters'                  => 'Filtres',
                        'filters-sub'              => 'Escala de grisos i inversió',
                        'greyscale'                => 'Escala de grisos',
                        'invert'                   => 'Invertir',
                        'error-no-filter'          => 'Seleccioneu almenys un filtre per aplicar.',
                        'error-provider-no-images' => 'Aquest proveïdor d\'IA no admet la generació d\'imatges.',
                        'error-no-image-platform'  => 'No s\'han trobat plataformes d\'IA amb suport per a la generació d\'imatges. Configureu una plataforma OpenAI, Gemini o xAI.',
                    ],
                ],
                'linked-resources' => [
                    'index' => [
                        'datagrid' => [
                            'product'       => 'Producte',
                            'category'      => 'Categoria',
                            'product-sku'   => 'Sku del Producte: ',
                            'category code' => 'Codi de la Categoria: ',
                            'resource-type' => 'Tipus de Recurs',
                            'resource'      => 'Recurs',
                            'resource-view' => 'Vista del Recurs',
                        ],
                    ],
                    'found-success' => 'Recurs trobat correctament',
                    'not-found'     => 'Recurs no trobat',
                ],
                'tags' => [
                    'create' => [
                        'create-success' => 'Les etiquetes s\'han afegit correctament',
                    ],
                    'found-success'  => 'Etiqueta trobada correctament',
                    'not-found'      => 'Etiquetes no trobades',
                    'delete-success' => 'Etiquetes de l\'Actiu eliminades correctament',
                ],
                'delete-success'                          => 'Actiu eliminat correctament',
                'delete-failed-due-to-attached-resources' => 'Actiu en ús. Desvinculeu-lo abans d\'eliminar-lo',
                'datagrid'                                => [
                    'mass-delete-success'                 => 'Eliminació massiva realitzada correctament.',
                    'mass-update-success'                 => 'Actualització massiva realitzada correctament.',
                    'files-upload-success'                => 'Fitxers pujats correctament.',
                    'file-upload-success'                 => 'Fitxer pujat correctament.',
                    'not-found'                           => 'Fitxer no trobat',
                    'not-found-to-show'                   => 'Fitxer no trobat per a la visualització',
                    'not-found-to-download'               => 'Fitxer no trobat per a la descàrrega',
                    'edit-success'                        => 'Fitxer pujat correctament',
                    'no-file-name'                        => 'sense nom de fitxer',
                    'show-success'                        => 'Fitxer trobat correctament',
                    'update-success'                      => 'Fitxer actualitzat correctament',
                    'not-found-to-update'                 => 'El fitxer no existeix',
                    'not-found-to-destroy'                => 'El fitxer no existeix',
                    'files-upload-failed'                 => 'No s\'han pogut pujar els fitxers.',
                    'file-upload-failed'                  => 'No s\'ha pogut pujar el fitxer',
                    'invalid-file'                        => 'Fitxer no vàlid proporcionat',
                    'invalid-file-format-or-not-provided' => 'No s\'han proporcionat fitxers o el format no és vàlid.',
                    'file-process-failed'                 => 'Alguns fitxers no s\'han pogut processar',
                    'file-forbidden-type'                 => 'El fitxer té un tipus o extensió prohibits.',
                    'file-too-large'                      => 'El fitxer és massa gran. La mida màxima permesa és :size.',
                    'storage-upload-failed'               => 'Error en pujar ":name" a l\'emmagatzematge. Comproveu les credencials del disc i els permisos.',
                    'not-found-or-no-file'                => 'L\'actiu no s\'ha trobat o el fitxer no existeix.',
                    'download-link-ready'                 => 'L\'actiu s\'ha trobat. Podeu descarregar el fitxer des de l\'enllaç proporcionat.',
                ],
            ],
            'share' => [
                'target-not-found' => 'No s\'ha trobat el recurs o directori que s\'està compartint.',
                'not-found'        => 'Enllaç de compartició no trobat.',
                'revoked'          => 'Enllaç de compartició revocat.',
                'deleted'          => 'Enllaç de compartició eliminat.',
                'already-revoked'  => 'Aquest enllaç de compartició ja està revocat.',
                'reauthorized'     => 'Enllaç de compartició reautoritzat — l\'URL original torna a estar activa.',
                'not-revoked'      => 'Aquest enllaç de compartició no està revocat.',
                'updated'          => 'Enllaç de compartició actualitzat.',
                'update-failed'    => 'No s\'ha pogut actualitzar l\'enllaç de compartició.',
                'delete-failed'    => 'No s\'ha pogut eliminar l\'enllaç de compartició.',
                'modal'            => [
                    'title-asset'        => 'Compartir recurs',
                    'title-directory'    => 'Compartir directori',
                    'edit-title'         => 'Edita l\'enllaç de compartició',
                    'edit-name'          => 'Reanomena la compartició',
                    'unnamed-share'      => '(sense nom personalitzat)',
                    'name-label'         => 'Nom personalitzat',
                    'name-hint'          => 'Deixa-ho en blanc per conservar el nom original del fitxer / directori com a etiqueta.',
                    'save'               => 'Desa els canvis',
                    'saving'             => 'Desant…',
                    'cancel'             => 'Cancel·la',
                    'create-new'         => 'Crea un nou enllaç de compartició',
                    'expiry'             => 'L\'enllaç expira després de',
                    'expiry-1d'          => '1 dia',
                    'expiry-7d'          => '7 dies',
                    'expiry-30d'         => '30 dies',
                    'expiry-365d'        => '1 any',
                    'no-expiry'          => 'Sense expiració',
                    'create'             => 'Genera enllaç',
                    'creating'           => 'Generant…',
                    'active-links'       => 'Enllaços actius',
                    'loading'            => 'Carregant…',
                    'no-active-links'    => 'Encara no hi ha enllaços de compartició actius per a aquest element.',
                    'copy'               => 'Copiar',
                    'copied'             => 'Enllaç copiat al porta-retalls',
                    'revoke'             => 'Revocar',
                    'delete'             => 'Eliminar',
                    'revoked'            => 'Enllaç de compartició revocat',
                    'revoke-failed'      => 'No s\'ha pogut revocar l\'enllaç de compartició.',
                    'expires-on'         => 'Expira el',
                    'never-expires'      => 'No expira mai',
                    'views'              => 'Visualitzacions',
                    'downloads'          => 'Descàrregues',
                    'created'            => 'Enllaç de compartició creat.',
                    'create-failed'      => 'No s\'ha pogut crear l\'enllaç de compartició.',
                    'load-failed'        => 'No s\'han pogut carregar els enllaços de compartició existents.',
                    'advanced'           => 'Avançat',
                    'revoked-status'     => 'Enllaç revocat',
                    'reauthorize'        => 'Reautoritzar',
                    'reauthorizing'      => 'Reautoritzant…',
                    'reauthorize-failed' => 'No s\'ha pogut reautoritzar l\'enllaç de compartició.',
                    'revoked-notice'     => 'Aquest enllaç està revocat. Reautoritzeu-lo per tornar a activar l\'URL original.',
                ],
                'index' => [
                    'title'       => 'Enllaços compartits',
                    'description' => 'Tots els enllaços de compartició actius i històrics per a recursos i directoris al DAM.',
                ],
                'datagrid' => [
                    'type'           => 'Tipus',
                    'asset'          => 'Recurs',
                    'directory'      => 'Directori',
                    'target'         => 'Destí',
                    'custom-name'    => 'Nom personalitzat',
                    'created-by'     => 'Creat per',
                    'expires-at'     => 'Expira el',
                    'never'          => 'Mai',
                    'status'         => 'Estat',
                    'status-active'  => 'Actiu',
                    'status-expired' => 'Expirat',
                    'status-revoked' => 'Revocat',
                    'views'          => 'Visualitzacions',
                    'downloads'      => 'Descàrregues',
                    'created-at'     => 'Creat el',
                    'edit'           => 'Edita',
                    'copy-link'      => 'Copia l\'enllaç',
                    'revoke'         => 'Revocar',
                    'delete'         => 'Eliminar',
                ],
            ],
            'upload' => [
                'pause'     => 'Pausar',
                'resume'    => 'Reprendre',
                'retry'     => 'Torna-ho a intentar',
                'cancel'    => 'Cancel·lar',
                'paused'    => 'Càrrega en pausa. Els recursos existents no s\'han vist afectats — reprèn quan vulguis.',
                'resumed'   => 'Càrrega represa.',
                'cancelled' => 'Càrrega cancel·lada. Els recursos ja carregats s\'han mantingut.',
                'retried'   => 'Reintentant les càrregues fallides en segon pla.',
            ],
            'tag' => [
                'not-found'           => 'Etiqueta no trobada.',
                'create-success'      => 'Etiqueta creada correctament.',
                'update-success'      => 'Etiqueta actualitzada correctament.',
                'delete-success'      => 'Etiqueta eliminada correctament.',
                'mass-delete-success' => 'Etiquetes seleccionades eliminades correctament.',
                'index'               => [
                    'title'       => 'Etiquetes',
                    'description' => 'Gestioneu les etiquetes utilitzades per etiquetar i organitzar els recursos al DAM.',
                    'create'      => 'Crear etiqueta',
                ],
                'datagrid' => [
                    'name'         => 'Nom',
                    'assets-count' => 'Recursos',
                    'created-at'   => 'Creat el',
                    'edit'         => 'Editar',
                    'delete'       => 'Eliminar',
                ],
                'modal' => [
                    'create-title'     => 'Crear etiqueta',
                    'edit-title'       => 'Editar etiqueta',
                    'name-label'       => 'Nom de l\'etiqueta',
                    'name-placeholder' => 'p. ex. campanya-estiu',
                    'save'             => 'Desar',
                    'saving'           => 'Desant…',
                    'cancel'           => 'Cancel·lar',
                ],
                'mass-action' => [
                    'assign-tags'              => 'Assignar etiquetes',
                    'modal-title'              => 'Assignar etiquetes',
                    'modal-subtitle'           => 'Afegiu etiquetes a :count recurs(os) seleccionat(s). Es mantenen les etiquetes existents.',
                    'modal-subtitle-recursive' => 'Afegiu etiquetes als recursos seleccionats i a cada recurs dins de les carpetes seleccionades, de manera recursiva. Es mantenen les etiquetes existents.',
                    'tags-label'               => 'Etiquetes',
                    'tags-placeholder'         => 'Cerca etiquetes o escriu per crear-ne…',
                    'add-tag'                  => 'Premeu Retorn per afegir',
                    'assign'                   => 'Assignar',
                    'assigning'                => 'S\'està assignant…',
                    'cancel'                   => 'Cancel·lar',
                    'assign-success'           => 'Etiquetes assignades a :count recurs(os).',
                    'assign-failed'            => 'No s\'han pogut assignar les etiquetes.',
                    'no-assets'                => 'No hi ha cap recurs seleccionat.',
                    'no-items'                 => 'Seleccioneu com a mínim un recurs o carpeta.',
                    'no-tags'                  => 'Seleccioneu o afegiu com a mínim una etiqueta.',
                ],
            ],
        ],
        'catalog' => [
            'attributes' => [
                'type' => [
                    'asset' => 'Actiu',
                ],
            ],
            'category-fields' => [
                'type' => [
                    'asset' => 'Actiu',
                ],
            ],
        ],
        'acl' => [
            'menu'             => 'Gestió d\'actius',
            'asset'            => 'Actiu',
            'property'         => 'Propietat',
            'comment'          => 'Comentari',
            'linked_resources' => 'Recursos Vinculats',
            'meta_data'        => 'Informació de metadades incrustada',
            'directory'        => 'Directori',
            'tag'              => 'Etiqueta',
            'create'           => 'Crear',
            'edit'             => 'Editar',
            'update'           => 'Actualitzar',
            'delete'           => 'Eliminar',
            'list'             => 'Llista',
            'view'             => 'Veure',
            'upload'           => 'Pujar',
            're_upload'        => 'Tornar a Pujar',
            'mass_update'      => 'Actualització Massiva',
            'mass_delete'      => 'Eliminació Massiva',
            'download'         => 'Descarregar',
            'custom_download'  => 'Descàrrega Personalitzada',
            'rename'           => 'Canviar el nom',
            'move'             => 'Moure',
            'copy'             => 'Copiar',
            'copy-structure'   => 'Copiar Estructura del Directori',
            'download-zip'     => 'Descarregar Zip',
            'asset-assign'     => 'Assignar Actiu',
            'share'            => 'Compartir',
            'assets'           => 'Recursos',
            'shares'           => 'Enllaços compartits',
            'revoke'           => 'Revocar',
            'tags'             => 'Etiquetes',
            'configuration'    => 'Configuració',
        ],
        'permissions' => [
            'title'                 => 'Permisos del directori DAM',
            'tab-subtitle'          => 'Directoris als quals aquest rol pot accedir.',
            'no-actions'            => 'Només visualització. Obre una subcarpeta a la qual tinguis accés.',
            'unauthorized'          => 'No teniu permís per gestionar els permisos del directori DAM.',
            'all-directories'       => 'Tots els directoris — atorgar accés a tots els directoris ara i en el futur',
            'inherit-children'      => 'Hereda subdirectoris — accedeix automàticament a tots els subdirectoris dels directoris concedits',
            'loading'               => 'S\'estan carregant els directoris…',
            'empty'                 => 'No s\'han trobat directoris.',
            'error'                 => 'No s\'han pogut carregar els directoris. Torneu-ho a intentar.',
            'retry'                 => 'Torna-ho a intentar',
            'grants-load-warning'   => 'Alguns permisos de directori no s\'han pogut carregar. Verifiqueu les vostres seleccions abans de desar.',
            'inherit-children-note' => 'Els subdirectoris s\'inclouran automàticament en desar.',
        ],
        'validation' => [
            'asset' => [
                'required' => 'El camp :attribute és obligatori.',
            ],
            'comment' => [
                'required' => 'El missatge del comentari és obligatori.',
            ],
            'property' => [
                'name' => [
                    'required' => 'El camp Nom és obligatori.',
                    'unique'   => 'El Nom ja ha estat pres.',
                ],
                'language' => [
                    'not-found' => 'L\'idioma seleccionat no s\'ha trobat o actualment està desactivat.',
                ],
                'sort-order' => [
                    'required' => 'El camp Ordre de classificació és obligatori quan Afegir al filtre està activat.',
                ],
            ],
        ],
        'errors' => [
            '401' => 'Aquesta acció no està autoritzada.',
        ],
        'configuration' => [
            'save-btn'    => 'Desa',
            'title'       => 'Configuració',
            'saved'       => 'Configuració desada correctament.',
            'description' => 'Configura els paràmetres de gestió d\'actius digitals (DAM) de la teva aplicació.',
            'general'     => [
                'title'            => 'Configuració general',
                'description'      => 'Aquesta configuració controla l\'experiència general de navegació del DAM.',
                'explorer-enabled' => [
                    'label' => 'Activa la vista d\'exploració',
                    'hint'  => 'Utilitzeu la interfície d\'estil Explorador per navegar i gestionar els recursos.',
                ],
                'bookmarks-enabled' => [
                    'label' => 'Mostra el tauler de marcadors',
                    'hint'  => 'Mostra una barra lateral dedicada amb les carpetes i els recursos que heu afegit als marcadors.',
                ],
            ],
            'directory' => [
                'title'       => 'Configuració del directori',
                'description' => 'Aquesta configuració controla com es mostren les carpetes i els recursos al tauler del directori.',
                'show-tree'   => [
                    'label'       => 'Mostra l\'arbre de directoris',
                    'hint'        => 'Mostra la jerarquia de carpetes a la barra lateral esquerra per facilitar la navegació.',
                    'locked-hint' => 'Sempre actiu mentre la vista d\'exploració està desactivada.',
                ],
                'tree-show-assets' => [
                    'label' => 'Mostra els recursos a l\'arbre de directoris',
                    'hint'  => 'Mostra els recursos juntament amb les carpetes dins de l\'arbre de directoris en lloc de mostrar només les carpetes.',
                ],
            ],
            'ai-tagging' => [
                'title'       => 'Etiquetatge amb IA',
                'description' => 'Suggereix automàticament etiquetes per a les imatges pujades utilitzant la vostra plataforma Magic AI configurada.',
                'enabled'     => [
                    'label' => 'Etiquetar automàticament els recursos amb IA',
                    'hint'  => 'Utilitza la vostra plataforma Magic AI configurada per suggerir etiquetes per a les imatges automàticament quan es pugen.',
                ],
                'platform' => [
                    'label'                    => 'Plataforma',
                    'use-default'              => 'Utilitza la plataforma predeterminada',
                    'none-configured'          => 'No hi ha cap plataforma d\'IA configurada',
                    'none-configured-hint'     => 'Afegiu primer una plataforma Magic AI amb capacitat de visió — vegeu Configuració → Magic AI → Plataformes.',
                ],
                'max-tags' => [
                    'label' => 'Etiquetes màximes',
                    'hint'  => 'Nombre màxim d\'etiquetes a suggerir per imatge (1-20).',
                ],
                'rate-limit' => [
                    'label' => 'Límit de velocitat d\'etiquetatge IA',
                    'hint'  => 'Nombre màxim de sol·licituds d\'etiquetatge IA per minut (1-120). Redueix-ho per ajustar-te al límit del teu proveïdor d\'IA.',
                ],
            ],
        ],
        'explorer' => [
            'not-found'         => 'No s\'ha trobat el directori.',
            'access-denied'     => 'No teniu accés a aquest directori.',
            'empty'             => 'No s\'han trobat directoris.',
            'action-completed'  => 'Acció completada correctament.',
            'foreign-selection' => [
                'notice' => ':count element(s) seleccionat(s) en una altra pestanya.',
                'action' => 'Selecciona\'ls aquí també',
            ],
            'bookmarks' => [
                'title'     => 'Marcadors',
                'drag-hint' => 'Arrossegueu una carpeta aquí per afegir-la als marcadors',
                'max'       => 'S\'ha arribat al màxim de 20 marcadors.',
                'remove'    => 'Elimina el marcador',
                'empty'     => 'Encara no hi ha marcadors.',
            ],
            'tab' => [
                'new'   => 'Pestanya nova',
                'close' => 'Tanca la pestanya',
            ],
            'sections' => [
                'folders' => 'Carpetes',
                'files'   => 'Fitxers',
                'folder'  => 'Carpeta',
            ],
            'clipboard' => [
                'ready'   => 'Preparat per enganxar',
                'dismiss' => 'Esborra',
            ],
            'context' => [
                'open'                 => 'Obre',
                'open-new-tab'         => 'Obre en una pestanya nova',
                'bookmark'             => 'Afegeix directori als marcadors',
                'folder-upload'        => 'Carrega carpeta',
                'copy'                 => 'Copia',
                'paste'                => 'Enganxa',
                'copy-progress'        => 'Còpia en cua.',
                'copy-done'            => 'Copiat.',
                'paste-file-started'   => 'Enganxant el fitxer...',
                'paste-folder-started' => 'Enganxant la carpeta...',
                'paste-done'           => 'Enganxat correctament.',
                'move-asset-started'   => 'Movent el fitxer...',
                'move-folder-started'  => 'Movent la carpeta...',
                'move-done'            => 'Carpeta moguda correctament.',
                'move-asset-done'      => 'Fitxer mogut correctament.',
                'delete-done'          => 'Directori eliminat correctament.',
            ],
            'folder' => [
                'deleted' => 'Aquesta carpeta s\'ha eliminat — torneu a l\'arrel.',
            ],
            'search' => [
                'placeholder' => 'Cerca fitxers i carpetes…',
            ],
            'list' => [
                'header' => [
                    'name'     => 'Nom',
                    'type'     => 'Tipus',
                    'size'     => 'Mida',
                    'modified' => 'Modificat',
                    'actions'  => 'Accions',
                ],
                'items-count' => ':count elements',
            ],
            'dialog' => [
                'rename-asset' => [
                    'title'       => 'Canvia el nom del fitxer',
                    'placeholder' => 'Nom nou',
                ],
                'save'   => 'Desa',
                'cancel' => 'Cancel·la',
            ],
            'view' => [
                'grid' => 'Vista de quadrícula',
                'list' => 'Vista de llista',
            ],
            'mass-actions' => [
                'select-count'   => ':count seleccionat(s)',
                'select-action'  => 'Selecciona l\'acció',
                'delete'         => 'Eliminar',
                'confirm'        => 'Voleu eliminar :count element(s)? Les carpetes s\'eliminaran de manera recursiva i no es pot desfer.',
                'deleting'       => 'Eliminant :count element(s)...',
                'deleted-assets' => ':count recurs(os) eliminat(s).',
                'deleted-dirs'   => ':count carpeta(es) en cua per a l\'eliminació.',
                'move-to'        => 'Mou a…',
                'copy-to'        => 'Copia a…',
                'moving'         => 'S\'estan movent…',
                'copying'        => 'S\'estan copiant…',
                'moved-assets'   => ':count arxiu(s) mogut(s).',
                'copied-assets'  => ':count arxiu(s) copiat(s).',
                'moved-dirs'     => ':count carpeta(es) moguda(es).',
                'copied-dirs'    => ':count carpeta(es) copiada(es).',
                'pick-dest'      => 'Selecciona la destinació',
                'select-here'    => 'Selecciona aquí',
                'new-folder'     => 'Nova carpeta',
                'folder-name'    => 'Nom de la carpeta',
                'create-error'   => 'No s\'ha pogut crear la carpeta.',
                'move-done'      => ':count Fitxers moguts.',
                'copy-done'      => ':count Fitxers copiats.',
                'still-running'  => 'L\'operació encara s\'està executant en segon pla',
                'progress'       => 'Progrés',
                'total-files'    => ':count fitxer(s)',
            ],
            'toolbar' => [
                'toggle-tree'    => 'Commuta l\'arbre de directoris',
                'toggle-sidebar' => 'Commuta la barra lateral',
            ],
            'upload' => [
                'queued'              => 'En cua',
                'uploading'           => 'Pujant',
                'creating'            => 'Creant la carpeta',
                'done'                => 'Fet',
                'failed'              => 'Ha fallat',
                'interrupted'         => 'Interromput: torna a afegir-lo per reintentar',
                'resuming'            => 'Reprenent…',
                'overall'             => ':done de :total pujats',
                'too-large-to-resume' => 'El lot és massa gran per reprendre\'s automàticament després d\'actualitzar',
            ],
        ],
    ],
    'share' => [
        'public' => [
            'app-name'              => 'DAM',
            'download'              => 'Descarregar',
            'back-to-gallery'       => 'Tornar a la galeria',
            'back'                  => 'Enrere',
            'file-name'             => 'Nom del fitxer',
            'file-type'             => 'Tipus de fitxer',
            'file-size'             => 'Mida del fitxer',
            'video-not-supported'   => 'El vostre navegador no admet l\'etiqueta de vídeo.',
            'audio-not-supported'   => 'El vostre navegador no admet l\'etiqueta d\'àudio.',
            'preview-not-available' => 'La previsualització no està disponible per a aquest fitxer. Useu el botó Descarregar a dalt.',
            'files-count'           => ':count fitxer(s)',
            'empty-directory'       => 'Aquest directori compartit encara no té fitxers.',
            'expires-on'            => 'Aquest enllaç expira el',
            'powered-by-dam'        => 'Impulsat per Unopim DAM',
            'expired-title'         => 'Enllaç expirat',
            'expired-message'       => 'Aquest enllaç de compartició ha expirat i ja no és accessible. Sol·liciteu un nou enllaç al propietari.',
            'revoked-title'         => 'Enllaç revocat',
            'revoked-message'       => 'Aquest enllaç de compartició ha estat revocat pel seu propietari i ja no és accessible.',
            'not-found-title'       => 'Enllaç no trobat',
            'not-found-message'     => 'L\'enllaç compartit que heu obert no existeix. Pot ser que s\'hagi escrit malament o s\'hagi eliminat.',
            'download-zip'          => 'Descarregar tot com a ZIP',
            'per-page'              => 'Per pàgina',
        ],
    ],
    'data-transfer' => [
        'bundle' => [
            'invalid-archive'            => 'The uploaded archive could not be opened.',
            'invalid-zip'                => 'The uploaded archive is damaged and cannot be read.',
            'no-data-file'               => 'The archive contains no CSV or Excel file to import.',
            'zip-too-many-entries'       => 'The archive contains :count entries, which is above the limit of :limit.',
            'zip-contents-too-large'     => 'The archive expands to more than :limit MB.',
            'zip-compression-suspicious' => 'The entry ":entry" is compressed far beyond a normal ratio and was rejected.',
            'asset-not-found'            => 'No asset was found at path ":path".',
            'asset-tree-misnamed'        => 'L\'arxiu conté una carpeta anomenada ":name" en lloc de "assets", de manera que no s\'ha pogut importar cap dels seus actius. Canvieu-li el nom a "assets" i mantingueu les carpetes de sota coincidents amb les rutes del fitxer de dades, per exemple "assets/Root/Seed/Tech Sheets/Cotton/". macOS desa una "/" escrita en un nom de carpeta com a ":", que és la causa habitual.',
        ],
    ],
];
