<?php

return [
    'admin' => [
        'components' => [
            'layouts' => [
                'sidebar' => [
                    'dam'           => '资产管理',
                    'assets'        => '资产',
                    'shares'        => '共享链接',
                    'tags'          => '标签',
                    'configuration' => '配置',
                ],
            ],
            'modal' => [
                'confirm' => [
                    'message' => '删除此目录将同时删除其中的所有子目录。此操作是永久性的，无法撤销。',
                ],
            ],
            'asset' => [
                'field' => [
                    'add-asset'     => '添加资产',
                    'assign-assets' => '分配资产',
                    'preview'       => '预览',
                    'remove'        => '移除',
                    'download'      => '下载',
                ],
            ],
        ],
        'dam' => [
            'file' => [
                'deleted'   => '文件已删除',
                'not-found' => '文件未找到',
            ],
            'image-edit' => [
                'cannot-decode'  => '无法解码图像',
                'preview-failed' => '预览失败',
            ],
            'index' => [
                'title'    => '数字资产管理',
                'datagrid' => [
                    'file-name'      => '文件名',
                    'tags'           => '标签',
                    'property-name'  => '属性名称',
                    'property-value' => '属性值',
                    'created-at'     => '创建时间',
                    'updated-at'     => '更新时间',
                    'extension'      => '扩展名',
                    'path'           => '路径',
                ],
                'mass-action' => [
                    'deleting'   => '正在删除 :count 个资产…',
                    'processing' => '正在处理 :count 个资产…',
                ],
                'move' => [
                    'directory' => '正在移动目录 ":name"…',
                    'asset'     => '正在移动资产 ":name"…',
                ],
                'directory' => [
                    'title'  => '目录',
                    'create' => [
                        'title'     => '创建目录',
                        'name'      => '名称',
                        'file_name' => '文件名',
                        'save-btn'  => '保存目录',
                    ],
                    'rename' => [
                        'title' => '重命名目录',
                    ],
                    'search' => [
                        'placeholder'     => '搜索目录...',
                        'no-matches'      => '无匹配项',
                        'error'           => '搜索失败 — 请重试',
                        'not-found-flash' => '目录已无法访问',
                        'count'           => ':total 中的 :shown 个匹配',
                        'count-total'     => ':total 个匹配',
                    ],
                    'asset' => [
                        'rename' => [
                            'title'    => '重命名资产',
                            'save-btn' => '保存资产',
                        ],
                    ],
                    'actions' => [
                        'delete'                    => '删除',
                        'edit'                      => '编辑',
                        'rename'                    => '重命名',
                        'copy'                      => '复制',
                        'cut'                       => '剪切',
                        'download'                  => '下载',
                        'download-zip'              => '下载 Zip',
                        'paste'                     => '粘贴',
                        'add-directory'             => '添加目录',
                        'upload-files'              => '上传文件',
                        'upload-files-and-folders'  => '上传文件和文件夹',
                        'upload-folder'             => '上传文件夹',
                        'upload-drop-hint'          => '将文件或文件夹拖放到此处',
                        'upload-or'                 => '或浏览：',
                        'copy-directory-structured' => '复制目录结构',
                        'get'                       => 'Get',
                        'get-by-id'                 => '通过 ID 获取',
                        'comment'                   => '评论',
                        'share'                     => '共享目录',
                    ],
                    'linked-resources'                          => '链接资源',
                    'not-found'                                 => '未找到目录',
                    'created-success'                           => '目录创建成功',
                    'creation-failed'                           => '创建目录失败',
                    'updated-success'                           => '目录更新成功',
                    'update-failed'                             => '更新目录失败',
                    'moved-success'                             => '目录移动成功',
                    'cannot-move'                               => '无法移动目录',
                    'cannot-copy'                               => '无法将文件夹复制到其自身或其子文件夹中。',
                    'fetch-all-success'                         => '目录获取成功',
                    'can-not-deleted'                           => '不能删除根目录。',
                    'deleting-in-progress'                      => '目录删除中',
                    'can-not-copy'                              => '根目录不能被复制。',
                    'coping-in-progress'                        => '目录结构复制中。',
                    'asset-not-found'                           => '未找到资产',
                    'asset-renamed-success'                     => '资产重命名成功',
                    'asset-moved-success'                       => '资产移动成功',
                    'asset-name-already-exist'                  => '新名称已存在，另有一个资产名为 :asset_name',
                    'asset-name-conflict-in-the-same-directory' => '资产名称与同一目录中的现有文件冲突。',
                    'old-file-not-found'                        => '在路径 :old_path 请求的文件未找到。',
                    'image-name-is-the-same'                    => '该名称已存在，请输入一个不同的名称。',
                    'not-writable'                              => '您没有权限在此位置 ":path" 执行 :actionType 操作。',
                    'empty-directory'                           => '此目录为空。',
                    'failed-download-directory'                 => '无法创建 zip 文件。',
                    'not-allowed'                               => '不允许上传脚本文件。',
                    'copy-success'                              => '文件夹复制成功。',
                    'name-unique'                               => '目录名称在同一父目录中必须唯一。',
                    'folder-upload-success'                     => '文件夹上传成功。',
                    'folder-upload-partial'                     => '文件夹已上传，由于错误跳过了 :count 个文件。',
                    'load-more'                                 => '加载更多',
                ],
                'description'                    => '该工具可以帮助您在一个地方组织、存储和管理所有的媒体资产。',
                'root'                           => '根目录',
                'upload'                         => '上传',
                'new'                            => '新建',
                'uploading'                      => '上传中...',
                'cancel'                         => '取消',
                'upload-cancelled'               => '上传已取消。',
                'folder-upload-https-required'   => '文件夹上传需要安全连接 (HTTPS)。',
                'folder-upload-no-https-warning' => '没有安全连接 (HTTPS) 将不会创建空文件夹。文件仍将被上传。',
                'folder-upload-no-files'         => '所选文件夹不包含任何文件。没有安全连接 (HTTPS) 无法检测空子文件夹。',
                'upload-in-progress-title'       => '上传进行中',
                'upload-in-progress-message'     => '离开此页面将取消正在进行的上传。您确定要离开吗？',
                'upload-leave-page'              => '离开',
                'upload-stay-page'               => '留下',
                'drop-zone-hint'                 => '拖拽文件或文件夹以上传',
                'select-all'                     => '全选',
                'select-directory-to-upload'     => '请选择要上传到的目录。',
                'upload-session'                 => [
                    'uploading'            => '正在上传 :total 个文件…',
                    'paused'               => '已暂停 — 已上传 :done / :total',
                    'interrupted'          => '已上传 :done / :total · :interrupted 个中断',
                    'failed'               => '已上传 :done 个，:failed 个失败',
                    'completed'            => '已上传 :done / :total',
                    'create-folder-failed' => '无法创建文件夹',
                ],
                'drop-zone-no-permission' => '您没有在此处上传的权限。',
                'upload-complete'         => '上传完成',
                'item-upload-complete'    => '上传完成',
                'copy'                    => [
                    'directory' => '正在复制目录 ":name"…',
                ],
                'delete' => [
                    'directory' => '正在删除目录 ":name"…',
                ],
            ],
            'asset' => [
                'properties' => [
                    'index' => [
                        'title'      => '资产属性',
                        'create-btn' => '创建属性',
                        'datagrid'   => [
                            'name'     => '名称',
                            'type'     => '类型',
                            'language' => '语言',
                            'value'    => '值',
                            'edit'     => '编辑',
                            'delete'   => '删除',
                        ],
                        'create' => [
                            'title'                  => '创建属性',
                            'name'                   => '名称',
                            'type'                   => '类型',
                            'language'               => '语言',
                            'value'                  => '值',
                            'add-to-filter'          => '添加到筛选',
                            'add-to-filter-help'     => '启用后，此属性将作为筛选条件显示在资产列表侧边栏中。',
                            'sort-order'             => '排序顺序',
                            'sort-order-help'        => '数字越小在筛选侧边栏中越靠前。相同数字的属性按字母顺序排列。',
                            'sort-order-placeholder' => '例如 10',
                            'save-btn'               => '保存',
                        ],
                        'edit' => [
                            'title' => '编辑属性',
                        ],
                        'delete-success' => '资产属性删除成功',
                        'delete-failed'  => '删除资产属性失败',
                        'create-success' => '资产属性创建成功',
                        'update-success' => '资产属性更新成功',
                        'update-failure' => '更新资产属性失败',
                        'not-found'      => '未找到属性',
                        'found-success'  => '属性查找成功',
                    ],
                ],
                'comments' => [
                    'create' => [
                        'create-success' => '评论已成功添加',
                        'create-failure' => '评论创建失败',
                    ],
                    'post-comment'    => '发布评论',
                    'post-reply'      => '发布回复',
                    'reply'           => '回复',
                    'edit-btn'        => '编辑',
                    'delete-btn'      => '删除',
                    'save-btn'        => '保存',
                    'cancel-btn'      => '取消',
                    'add-reply'       => '添加回复',
                    'add-comment'     => '添加评论',
                    'no-comments'     => '暂无评论',
                    'not-found'       => '未找到评论',
                    'updated-success' => '评论更新成功',
                    'update-failed'   => '评论更新失败',
                    'delete-success'  => '资产评论删除成功',
                    'delete-failed'   => '资产评论删除失败',
                    'deleted-user'    => '已删除用户',
                ],
                'edit' => [
                    'title'                 => '编辑资产',
                    'back'                  => '返回',
                    'previous'              => '上一个',
                    'next'                  => '下一个',
                    'name'                  => '名称',
                    'value'                 => '值',
                    'save-btn'              => '保存',
                    'file-name'             => '文件名',
                    'file-info'             => '文件信息',
                    'type'                  => '类型',
                    'size'                  => '大小',
                    'dimensions'            => '尺寸',
                    'path'                  => '路径',
                    'created-at'            => '创建时间',
                    'updated-at'            => '更新时间',
                    'embedded_meta_info'    => '嵌入的元数据',
                    'no-metadata-available' => '没有可用的元数据',
                    'tags'                  => '标签',
                    'select-tags'           => '选择或创建标签',
                    'tag-attach-success'    => '标签添加成功。',
                    'tag-detach-success'    => '已从资产中移除标签。',
                    'directory-path'        => '目录路径',
                    'no-directory'          => '未分配目录',
                    'details'               => '详情',
                    'breadcrumb-assets'     => '资产',
                    'tab'                   => [
                        'preview'          => '预览',
                        'properties'       => '属性',
                        'comments'         => '评论',
                        'linked_resources' => '关联资源',
                        'history'          => '历史',
                    ],
                    'button' => [
                        'download'            => '下载',
                        'custom_download'     => '自定义下载',
                        'rename'              => '重命名',
                        're_upload'           => '重新上传',
                        're_uploading'        => '重新上传中...',
                        'cancel'              => '取消',
                        're-upload-cancelled' => '重新上传已取消。',
                        'delete'              => '删除',
                        'share'               => '共享',
                    ],
                    'share' => [
                        'title'      => '共享资产',
                        'link'       => '共享链接',
                        'copy'       => '复制',
                        'copied'     => '链接已复制到剪贴板',
                        'expiry'     => '链接过期时间',
                        'expiry-1d'  => '1天',
                        'expiry-7d'  => '7天',
                        'expiry-30d' => '30天',
                        'expires-on' => '到期日期',
                        'failed'     => '生成共享链接失败。',
                    ],
                    'preview-modal' => [
                        'not-available' => '此文件类型不支持预览。',
                        'download-file' => '下载文件',
                        'download-zip'  => '下载为ZIP',
                        'close'         => '关闭预览',
                        'mime'          => 'MIME',
                        'card'          => [
                            'click-for-details' => '点击查看完整详情',
                            'preview'           => '预览',
                            'edit-image'        => '编辑图片',
                        ],
                        'image-viewer' => [
                            'rotate-left'   => '向左旋转 (L)',
                            'rotate-right'  => '向右旋转 (R)',
                            'zoom-out'      => '缩小 (-)',
                            'zoom-in'       => '放大 (+)',
                            'fit-to-screen' => '适应屏幕',
                            'actual-size'   => '实际大小',
                            'reset-all'     => '重置全部 (0)',
                        ],
                        'video-player' => [
                            'back-10s'           => '后退 10 秒',
                            'forward-10s'        => '前进 10 秒',
                            '10s'                => '10 秒',
                            'speed'              => '速度',
                            'loop'               => '循环 (L)',
                            'mute'               => '静音 (M)',
                            'picture-in-picture' => '画中画',
                            'fullscreen'         => '全屏 (F)',
                            'more-actions'       => '更多操作',
                            'copy-link'          => '复制链接',
                            'link-copied'        => '已复制！',
                            'open-in-new-tab'    => '在新标签页中打开',
                        ],
                    ],
                    'custom-download' => [
                        'title'              => '自定义下载',
                        'format'             => '格式',
                        'width'              => '宽度（px）',
                        'width-placeholder'  => '200',
                        'height'             => '高度（px）',
                        'height-placeholder' => '200',
                        'download-btn'       => '下载',
                        'extension-types'    => [
                            'jpg'      => 'JPG',
                            'png'      => 'PNG',
                            'jpeg'     => 'JPEG',
                            'webp'     => 'WEBP',
                            'original' => '原始',
                        ],
                    ],
                    'tag-already-exists'        => '标签已存在',
                    'image-source-not-readable' => '无法读取图像源',
                    'failed-to-read'            => '无法读取图像元数据 :exception',
                    'file-re-upload-success'    => '文件重新上传成功。',
                    'image-processing-failed'   => '图像处理失败：:message',
                    'image-editor'              => [
                        'tools'                    => '工具',
                        'close'                    => '关闭编辑器',
                        'crop'                     => '裁剪与调整大小',
                        'crop-sub'                 => '裁剪或缩放图像',
                        'adjust'                   => '亮度与对比度',
                        'adjust-sub'               => '调整光线和色调',
                        'rotate'                   => '旋转与翻转',
                        'rotate-sub'               => '变换方向',
                        'selection'                => '选择',
                        'drag-handles'             => '拖动手柄',
                        'scale-after-crop'         => '裁剪后缩放',
                        'optional'                 => '可选',
                        'width-px'                 => '宽度 (px)',
                        'height-px'                => '高度 (px)',
                        'blank-keep-dims'          => '留空以保持裁剪尺寸。',
                        'auto'                     => '自动',
                        'adjustments'              => '调整',
                        'brightness'               => '亮度',
                        'contrast'                 => '对比度',
                        'rotation'                 => '旋转',
                        'flip'                     => '翻转',
                        'horizontal'               => '水平',
                        'vertical'                 => '垂直',
                        'platform'                 => '平台',
                        'platform-loading'         => '加载中…',
                        'no-platforms'             => '无平台',
                        'color-none'               => '无',
                        'color-white'              => '白色',
                        'color-silver'             => '银色',
                        'color-light-gray'         => '浅灰色',
                        'color-gray'               => '灰色',
                        'color-slate'              => '石板灰',
                        'color-dark-gray'          => '深灰色',
                        'color-charcoal'           => '炭灰色',
                        'color-black'              => '黑色',
                        'color-light-red'          => '浅红色',
                        'color-red'                => '红色',
                        'color-light-yellow'       => '浅黄色',
                        'color-yellow'             => '黄色',
                        'color-light-green'        => '浅绿色',
                        'color-green'              => '绿色',
                        'color-light-cyan'         => '浅青色',
                        'color-cyan'               => '青色',
                        'color-light-blue'         => '浅蓝色',
                        'color-blue'               => '蓝色',
                        'color-light-purple'       => '浅紫色',
                        'color-purple'             => '紫色',
                        'color-light-pink'         => '浅粉色',
                        'color-pink'               => '粉色',
                        'color-light-rose'         => '浅玫瑰色',
                        'color-rose'               => '玫瑰色',
                        'model'                    => '模型',
                        'no-models'                => '无模型',
                        'prompt'                   => '提示',
                        'apply'                    => '应用',
                        'applying'                 => '应用中…',
                        'error-platforms'          => '加载 AI 平台失败。',
                        'error-select-color'       => '请选择背景颜色。',
                        'error-operation'          => '操作失败，请重试。',
                        'success-updated'          => '图像已成功更新。',
                        'success-adjusted'         => '图像已成功调整。',
                        'success-transformed'      => '图像已成功变换。',
                        'error-no-ai-image'        => 'AI 未返回任何图像。',
                        'error-provide-dims'       => '至少提供一个裁剪区域或输出尺寸。',
                        'edit-bg'                  => '编辑背景',
                        'edit-bg-sub'              => '颜色、上传或提示词',
                        'bg-tab-color'             => '颜色',
                        'bg-tab-upload'            => '上传',
                        'bg-tab-ai'                => '提示词',
                        'bg-color-label'           => '背景颜色',
                        'bg-color-mode-normal'     => '普通',
                        'bg-color-mode-ai'         => 'AI',
                        'bg-custom-color'          => '自定义',
                        'bg-upload-hint'           => '点击上传背景图片',
                        'bg-upload-change'         => '点击更改',
                        'bg-ai-prompt-placeholder' => '描述您想要的背景…',
                        'sharpen'                  => '锐化',
                        'blur'                     => '模糊',
                        'filters'                  => '滤镜',
                        'filters-sub'              => '灰度与反转',
                        'greyscale'                => '灰度',
                        'invert'                   => '反转',
                        'error-no-filter'          => '请至少选择一个滤镜。',
                        'error-provider-no-images' => '此AI提供商不支持图像生成。',
                        'error-no-image-platform'  => '未找到支持图像生成的AI平台。请配置OpenAI、Gemini或xAI平台。',
                    ],
                ],
                'linked-resources' => [
                    'index' => [
                        'datagrid' => [
                            'product'       => '产品',
                            'category'      => '分类',
                            'product-sku'   => '产品SKU：',
                            'category code' => '分类代码：',
                            'resource-type' => '资源类型',
                            'resource'      => '资源',
                            'resource-view' => '资源视图',
                        ],
                    ],
                    'found-success' => '资源找到成功',
                    'not-found'     => '未找到资源',
                ],
                'tags' => [
                    'create' => [
                        'create-success' => '标签已成功添加',
                    ],
                    'found-success'  => '标签找到成功',
                    'not-found'      => '未找到标签',
                    'delete-success' => '资产标签已成功移除',
                ],
                'delete-success'                          => '资产删除成功',
                'delete-failed-due-to-attached-resources' => '资源正在使用中。删除前请先取消关联',
                'datagrid'                                => [
                    'mass-delete-success'                 => '批量删除成功。',
                    'mass-update-success'                 => '批量更新成功。',
                    'files-upload-success'                => '文件上传成功。',
                    'file-upload-success'                 => '文件上传成功。',
                    'not-found'                           => '未找到文件',
                    'not-found-to-show'                   => '找不到用于显示的文件',
                    'not-found-to-download'               => '找不到用于下载的文件',
                    'edit-success'                        => '文件上传成功',
                    'no-file-name'                        => '无文件名',
                    'show-success'                        => '文件找到成功',
                    'update-success'                      => '文件更新成功',
                    'not-found-to-update'                 => '文件不存在',
                    'not-found-to-destroy'                => '文件不存在',
                    'files-upload-failed'                 => '文件上传失败。',
                    'file-upload-failed'                  => '文件上传失败',
                    'invalid-file'                        => '提供的文件无效',
                    'invalid-file-format-or-not-provided' => '没有提供文件或格式无效。',
                    'file-process-failed'                 => '一些文件处理失败',
                    'file-forbidden-type'                 => '文件类型或扩展名不允许。',
                    'file-too-large'                      => '文件过大。允许的最大大小为 :size。',
                    'storage-upload-failed'               => '上传":name"到存储失败。请检查磁盘凭据和权限。',
                    'not-found-or-no-file'                => '未找到资产或文件不存在。',
                    'download-link-ready'                 => '已找到资产。您可以从提供的链接下载文件。',
                ],
            ],
            'share' => [
                'target-not-found' => '找不到要共享的资产或目录。',
                'not-found'        => '共享链接不存在。',
                'revoked'          => '共享链接已撤销。',
                'deleted'          => '分享链接已删除。',
                'already-revoked'  => '此共享链接已被撤销。',
                'reauthorized'     => '分享链接已重新授权 — 原始链接重新激活。',
                'not-revoked'      => '此分享链接未被撤销。',
                'updated'          => '分享链接已更新。',
                'update-failed'    => '更新分享链接失败。',
                'delete-failed'    => '删除分享链接失败。',
                'modal'            => [
                    'title-asset'        => '共享资产',
                    'title-directory'    => '共享目录',
                    'edit-title'         => '编辑分享链接',
                    'edit-name'          => '重命名分享',
                    'unnamed-share'      => '（无自定义名称）',
                    'name-label'         => '自定义名称',
                    'name-hint'          => '留空以保留原始文件 / 目录名称作为标签。',
                    'save'               => '保存更改',
                    'saving'             => '保存中…',
                    'cancel'             => '取消',
                    'create-new'         => '创建新的共享链接',
                    'expiry'             => '链接过期时间',
                    'expiry-1d'          => '1天',
                    'expiry-7d'          => '7天',
                    'expiry-30d'         => '30天',
                    'expiry-365d'        => '1年',
                    'no-expiry'          => '永不过期',
                    'create'             => '生成链接',
                    'creating'           => '生成中…',
                    'active-links'       => '有效链接',
                    'loading'            => '加载中…',
                    'no-active-links'    => '此项目暂无有效共享链接。',
                    'copy'               => '复制',
                    'copied'             => '链接已复制到剪贴板',
                    'revoke'             => '撤销',
                    'delete'             => 'Delete',
                    'revoked'            => '共享链接已撤销',
                    'revoke-failed'      => '撤销共享链接失败。',
                    'expires-on'         => '到期日期',
                    'never-expires'      => '永不过期',
                    'views'              => '浏览次数',
                    'downloads'          => '下载次数',
                    'created'            => '共享链接已创建。',
                    'create-failed'      => '创建共享链接失败。',
                    'load-failed'        => '加载现有共享链接失败。',
                    'advanced'           => '高级',
                    'revoked-status'     => '链接已撤销',
                    'reauthorize'        => '重新授权',
                    'reauthorizing'      => '重新授权中…',
                    'reauthorize-failed' => '重新授权分享链接失败。',
                    'revoked-notice'     => '此链接已撤销。重新授权以使原始链接重新激活。',
                ],
                'index' => [
                    'title'       => '共享链接',
                    'description' => 'DAM中资产和目录的所有有效及历史共享链接。',
                ],
                'datagrid' => [
                    'type'           => '类型',
                    'asset'          => '资产',
                    'directory'      => '目录',
                    'target'         => '目标',
                    'custom-name'    => '自定义名称',
                    'created-by'     => '创建者',
                    'expires-at'     => '过期时间',
                    'never'          => '永不',
                    'status'         => '状态',
                    'status-active'  => '有效',
                    'status-expired' => '已过期',
                    'status-revoked' => '已撤销',
                    'views'          => '浏览次数',
                    'downloads'      => '下载次数',
                    'created-at'     => '创建时间',
                    'edit'           => '编辑',
                    'copy-link'      => '复制链接',
                    'revoke'         => '撤销',
                    'delete'         => '删除',
                ],
            ],
            'upload' => [
                'pause'     => '暂停',
                'resume'    => '恢复',
                'retry'     => '重试',
                'cancel'    => '取消',
                'paused'    => '上传已暂停。现有资产不受影响 — 可随时恢复。',
                'resumed'   => '上传已恢复。',
                'cancelled' => '上传已取消。已上传的资产已保留。',
                'retried'   => '正在后台重试失败的上传。',
            ],
            'tag' => [
                'not-found'           => '未找到标签。',
                'create-success'      => '标签创建成功。',
                'update-success'      => '标签更新成功。',
                'delete-success'      => '标签删除成功。',
                'mass-delete-success' => '已选标签删除成功。',
                'index'               => [
                    'title'       => '标签',
                    'description' => '管理用于标记和组织 DAM 中资产的标签。',
                    'create'      => '创建标签',
                ],
                'datagrid' => [
                    'name'         => '名称',
                    'assets-count' => '资产',
                    'created-at'   => '创建时间',
                    'edit'         => '编辑',
                    'delete'       => '删除',
                ],
                'modal' => [
                    'create-title'     => '创建标签',
                    'edit-title'       => '编辑标签',
                    'name-label'       => '标签名称',
                    'name-placeholder' => '例如：夏季推广',
                    'save'             => '保存',
                    'saving'           => '保存中…',
                    'cancel'           => '取消',
                ],
                'mass-action' => [
                    'assign-tags'              => '分配标签',
                    'modal-title'              => '分配标签',
                    'modal-subtitle'           => '为已选择的 :count 个资产添加标签。现有标签将被保留。',
                    'modal-subtitle-recursive' => '为已选择的资产以及所选文件夹内的所有资产递归添加标签。现有标签将被保留。',
                    'tags-label'               => '标签',
                    'tags-placeholder'         => '搜索标签，或输入以创建…',
                    'add-tag'                  => '按回车键添加',
                    'assign'                   => '分配',
                    'assigning'                => '分配中…',
                    'cancel'                   => '取消',
                    'assign-success'           => '标签已分配给 :count 个资产。',
                    'assign-failed'            => '标签分配失败。',
                    'no-assets'                => '未选择任何资产。',
                    'no-items'                 => '请至少选择一个资产或文件夹。',
                    'no-tags'                  => '请选择或添加至少一个标签。',
                ],
            ],
        ],
        'catalog' => [
            'attributes' => [
                'type' => [
                    'asset' => '资产',
                ],
            ],
            'category-fields' => [
                'type' => [
                    'asset' => '资产',
                ],
            ],
        ],
        'acl' => [
            'menu'             => '资产管理',
            'asset'            => '资产',
            'property'         => '属性',
            'comment'          => '评论',
            'linked_resources' => '关联资源',
            'meta_data'        => '嵌入式元数据信息',
            'directory'        => '目录',
            'tag'              => '标签',
            'create'           => '创建',
            'edit'             => '编辑',
            'update'           => '更新',
            'delete'           => 'Delete',
            'list'             => '列表',
            'view'             => '查看',
            'upload'           => '上传',
            're_upload'        => '重新上传',
            'mass_update'      => '批量更新',
            'mass_delete'      => '批量删除',
            'download'         => '下载',
            'custom_download'  => '自定义下载',
            'rename'           => '重命名',
            'move'             => '移动',
            'copy'             => '复制',
            'copy-structure'   => '复制目录结构',
            'download-zip'     => '下载 Zip',
            'asset-assign'     => '分配资产',
            'share'            => '共享',
            'assets'           => '资产',
            'shares'           => '共享链接',
            'revoke'           => '撤销',
            'tags'             => '标签',
            'configuration'    => '配置',
        ],
        'permissions' => [
            'title'                 => 'DAM目录权限',
            'tab-subtitle'          => '此角色可以访问的目录。',
            'no-actions'            => '仅查看。请打开您有权限访问的子文件夹。',
            'unauthorized'          => '您没有权限管理DAM目录权限。',
            'all-directories'       => '所有目录 — 授予现在和将来对所有目录的访问权限',
            'inherit-children'      => '继承子目录 — 自动访问已授权目录内的所有子目录',
            'loading'               => '正在加载目录…',
            'empty'                 => '未找到目录。',
            'error'                 => '加载目录失败，请重试。',
            'retry'                 => '重试',
            'grants-load-warning'   => '部分目录授权加载失败，请在保存前确认您的选择。',
            'inherit-children-note' => '保存时将自动包含子目录。',
        ],
        'validation' => [
            'asset' => [
                'required' => '字段 :attribute 是必需的。',
            ],
            'comment' => [
                'required' => '评论内容是必需的。',
            ],
            'property' => [
                'name' => [
                    'required' => '名称字段是必需的。',
                    'unique'   => '名称已被使用。',
                ],
                'language' => [
                    'not-found' => '所选语言未找到或当前已被禁用。',
                ],
                'sort-order' => [
                    'required' => '启用“添加到筛选”时，排序顺序为必填项。',
                ],
            ],
        ],
        'errors' => [
            '401' => '此操作未经授权。',
        ],
        'configuration' => [
            'save-btn'    => '保存',
            'title'       => '配置',
            'saved'       => '配置已成功保存。',
            'description' => '配置应用的数字资产管理 (DAM) 设置。',
            'general'     => [
                'title'            => '常规设置',
                'description'      => '这些设置控制 DAM 的整体浏览体验。',
                'explorer-enabled' => [
                    'label' => '启用资源管理器视图',
                    'hint'  => '使用资源管理器风格的界面来浏览和管理资产。',
                ],
                'bookmarks-enabled' => [
                    'label' => '显示书签面板',
                    'hint'  => '显示一个专用侧边栏，包含您收藏的文件夹和资产。',
                ],
            ],
            'directory' => [
                'title'       => '目录设置',
                'description' => '这些设置控制文件夹和资产在目录面板中的显示方式。',
                'show-tree'   => [
                    'label'       => '显示目录树',
                    'hint'        => '在左侧边栏中显示文件夹层级结构，以便更轻松地导航。',
                    'locked-hint' => '在禁用资源管理器视图时始终开启。',
                ],
                'tree-show-assets' => [
                    'label' => '在目录树中显示资产',
                    'hint'  => '在目录树中将资产与文件夹一同显示，而不仅仅显示文件夹。',
                ],
            ],
            'ai-tagging' => [
                'title'       => 'AI 标记',
                'description' => '使用您配置的 Magic AI 平台自动为上传的图片建议标签。',
                'enabled'     => [
                    'label' => '使用 AI 自动标记资产',
                    'hint'  => '在图片上传时，使用您配置的 Magic AI 平台自动建议标签。',
                ],
                'platform' => [
                    'label'                    => '平台',
                    'use-default'              => '使用默认平台',
                    'none-configured'          => '未配置 AI 平台',
                    'none-configured-hint'     => '请先添加支持视觉功能的 Magic AI 平台 — 参见设置 → Magic AI → 平台。',
                ],
                'max-tags' => [
                    'label' => '最大标签数',
                    'hint'  => '每张图片建议的最大标签数（1-20）。',
                ],
                'rate-limit' => [
                    'label' => 'AI标记速率限制',
                    'hint'  => '每分钟AI标记请求的最大数量（1-120）。降低此值以匹配您的AI提供商的速率限制。',
                ],
            ],
        ],
        'explorer' => [
            'not-found'         => '目录未找到。',
            'access-denied'     => '您无权访问此目录。',
            'empty'             => '未找到目录。',
            'action-completed'  => '操作已成功完成。',
            'foreign-selection' => [
                'notice' => '已在另一个标签页中选择 :count 个项目。',
                'action' => '在此处也选择它们',
            ],
            'bookmarks' => [
                'title'     => '书签',
                'drag-hint' => '将文件夹拖拽至此处以添加书签',
                'max'       => '书签数量已达上限 20 个。',
                'remove'    => '移除书签',
                'empty'     => '暂无书签。',
            ],
            'tab' => [
                'new'   => '新建标签页',
                'close' => '关闭标签页',
            ],
            'sections' => [
                'folders' => '文件夹',
                'files'   => '文件',
                'folder'  => '文件夹',
            ],
            'clipboard' => [
                'ready'   => '准备粘贴',
                'dismiss' => '清除',
            ],
            'context' => [
                'open'                 => '打开',
                'open-new-tab'         => '在新标签页中打开',
                'bookmark'             => '收藏目录',
                'folder-upload'        => '上传文件夹',
                'copy'                 => '复制',
                'paste'                => '粘贴',
                'copy-progress'        => '复制已加入队列。',
                'copy-done'            => '已复制。',
                'paste-file-started'   => '正在粘贴文件...',
                'paste-folder-started' => '正在粘贴文件夹...',
                'paste-done'           => '粘贴成功。',
                'move-asset-started'   => '正在移动文件...',
                'move-folder-started'  => '正在移动文件夹...',
                'move-done'            => '文件夹已成功移动。',
                'move-asset-done'      => '文件已成功移动。',
                'delete-done'          => '目录已成功删除。',
            ],
            'folder' => [
                'deleted' => '此文件夹已被删除 — 已返回根目录。',
            ],
            'search' => [
                'placeholder' => '搜索文件和文件夹…',
            ],
            'list' => [
                'header' => [
                    'name'     => '名称',
                    'type'     => '类型',
                    'size'     => '大小',
                    'modified' => '修改时间',
                    'actions'  => '操作',
                ],
                'items-count' => ':count 个项目',
            ],
            'dialog' => [
                'rename-asset' => [
                    'title'       => '重命名文件',
                    'placeholder' => '新名称',
                ],
                'save'   => '保存',
                'cancel' => '取消',
            ],
            'view' => [
                'grid' => '网格视图',
                'list' => '列表视图',
            ],
            'mass-actions' => [
                'select-count'   => '已选择 :count 项',
                'select-action'  => '选择操作',
                'delete'         => '删除',
                'confirm'        => '删除 :count 个项目？文件夹将被递归删除且无法撤销。',
                'deleting'       => '正在删除 :count 个项目…',
                'deleted-assets' => '已删除 :count 个资产。',
                'deleted-dirs'   => ':count 个文件夹已加入删除队列。',
                'move-to'        => '移动到…',
                'copy-to'        => '复制到…',
                'moving'         => '正在移动…',
                'copying'        => '正在复制…',
                'moved-assets'   => ':count 个文件已移动。',
                'copied-assets'  => ':count 个文件已复制。',
                'moved-dirs'     => ':count 个文件夹已移动。',
                'copied-dirs'    => ':count 个文件夹已复制。',
                'pick-dest'      => '选择目标文件夹',
                'select-here'    => '在此选择',
                'new-folder'     => '新建文件夹',
                'folder-name'    => '文件夹名称',
                'create-error'   => '无法创建文件夹。',
                'move-done'      => ':count 个文件已移动。',
                'copy-done'      => ':count 个文件已复制。',
                'still-running'  => '操作仍在后台运行',
                'progress'       => '进度',
                'total-files'    => ':count 个文件',
            ],
            'toolbar' => [
                'toggle-tree'    => '切换目录树',
                'toggle-sidebar' => '切换侧边栏',
            ],
            'upload' => [
                'queued'              => '排队中',
                'uploading'           => '上传中',
                'creating'            => '正在创建文件夹',
                'done'                => '完成',
                'failed'              => '失败',
                'interrupted'         => '已中断 — 重新添加以重试',
                'resuming'            => '正在恢复…',
                'overall'             => '已上传 :done / :total',
                'too-large-to-resume' => '批次过大，刷新后无法自动恢复',
            ],
        ],
    ],
    'share' => [
        'public' => [
            'app-name'              => 'DAM',
            'download'              => '下载',
            'back-to-gallery'       => '返回图库',
            'back'                  => '返回',
            'file-name'             => '文件名',
            'file-type'             => '文件类型',
            'file-size'             => '文件大小',
            'video-not-supported'   => '您的浏览器不支持 video 标签。',
            'audio-not-supported'   => '您的浏览器不支持 audio 标签。',
            'preview-not-available' => '此文件无法预览，请使用上方的下载按钮。',
            'files-count'           => ':count 个文件',
            'empty-directory'       => '此共享目录暂无文件。',
            'expires-on'            => '此链接过期时间',
            'powered-by-dam'        => '由 Unopim DAM 提供支持',
            'expired-title'         => '链接已过期',
            'expired-message'       => '此共享链接已过期，无法访问。请向所有者申请新链接。',
            'revoked-title'         => '链接已撤销',
            'revoked-message'       => '此共享链接已被所有者撤销，无法访问。',
            'not-found-title'       => '链接不存在',
            'not-found-message'     => '您打开的共享链接不存在，可能已被误输入或删除。',
            'download-zip'          => '下载全部为 ZIP',
            'per-page'              => '每页',
        ],
    ],
    'data-transfer' => [
        'bundle' => [
            'invalid-archive'            => 'The uploaded archive could not be opened.',
            'invalid-zip'                => 'The uploaded archive is damaged and cannot be read.',
            'no-data-file'               => 'The archive contains no CSV or Excel file to import.',
            'zip-too-many-entries'       => 'The archive contains :count entries, which is above the limit of :limit.',
            'zip-contents-too-large'     => 'The archive expands to more than :limit MB.',
            'zip-compression-suspicious' => 'The entry ":entry" is compressed far beyond a normal ratio and was rejected.',
            'asset-not-found'            => 'No asset was found at path ":path".',
            'asset-tree-misnamed'        => '压缩包中包含名为“:name”的文件夹而不是“assets”，因此无法导入其中的任何资源。请将其重命名为“assets”，并保持其下的文件夹与数据文件中的路径一致，例如“assets/Root/Seed/Tech Sheets/Cotton/”。macOS 会将文件夹名称中输入的“/”保存为“:”，这是常见原因。',
        ],
    ],
];
