<?php

return [
    'admin' => [
        'components' => [
            'layouts' => [
                'sidebar' => [
                    'dam'           => 'Ressursstyring',
                    'assets'        => 'Ressurser',
                    'shares'        => 'Delte lenker',
                    'tags'          => 'Tagger',
                    'configuration' => 'Konfigurasjon',
                ],
            ],
            'modal' => [
                'confirm' => [
                    'message' => 'Hvis du sletter denne mappen, slettes også alle undermapper i den. Denne handlingen er permanent og kan ikke angres.',
                ],
            ],
            'asset' => [
                'field' => [
                    'add-asset'     => 'Legg til ressurs',
                    'assign-assets' => 'Tildel ressurser',
                    'preview'       => 'Forhåndsvisning',
                    'remove'        => 'Fjern',
                    'download'      => 'Last ned',
                ],
            ],
        ],
        'dam' => [
            'file' => [
                'deleted'   => 'Fil slettet',
                'not-found' => 'Fil ikke funnet',
            ],
            'image-edit' => [
                'cannot-decode'  => 'Kan ikke dekode bilde',
                'preview-failed' => 'Forhåndsvisning mislyktes',
            ],
            'index' => [
                'title'    => 'Digital ressursstyring',
                'datagrid' => [
                    'file-name'      => 'Filnavn',
                    'tags'           => 'Tagger',
                    'property-name'  => 'Egenskapsnavn',
                    'property-value' => 'Egenskapsverdi',
                    'created-at'     => 'Opprettet',
                    'updated-at'     => 'Oppdatert',
                    'extension'      => 'Filtype',
                    'path'           => 'Sti',
                ],
                'mass-action' => [
                    'deleting'   => 'Sletter :count ressurser…',
                    'processing' => 'Behandler :count ressurser…',
                ],
                'move' => [
                    'directory' => 'Flytter mappe ":name"…',
                    'asset'     => 'Flytter ressurs ":name"…',
                ],
                'directory' => [
                    'title'  => 'Mappe',
                    'create' => [
                        'title'     => 'Opprett mappe',
                        'name'      => 'Navn',
                        'file_name' => 'Filnavn',
                        'save-btn'  => 'Lagre mappe',
                    ],
                    'rename' => [
                        'title' => 'Gi mappen nytt navn',
                    ],
                    'search' => [
                        'placeholder'     => 'Søk i mapper...',
                        'no-matches'      => 'Ingen treff',
                        'error'           => 'Søket mislyktes — prøv igjen',
                        'not-found-flash' => 'Mappen er ikke lenger tilgjengelig',
                        'count'           => ':shown av :total treff',
                        'count-total'     => ':total treff',
                    ],
                    'asset' => [
                        'rename' => [
                            'title'    => 'Gi ressursen nytt navn',
                            'save-btn' => 'Lagre ressurs',
                        ],
                    ],
                    'actions' => [
                        'delete'                    => 'Slett',
                        'edit'                      => 'Rediger',
                        'rename'                    => 'Gi nytt navn',
                        'copy'                      => 'Kopier',
                        'cut'                       => 'Klipp',
                        'download'                  => 'Last ned',
                        'download-zip'              => 'Last ned zip',
                        'paste'                     => 'Lim inn',
                        'add-directory'             => 'Legg til mappe',
                        'upload-files'              => 'Last opp filer',
                        'upload-files-and-folders'  => 'Last opp filer og mapper',
                        'upload-folder'             => 'Last opp mappe',
                        'upload-drop-hint'          => 'Dra filer eller mapper hit',
                        'upload-or'                 => 'eller bla:',
                        'copy-directory-structured' => 'Kopier mappestruktur',
                        'get'                       => 'Get',
                        'get-by-id'                 => 'Hent etter ID',
                        'comment'                   => 'Kommentar',
                        'share'                     => 'Del mappe',
                    ],
                    'linked-resources'                          => 'Tilknyttede ressurser',
                    'not-found'                                 => 'Ingen mappe funnet',
                    'created-success'                           => 'Mappe opprettet',
                    'creation-failed'                           => 'Oppretting av mappe mislyktes',
                    'updated-success'                           => 'Mappe oppdatert',
                    'update-failed'                             => 'Oppdatering av mappe mislyktes',
                    'moved-success'                             => 'Mappe flyttet',
                    'cannot-move'                               => 'Kan ikke flytte mappen',
                    'cannot-copy'                               => 'Kan ikke kopiere en mappe inn i seg selv eller en av sine egne undermapper.',
                    'fetch-all-success'                         => 'Mapper hentet',
                    'can-not-deleted'                           => 'Mappen kan ikke slettes fordi den er rotmappen.',
                    'deleting-in-progress'                      => 'Sletting av mappe pågår',
                    'can-not-copy'                              => 'Mappen kan ikke kopieres fordi den er rotmappen.',
                    'coping-in-progress'                        => 'Kopiering av mappestruktur pågår.',
                    'asset-not-found'                           => 'Ingen ressurs funnet',
                    'asset-renamed-success'                     => 'Ressursen fikk nytt navn',
                    'asset-moved-success'                       => 'Ressursen er flyttet',
                    'asset-name-already-exist'                  => 'Det nye navnet finnes allerede på en annen ressurs med navnet :asset_name',
                    'asset-name-conflict-in-the-same-directory' => 'Ressursnavnet er i konflikt med en eksisterende fil i samme mappe.',
                    'old-file-not-found'                        => 'Filen som ble forespurt på stien :old_path ble ikke funnet.',
                    'image-name-is-the-same'                    => 'Dette navnet finnes allerede. Vennligst oppgi et annet.',
                    'not-writable'                              => 'Du har ikke tillatelse til å :actionType en :type på denne plasseringen ":path".',
                    'empty-directory'                           => 'Denne mappen er tom.',
                    'failed-download-directory'                 => 'Kunne ikke opprette zip-filen.',
                    'not-allowed'                               => 'Opplasting av skriptfiler er ikke tillatt.',
                    'copy-success'                              => 'Mappen er kopiert.',
                    'name-unique'                               => 'Mappenavnet må være unikt i samme overordnede mappe.',
                    'folder-upload-success'                     => 'Mappe lastet opp.',
                    'folder-upload-partial'                     => 'Mappe lastet opp med :count fil(er) hoppet over på grunn av feil.',
                    'load-more'                                 => 'Last inn flere',
                ],
                'description'                    => 'Verktøyet hjelper deg med å organisere, lagre og administrere alle medieressursene dine på ett sted',
                'root'                           => 'Rot',
                'upload'                         => 'Last opp',
                'new'                            => 'Ny',
                'uploading'                      => 'Laster opp...',
                'cancel'                         => 'Avbryt',
                'upload-cancelled'               => 'Opplasting avbrutt.',
                'folder-upload-https-required'   => 'Mappeopplasting krever en sikker tilkobling (HTTPS).',
                'folder-upload-no-https-warning' => 'Tomme mapper opprettes ikke uten en sikker tilkobling (HTTPS). Filer lastes fortsatt opp.',
                'folder-upload-no-files'         => 'Den/de valgte mappen(e) inneholder ingen filer. Tomme undermapper kan ikke oppdages uten en sikker tilkobling (HTTPS).',
                'upload-in-progress-title'       => 'Opplasting pågår',
                'upload-in-progress-message'     => 'Å forlate denne siden vil avbryte den aktive opplastingen. Er du sikker på at du vil forlate?',
                'upload-leave-page'              => 'Forlat',
                'upload-stay-page'               => 'Bli',
                'drop-zone-hint'                 => 'Dra filer eller mapper for å laste opp',
                'select-all'                     => 'Velg alle',
                'select-directory-to-upload'     => 'Velg en mappe å laste opp til.',
                'upload-session'                 => [
                    'uploading'            => 'Laster opp :total fil(er)…',
                    'paused'               => 'Satt på pause — :done av :total lastet opp',
                    'interrupted'          => ':done av :total lastet opp · :interrupted avbrutt',
                    'failed'               => ':done lastet opp, :failed mislyktes',
                    'completed'            => ':done av :total lastet opp',
                    'create-folder-failed' => 'Kunne ikke opprette mappen',
                ],
                'drop-zone-no-permission' => 'Du har ikke tillatelse til å laste opp her.',
                'upload-complete'         => 'Opplasting fullført',
                'item-upload-complete'    => 'Opplasting fullført',
                'copy'                    => [
                    'directory' => 'Kopierer mappe ":name"…',
                ],
                'delete' => [
                    'directory' => 'Sletter mappe ":name"…',
                ],
            ],
            'asset' => [
                'properties' => [
                    'index' => [
                        'title'      => 'Ressursegenskaper',
                        'create-btn' => 'Opprett egenskap',
                        'datagrid'   => [
                            'name'     => 'Navn',
                            'type'     => 'Type',
                            'language' => 'Språk',
                            'value'    => 'Verdi',
                            'edit'     => 'Rediger',
                            'delete'   => 'Slett',
                        ],
                        'create' => [
                            'title'                  => 'Opprett egenskap',
                            'name'                   => 'Navn',
                            'type'                   => 'Type',
                            'language'               => 'Språk',
                            'value'                  => 'Verdi',
                            'add-to-filter'          => 'Legg til i filter',
                            'add-to-filter-help'     => 'Når aktivert, vises denne egenskapen som et filter i sidefeltet for aktivalisten.',
                            'sort-order'             => 'Sorteringsrekkefølge',
                            'sort-order-help'        => 'Lavere tall vises først i filterets sidefelt. Egenskaper med samme tall sorteres alfabetisk.',
                            'sort-order-placeholder' => 'f.eks. 10',
                            'save-btn'               => 'Lagre',
                        ],
                        'edit' => [
                            'title' => 'Rediger egenskap',
                        ],
                        'delete-success' => 'Ressursegenskap slettet',
                        'delete-failed'  => 'Sletting av aktivaegenskap mislyktes',
                        'create-success' => 'Ressursegenskap opprettet',
                        'update-success' => 'Ressursegenskap oppdatert',
                        'update-failure' => 'Oppdatering av aktivaegenskap mislyktes',
                        'not-found'      => 'Egenskap ikke funnet',
                        'found-success'  => 'Egenskap funnet',
                    ],
                ],
                'comments' => [
                    'create' => [
                        'create-success' => 'Kommentaren er lagt til',
                        'create-failure' => 'Kunne ikke opprette kommentaren',
                    ],
                    'post-comment'    => 'Send kommentar',
                    'post-reply'      => 'Send svar',
                    'reply'           => 'Svar',
                    'edit-btn'        => 'Rediger',
                    'delete-btn'      => 'Slett',
                    'save-btn'        => 'Lagre',
                    'cancel-btn'      => 'Avbryt',
                    'add-reply'       => 'Legg til svar',
                    'add-comment'     => 'Legg til kommentar',
                    'no-comments'     => 'Ingen kommentarer ennå',
                    'not-found'       => 'Kommentarer ikke funnet',
                    'updated-success' => 'Kommentar oppdatert',
                    'update-failed'   => 'Kunne ikke oppdatere kommentaren',
                    'delete-success'  => 'Ressurskommentar slettet',
                    'delete-failed'   => 'Kunne ikke slette ressurskommentaren',
                    'deleted-user'    => 'Slettet bruker',
                ],
                'edit' => [
                    'title'                 => 'Rediger ressurs',
                    'back'                  => 'Tilbake',
                    'previous'              => 'Forrige',
                    'next'                  => 'Neste',
                    'name'                  => 'Navn',
                    'value'                 => 'Verdi',
                    'save-btn'              => 'Lagre',
                    'file-name'             => 'Filnavn',
                    'file-info'             => 'Filinformasjon',
                    'type'                  => 'Type',
                    'size'                  => 'Størrelse',
                    'dimensions'            => 'Dimensjoner',
                    'path'                  => 'Sti',
                    'created-at'            => 'Opprettet',
                    'updated-at'            => 'Oppdatert',
                    'embedded_meta_info'    => 'Innebygd metainformasjon',
                    'no-metadata-available' => 'Ingen metadata tilgjengelig',
                    'tags'                  => 'Tagger',
                    'select-tags'           => 'Velg eller opprett en tagg',
                    'tag-attach-success'    => 'Etikett lagt til.',
                    'tag-detach-success'    => 'Etikett fjernet fra ressursen.',
                    'directory-path'        => 'Mappebane',
                    'no-directory'          => 'Ingen mappe tildelt',
                    'details'               => 'Detaljer',
                    'breadcrumb-assets'     => 'Ressurser',
                    'tab'                   => [
                        'preview'          => 'Forhåndsvisning',
                        'properties'       => 'Egenskaper',
                        'comments'         => 'Kommentarer',
                        'linked_resources' => 'Tilknyttede ressurser',
                        'history'          => 'Historikk',
                    ],
                    'button' => [
                        'download'            => 'Last ned',
                        'custom_download'     => 'Tilpasset nedlasting',
                        'rename'              => 'Gi nytt navn',
                        're_upload'           => 'Last opp på nytt',
                        're_uploading'        => 'Laster opp på nytt...',
                        'cancel'              => 'Avbryt',
                        're-upload-cancelled' => 'Gjenopplasting avbrutt.',
                        'delete'              => 'Slett',
                        'share'               => 'Del',
                    ],
                    'share' => [
                        'title'      => 'Del ressurs',
                        'link'       => 'Delbar lenke',
                        'copy'       => 'Kopier',
                        'copied'     => 'Lenke kopiert til utklippstavlen',
                        'expiry'     => 'Lenken utløper etter',
                        'expiry-1d'  => '1 dag',
                        'expiry-7d'  => '7 dager',
                        'expiry-30d' => '30 dager',
                        'expires-on' => 'Utløper den',
                        'failed'     => 'Kunne ikke generere delingslenke.',
                    ],
                    'preview-modal' => [
                        'not-available' => 'Forhåndsvisning er ikke tilgjengelig for denne filtypen.',
                        'download-file' => 'Last ned fil',
                        'download-zip'  => 'Last ned som ZIP',
                        'close'         => 'Lukk forhåndsvisning',
                        'mime'          => 'MIME',
                        'card'          => [
                            'click-for-details' => 'Klikk for fullstendige detaljer',
                            'preview'           => 'Forhåndsvisning',
                            'edit-image'        => 'Rediger bilde',
                        ],
                        'image-viewer' => [
                            'rotate-left'   => 'Roter til venstre (L)',
                            'rotate-right'  => 'Roter til høyre (R)',
                            'zoom-out'      => 'Zoom ut (-)',
                            'zoom-in'       => 'Zoom inn (+)',
                            'fit-to-screen' => 'Tilpass til skjerm',
                            'actual-size'   => 'Faktisk størrelse',
                            'reset-all'     => 'Tilbakestill alt (0)',
                        ],
                        'video-player' => [
                            'back-10s'           => '10 s bakover',
                            'forward-10s'        => '10 s fremover',
                            '10s'                => '10 s',
                            'speed'              => 'Hastighet',
                            'loop'               => 'Gjenta (L)',
                            'mute'               => 'Demp (M)',
                            'picture-in-picture' => 'Bilde i bilde',
                            'fullscreen'         => 'Fullskjerm (F)',
                            'more-actions'       => 'Flere handlinger',
                            'copy-link'          => 'Kopier lenke',
                            'link-copied'        => 'Kopiert!',
                            'open-in-new-tab'    => 'Åpne i ny fane',
                        ],
                    ],
                    'custom-download' => [
                        'title'              => 'Tilpasset nedlasting',
                        'format'             => 'Format',
                        'width'              => 'Bredde (px)',
                        'width-placeholder'  => '200',
                        'height'             => 'Høyde (px)',
                        'height-placeholder' => '200',
                        'download-btn'       => 'Last ned',
                        'extension-types'    => [
                            'jpg'      => 'JPG',
                            'png'      => 'PNG',
                            'jpeg'     => 'JPEG',
                            'webp'     => 'WEBP',
                            'original' => 'Original',
                        ],
                    ],
                    'tag-already-exists'        => 'Taggen finnes allerede',
                    'image-source-not-readable' => 'Bildekilden kan ikke leses',
                    'failed-to-read'            => 'Kunne ikke lese bildets metadata :exception',
                    'file-re-upload-success'    => 'Filene ble lastet opp på nytt.',
                    'image-processing-failed'   => 'Bildebehandling mislyktes: :message',
                    'image-editor'              => [
                        'tools'                    => 'Verktøy',
                        'close'                    => 'Lukk redigerer',
                        'crop'                     => 'Beskjær og endre størrelse',
                        'crop-sub'                 => 'Trimme eller skalere bilde',
                        'adjust'                   => 'Lysstyrke og kontrast',
                        'adjust-sub'               => 'Juster lys og tone',
                        'rotate'                   => 'Roter og speil',
                        'rotate-sub'               => 'Transformer orientering',
                        'selection'                => 'Utvalg',
                        'drag-handles'             => 'drahåndtak',
                        'scale-after-crop'         => 'Skaler etter beskjæring',
                        'optional'                 => 'valgfritt',
                        'width-px'                 => 'Bredde (px)',
                        'height-px'                => 'Høyde (px)',
                        'blank-keep-dims'          => 'La stå tomt for å beholde beskårne dimensjoner.',
                        'auto'                     => 'auto',
                        'adjustments'              => 'Justeringer',
                        'brightness'               => 'Lysstyrke',
                        'contrast'                 => 'Kontrast',
                        'rotation'                 => 'Rotasjon',
                        'flip'                     => 'Speil',
                        'horizontal'               => 'Horisontal',
                        'vertical'                 => 'Vertikal',
                        'platform'                 => 'Plattform',
                        'platform-loading'         => 'Laster…',
                        'no-platforms'             => 'Ingen plattformer',
                        'color-none'               => 'Ingen',
                        'color-white'              => 'Hvit',
                        'color-silver'             => 'Sølv',
                        'color-light-gray'         => 'Lysegrå',
                        'color-gray'               => 'Grå',
                        'color-slate'              => 'Skifergrå',
                        'color-dark-gray'          => 'Mørkegrå',
                        'color-charcoal'           => 'Kullgrå',
                        'color-black'              => 'Sort',
                        'color-light-red'          => 'Lysrød',
                        'color-red'                => 'Rød',
                        'color-light-yellow'       => 'Lysgul',
                        'color-yellow'             => 'Gul',
                        'color-light-green'        => 'Lysgrønn',
                        'color-green'              => 'Grønn',
                        'color-light-cyan'         => 'Lyscyan',
                        'color-cyan'               => 'Cyan',
                        'color-light-blue'         => 'Lysblå',
                        'color-blue'               => 'Blå',
                        'color-light-purple'       => 'Lyslilla',
                        'color-purple'             => 'Lilla',
                        'color-light-pink'         => 'Lysrosa',
                        'color-pink'               => 'Rosa',
                        'color-light-rose'         => 'Lys rose',
                        'color-rose'               => 'Rose',
                        'model'                    => 'Modell',
                        'no-models'                => 'Ingen modeller',
                        'prompt'                   => 'Melding',
                        'apply'                    => 'Bruk',
                        'applying'                 => 'Bruker…',
                        'error-platforms'          => 'Kunne ikke laste AI-plattformer.',
                        'error-select-color'       => 'Velg en bakgrunnsfarge.',
                        'error-operation'          => 'Operasjonen mislyktes. Vennligst prøv igjen.',
                        'success-updated'          => 'Bilde oppdatert.',
                        'success-adjusted'         => 'Bilde justert.',
                        'success-transformed'      => 'Bilde transformert.',
                        'error-no-ai-image'        => 'AI returnerte intet bilde.',
                        'error-provide-dims'       => 'Angi minst en beskjæringsregion eller utdatadimensjoner.',
                        'edit-bg'                  => 'Rediger bakgrunn',
                        'edit-bg-sub'              => 'Farge, opplasting eller ledetekst',
                        'bg-tab-color'             => 'Farge',
                        'bg-tab-upload'            => 'Last opp',
                        'bg-tab-ai'                => 'Ledetekst',
                        'bg-color-label'           => 'Bakgrunnsfarge',
                        'bg-color-mode-normal'     => 'Normal',
                        'bg-color-mode-ai'         => 'AI',
                        'bg-custom-color'          => 'Tilpasset',
                        'bg-upload-hint'           => 'Klikk for å laste opp et bakgrunnsbilde',
                        'bg-upload-change'         => 'Klikk for å endre',
                        'bg-ai-prompt-placeholder' => 'Beskriv bakgrunnen du ønsker…',
                        'sharpen'                  => 'Skarphet',
                        'blur'                     => 'Uskarpt',
                        'filters'                  => 'Filtre',
                        'filters-sub'              => 'Gråtoner og invertering',
                        'greyscale'                => 'Gråtoner',
                        'invert'                   => 'Inverter',
                        'error-no-filter'          => 'Velg minst ett filter å bruke.',
                        'error-provider-no-images' => 'Denne AI-leverandøren støtter ikke bildegenerering.',
                        'error-no-image-platform'  => 'Ingen AI-plattformer med støtte for bildegenerering funnet. Konfigurer en OpenAI-, Gemini- eller xAI-plattform.',
                    ],
                ],
                'linked-resources' => [
                    'index' => [
                        'datagrid' => [
                            'product'       => 'Produkt',
                            'category'      => 'Kategori',
                            'product-sku'   => 'Produkt-Sku: ',
                            'category code' => 'Kategorikode: ',
                            'resource-type' => 'Ressurstype',
                            'resource'      => 'Ressurs',
                            'resource-view' => 'Ressursvisning',
                        ],
                    ],
                    'found-success' => 'Ressurs funnet',
                    'not-found'     => 'Ressurs ikke funnet',
                ],
                'tags' => [
                    'create' => [
                        'create-success' => 'Taggene er lagt til',
                    ],
                    'found-success'  => 'Tagg funnet',
                    'not-found'      => 'Tagger ikke funnet',
                    'delete-success' => 'Ressurstagger fjernet',
                ],
                'delete-success'                          => 'Ressurs slettet',
                'delete-failed-due-to-attached-resources' => 'Ressursen er i bruk. Fjern tilknytningen før du sletter',
                'datagrid'                                => [
                    'mass-delete-success'                 => 'Masseslettet.',
                    'mass-update-success'                 => 'Masseoppдatering vellykket.',
                    'files-upload-success'                => 'Filer lastet opp.',
                    'file-upload-success'                 => 'Fil lastet opp.',
                    'not-found'                           => 'Fil ikke funnet',
                    'not-found-to-show'                   => 'Filen ble ikke funnet for visning',
                    'not-found-to-download'               => 'Filen ble ikke funnet for nedlasting',
                    'edit-success'                        => 'Fil lastet opp',
                    'no-file-name'                        => 'ingen filnavn',
                    'show-success'                        => 'Fil funnet',
                    'update-success'                      => 'Fil oppdatert',
                    'not-found-to-update'                 => 'Filen finnes ikke',
                    'not-found-to-destroy'                => 'Filen finnes ikke',
                    'files-upload-failed'                 => 'Kunne ikke laste opp filene.',
                    'file-upload-failed'                  => 'Kunne ikke laste opp filen',
                    'invalid-file'                        => 'Ugyldig fil oppgitt',
                    'invalid-file-format-or-not-provided' => 'Ingen filer oppgitt eller ugyldig format.',
                    'file-process-failed'                 => 'Noen filer kunne ikke behandles',
                    'file-forbidden-type'                 => 'Filen har en forbudt type eller filtype.',
                    'file-too-large'                      => 'Filen er for stor. Maksimal tillatt størrelse er :size.',
                    'storage-upload-failed'               => 'Opplasting av ":name" til lagring mislyktes. Kontroller disklegitimasjon og tillatelser.',
                    'not-found-or-no-file'                => 'Asset ikke funnet eller filen eksisterer ikke.',
                    'download-link-ready'                 => 'Asset funnet. Du kan laste ned filen fra den oppgitte lenken.',
                ],
            ],
            'share' => [
                'target-not-found' => 'Ressursen eller mappen som deles ble ikke funnet.',
                'not-found'        => 'Delingslenke ikke funnet.',
                'revoked'          => 'Delingslenke tilbakekalt.',
                'deleted'          => 'Delingslenke slettet.',
                'already-revoked'  => 'Denne delingslenken er allerede tilbakekalt.',
                'reauthorized'     => 'Delingskoblingen er autorisert på nytt — den opprinnelige URL-en er aktiv igjen.',
                'not-revoked'      => 'Denne delingslenken er ikke tilbakekalt.',
                'updated'          => 'Delingslenke oppdatert.',
                'update-failed'    => 'Kunne ikke oppdatere delingslenken.',
                'delete-failed'    => 'Kunne ikke slette delingslenke.',
                'modal'            => [
                    'title-asset'        => 'Del ressurs',
                    'title-directory'    => 'Del mappe',
                    'edit-title'         => 'Rediger delingslenke',
                    'edit-name'          => 'Endre navn på deling',
                    'unnamed-share'      => '(intet egendefinert navn)',
                    'name-label'         => 'Egendefinert navn',
                    'name-hint'          => 'La være tom for å beholde det opprinnelige fil- / mappenavnet som etikett.',
                    'save'               => 'Lagre endringer',
                    'saving'             => 'Lagrer…',
                    'cancel'             => 'Avbryt',
                    'create-new'         => 'Opprett en ny delingslenke',
                    'expiry'             => 'Lenken utløper etter',
                    'expiry-1d'          => '1 dag',
                    'expiry-7d'          => '7 dager',
                    'expiry-30d'         => '30 dager',
                    'expiry-365d'        => '1 år',
                    'no-expiry'          => 'Ingen utløpsdato',
                    'create'             => 'Generer lenke',
                    'creating'           => 'Genererer…',
                    'active-links'       => 'Aktive lenker',
                    'loading'            => 'Laster…',
                    'no-active-links'    => 'Ingen aktive delingslenker for dette elementet ennå.',
                    'copy'               => 'Kopier',
                    'copied'             => 'Lenke kopiert til utklippstavlen',
                    'revoke'             => 'Tilbakekall',
                    'delete'             => 'Delete',
                    'revoked'            => 'Delingslenke tilbakekalt',
                    'revoke-failed'      => 'Kunne ikke tilbakekalle delingslenken.',
                    'expires-on'         => 'Utløper den',
                    'never-expires'      => 'Utløper aldri',
                    'views'              => 'Visninger',
                    'downloads'          => 'Nedlastinger',
                    'created'            => 'Delingslenke opprettet.',
                    'create-failed'      => 'Kunne ikke opprette delingslenke.',
                    'load-failed'        => 'Kunne ikke laste eksisterende delingslenker.',
                    'advanced'           => 'Avansert',
                    'revoked-status'     => 'Kobling tilbakekalt',
                    'reauthorize'        => 'Autoriser på nytt',
                    'reauthorizing'      => 'Autoriserer på nytt…',
                    'reauthorize-failed' => 'Kunne ikke autorisere delingslenken på nytt.',
                    'revoked-notice'     => 'Denne koblingen er tilbakekalt. Autoriser på nytt for å gjøre den opprinnelige URL-en aktiv igjen.',
                ],
                'index' => [
                    'title'       => 'Delte lenker',
                    'description' => 'Alle aktive og historiske delingslenker for ressurser og mapper i DAM.',
                ],
                'datagrid' => [
                    'type'           => 'Type',
                    'asset'          => 'Ressurs',
                    'directory'      => 'Mappe',
                    'target'         => 'Mål',
                    'custom-name'    => 'Egendefinert navn',
                    'created-by'     => 'Opprettet av',
                    'expires-at'     => 'Utløper den',
                    'never'          => 'Aldri',
                    'status'         => 'Status',
                    'status-active'  => 'Aktiv',
                    'status-expired' => 'Utløpt',
                    'status-revoked' => 'Tilbakekalt',
                    'views'          => 'Visninger',
                    'downloads'      => 'Nedlastinger',
                    'created-at'     => 'Opprettet den',
                    'edit'           => 'Rediger',
                    'copy-link'      => 'Kopier lenke',
                    'revoke'         => 'Tilbakekall',
                    'delete'         => 'Slett',
                ],
            ],
            'upload' => [
                'pause'     => 'Pause',
                'resume'    => 'Gjenoppta',
                'retry'     => 'Prøv igjen',
                'cancel'    => 'Avbryt',
                'paused'    => 'Opplasting satt på pause. Eksisterende ressurser er upåvirket — gjenoppta når som helst.',
                'resumed'   => 'Opplasting gjenopptatt.',
                'cancelled' => 'Opplasting avbrutt. Allerede opplastede ressurser ble beholdt.',
                'retried'   => 'Prøver mislykkede opplastinger på nytt i bakgrunnen.',
            ],
            'tag' => [
                'not-found'           => 'Fant ikke taggen.',
                'create-success'      => 'Taggen ble opprettet.',
                'update-success'      => 'Taggen ble oppdatert.',
                'delete-success'      => 'Taggen ble slettet.',
                'mass-delete-success' => 'Valgte tagger ble slettet.',
                'index'               => [
                    'title'       => 'Tagger',
                    'description' => 'Administrer taggene som brukes til å merke og organisere ressurser i DAM.',
                    'create'      => 'Opprett tagg',
                ],
                'datagrid' => [
                    'name'         => 'Navn',
                    'assets-count' => 'Ressurser',
                    'created-at'   => 'Opprettet',
                    'edit'         => 'Rediger',
                    'delete'       => 'Slett',
                ],
                'modal' => [
                    'create-title'     => 'Opprett tagg',
                    'edit-title'       => 'Rediger tagg',
                    'name-label'       => 'Taggnavn',
                    'name-placeholder' => 'f.eks. sommerkampanje',
                    'save'             => 'Lagre',
                    'saving'           => 'Lagrer…',
                    'cancel'           => 'Avbryt',
                ],
                'mass-action' => [
                    'assign-tags'              => 'Tildel tagger',
                    'modal-title'              => 'Tildel tagger',
                    'modal-subtitle'           => 'Legg til tagger på :count valgte ressurs(er). Eksisterende tagger beholdes.',
                    'modal-subtitle-recursive' => 'Legg til tagger på de valgte ressursene og alle ressurser i de valgte mappene, rekursivt. Eksisterende tagger beholdes.',
                    'tags-label'               => 'Tagger',
                    'tags-placeholder'         => 'Søk etter tagger eller skriv for å opprette…',
                    'add-tag'                  => 'Trykk Enter for å legge til',
                    'assign'                   => 'Tildel',
                    'assigning'                => 'Tildeler…',
                    'cancel'                   => 'Avbryt',
                    'assign-success'           => 'Tagger tildelt :count ressurs(er).',
                    'assign-failed'            => 'Kunne ikke tildele tagger.',
                    'no-assets'                => 'Ingen ressurser valgt.',
                    'no-items'                 => 'Velg minst én ressurs eller mappe.',
                    'no-tags'                  => 'Velg eller legg til minst én tagg.',
                ],
            ],
        ],
        'catalog' => [
            'attributes' => [
                'type' => [
                    'asset' => 'Ressurs',
                ],
            ],
            'category-fields' => [
                'type' => [
                    'asset' => 'Ressurs',
                ],
            ],
        ],
        'acl' => [
            'menu'             => 'Ressursstyring',
            'asset'            => 'Ressurs',
            'property'         => 'Egenskap',
            'comment'          => 'Kommentar',
            'linked_resources' => 'Tilknyttede ressurser',
            'meta_data'        => 'Innebygd metainformasjon',
            'directory'        => 'Mappe',
            'tag'              => 'Tagg',
            'create'           => 'Opprett',
            'edit'             => 'Rediger',
            'update'           => 'Oppdater',
            'delete'           => 'Delete',
            'list'             => 'Liste',
            'view'             => 'Vis',
            'upload'           => 'Last opp',
            're_upload'        => 'Last opp på nytt',
            'mass_update'      => 'Masseoppdatering',
            'mass_delete'      => 'Masseslett',
            'download'         => 'Last ned',
            'custom_download'  => 'Tilpasset nedlasting',
            'rename'           => 'Gi nytt navn',
            'move'             => 'Flytt',
            'copy'             => 'Kopier',
            'copy-structure'   => 'Kopier mappestruktur',
            'download-zip'     => 'Last ned zip',
            'asset-assign'     => 'Tildel ressurs',
            'share'            => 'Del',
            'assets'           => 'Ressurser',
            'shares'           => 'Delte lenker',
            'revoke'           => 'Tilbakekall',
            'tags'             => 'Tagger',
            'configuration'    => 'Konfigurasjon',
        ],
        'permissions' => [
            'title'                 => 'DAM-mappetillatelser',
            'tab-subtitle'          => 'Mapper denne rollen har tilgang til.',
            'no-actions'            => 'Kun visning. Åpne en undermappe du har tilgang til.',
            'unauthorized'          => 'Du har ikke tillatelse til å administrere DAM-mappetillatelser.',
            'all-directories'       => 'Alle mapper — gi tilgang til alle mapper nå og i fremtiden',
            'inherit-children'      => 'Arv undermapper — få automatisk tilgang til alle undermapper i de tildelte mappene',
            'loading'               => 'Laster kataloger…',
            'empty'                 => 'Ingen kataloger funnet.',
            'error'                 => 'Kunne ikke laste kataloger. Prøv igjen.',
            'retry'                 => 'Prøv igjen',
            'grants-load-warning'   => 'Noen katalogtillatelser kunne ikke lastes. Bekreft valgene dine før du lagrer.',
            'inherit-children-note' => 'Underkataloger inkluderes automatisk ved lagring.',
        ],
        'validation' => [
            'asset' => [
                'required' => 'Feltet :attribute er påkrevd.',
            ],
            'comment' => [
                'required' => 'Kommentarmeldingen er påkrevd.',
            ],
            'property' => [
                'name' => [
                    'required' => 'Navnefeltet er påkrevd.',
                    'unique'   => 'Navnet er allerede tatt.',
                ],
                'language' => [
                    'not-found' => 'Det valgte språket ble ikke funnet eller er for øyeblikket deaktivert.',
                ],
                'sort-order' => [
                    'required' => 'Feltet Sorteringsrekkefølge er påkrevd når Legg til i filter er aktivert.',
                ],
            ],
        ],
        'errors' => [
            '401' => 'Denne handlingen er ikke autorisert.',
        ],
        'configuration' => [
            'save-btn'    => 'Lagre',
            'title'       => 'Konfigurasjon',
            'saved'       => 'Konfigurasjon lagret.',
            'description' => 'Konfigurer innstillingene for digital ressursstyring (DAM) for applikasjonen din.',
            'general'     => [
                'title'            => 'Generelle innstillinger',
                'description'      => 'Disse innstillingene styrer den generelle DAM-bla-opplevelsen.',
                'explorer-enabled' => [
                    'label' => 'Aktiver Utforskningsvisning',
                    'hint'  => 'Bruk det Utforsker-lignende grensesnittet for å bla gjennom og administrere ressurser.',
                ],
                'bookmarks-enabled' => [
                    'label' => 'Vis bokmerkepanel',
                    'hint'  => 'Vis et eget sidepanel med dine bokmerkede mapper og ressurser.',
                ],
            ],
            'directory' => [
                'title'       => 'Mappeinnstillinger',
                'description' => 'Disse innstillingene styrer hvordan mapper og ressurser vises i mappepanelet.',
                'show-tree'   => [
                    'label'       => 'Vis mappetre',
                    'hint'        => 'Vis mappehierarkiet i venstre sidepanel for enklere navigasjon.',
                    'locked-hint' => 'Alltid på mens Utforskningsvisning er deaktivert.',
                ],
                'tree-show-assets' => [
                    'label' => 'Vis ressurser i mappetre',
                    'hint'  => 'Vis ressurser sammen med mapper i mappetreet i stedet for å bare vise mapper.',
                ],
            ],
            'ai-tagging' => [
                'title'       => 'AI-tagging',
                'description' => 'Foreslår automatisk tagger for opplastede bilder ved hjelp av din konfigurerte Magic AI-plattform.',
                'enabled'     => [
                    'label' => 'Automatisk tagging av ressurser med AI',
                    'hint'  => 'Bruker din konfigurerte Magic AI-plattform til å foreslå tagger for bilder automatisk når de lastes opp.',
                ],
                'platform' => [
                    'label'                    => 'Plattform',
                    'use-default'              => 'Bruk standardplattform',
                    'none-configured'          => 'Ingen AI-plattform konfigurert',
                    'none-configured-hint'     => 'Legg først til en synskapabel Magic AI-plattform — se Innstillinger → Magic AI → Plattformer.',
                ],
                'max-tags' => [
                    'label' => 'Maks. tagger',
                    'hint'  => 'Maksimalt antall tagger som foreslås per bilde (1-20).',
                ],
                'rate-limit' => [
                    'label' => 'Hastighetsgrense for AI-tagging',
                    'hint'  => 'Maksimalt antall AI-taggeforespørsler per minutt (1-120). Reduser denne verdien for å matche hastighetsgrensen til AI-leverandøren din.',
                ],
            ],
        ],
        'explorer' => [
            'not-found'         => 'Mappen ble ikke funnet.',
            'access-denied'     => 'Du har ikke tilgang til denne mappen.',
            'empty'             => 'Ingen mapper funnet.',
            'action-completed'  => 'Handlingen ble fullført.',
            'foreign-selection' => [
                'notice' => ':count element(er) valgt i en annen fane.',
                'action' => 'Velg dem her også',
            ],
            'bookmarks' => [
                'title'     => 'Bokmerker',
                'drag-hint' => 'Dra en mappe hit for å bokmerke den',
                'max'       => 'Maksimalt 20 bokmerker nådd.',
                'remove'    => 'Fjern bokmerke',
                'empty'     => 'Ingen bokmerker ennå.',
            ],
            'tab' => [
                'new'   => 'Ny fane',
                'close' => 'Lukk fane',
            ],
            'sections' => [
                'folders' => 'Mapper',
                'files'   => 'Filer',
                'folder'  => 'Mappe',
            ],
            'clipboard' => [
                'ready'   => 'Klar til å lime inn',
                'dismiss' => 'Tøm',
            ],
            'context' => [
                'open'                 => 'Åpne',
                'open-new-tab'         => 'Åpne i ny fane',
                'bookmark'             => 'Bokmerk mappe',
                'folder-upload'        => 'Last opp mappe',
                'copy'                 => 'Kopier',
                'paste'                => 'Lim inn',
                'copy-progress'        => 'Kopiering satt i kø.',
                'copy-done'            => 'Kopiert.',
                'paste-file-started'   => 'Limer inn fil...',
                'paste-folder-started' => 'Limer inn mappe...',
                'paste-done'           => 'Limt inn.',
                'move-asset-started'   => 'Flytter fil...',
                'move-folder-started'  => 'Flytter mappe...',
                'move-done'            => 'Mappe flyttet.',
                'move-asset-done'      => 'Fil flyttet.',
                'delete-done'          => 'Mappe slettet.',
            ],
            'folder' => [
                'deleted' => 'Denne mappen ble slettet — returnert til Rot.',
            ],
            'search' => [
                'placeholder' => 'Søk etter filer og mapper…',
            ],
            'list' => [
                'header' => [
                    'name'     => 'Navn',
                    'type'     => 'Type',
                    'size'     => 'Størrelse',
                    'modified' => 'Endret',
                    'actions'  => 'Handlinger',
                ],
                'items-count' => ':count elementer',
            ],
            'dialog' => [
                'rename-asset' => [
                    'title'       => 'Gi filen nytt navn',
                    'placeholder' => 'Nytt navn',
                ],
                'save'   => 'Lagre',
                'cancel' => 'Avbryt',
            ],
            'view' => [
                'grid' => 'Rutenettvisning',
                'list' => 'Listevisning',
            ],
            'mass-actions' => [
                'select-count'   => ':count valgt',
                'select-action'  => 'Velg handling',
                'delete'         => 'Slett',
                'confirm'        => 'Slette :count element(er)? Mapper slettes rekursivt og kan ikke angres.',
                'deleting'       => 'Sletter :count element(er)...',
                'deleted-assets' => ':count ressurs(er) slettet.',
                'deleted-dirs'   => ':count mappe(r) satt i kø for sletting.',
                'move-to'        => 'Flytt til…',
                'copy-to'        => 'Kopier til…',
                'moving'         => 'Flytter…',
                'copying'        => 'Kopierer…',
                'moved-assets'   => ':count fil(er) flyttet.',
                'copied-assets'  => ':count fil(er) kopiert.',
                'moved-dirs'     => ':count mappe(r) flyttet.',
                'copied-dirs'    => ':count mappe(r) kopiert.',
                'pick-dest'      => 'Velg destinasjon',
                'select-here'    => 'Velg her',
                'new-folder'     => 'Ny mappe',
                'folder-name'    => 'Mappenavn',
                'create-error'   => 'Kunne ikke opprette mappen.',
                'move-done'      => ':count Filer er flyttet.',
                'copy-done'      => ':count Filer er kopiert.',
                'still-running'  => 'Operasjonen kjører fremdeles i bakgrunnen',
                'progress'       => 'Fremgang',
                'total-files'    => ':count fil(er)',
            ],
            'toolbar' => [
                'toggle-tree'    => 'Slå mappestruktur av/på',
                'toggle-sidebar' => 'Slå sidepanel av/på',
            ],
            'upload' => [
                'queued'              => 'I kø',
                'uploading'           => 'Laster opp',
                'creating'            => 'Oppretter mappe',
                'done'                => 'Ferdig',
                'failed'              => 'Mislyktes',
                'interrupted'         => 'Avbrutt — legg til på nytt for å prøve igjen',
                'resuming'            => 'Gjenopptar…',
                'overall'             => ':done av :total lastet opp',
                'too-large-to-resume' => 'Partiet er for stort til å gjenopptas automatisk etter oppdatering',
            ],
        ],
    ],
    'share' => [
        'public' => [
            'app-name'              => 'DAM',
            'download'              => 'Last ned',
            'back-to-gallery'       => 'Tilbake til galleri',
            'back'                  => 'Tilbake',
            'file-name'             => 'Filnavn',
            'file-type'             => 'Filtype',
            'file-size'             => 'Filstørrelse',
            'video-not-supported'   => 'Nettleseren din støtter ikke video-elementet.',
            'audio-not-supported'   => 'Nettleseren din støtter ikke audio-elementet.',
            'preview-not-available' => 'Forhåndsvisning er ikke tilgjengelig for denne filen. Bruk Last ned-knappen ovenfor.',
            'files-count'           => ':count fil(er)',
            'empty-directory'       => 'Denne delte mappen inneholder ingen filer ennå.',
            'expires-on'            => 'Denne lenken utløper den',
            'powered-by-dam'        => 'Drevet av Unopim DAM',
            'expired-title'         => 'Lenke utløpt',
            'expired-message'       => 'Denne delingslenken har utløpt og er ikke lenger tilgjengelig. Be om en ny lenke fra eieren.',
            'revoked-title'         => 'Lenke tilbakekalt',
            'revoked-message'       => 'Denne delingslenken er tilbakekalt av eieren og er ikke lenger tilgjengelig.',
            'not-found-title'       => 'Lenke ikke funnet',
            'not-found-message'     => 'Delingslenken du åpnet eksisterer ikke. Det kan hende den ble skrevet feil eller fjernet.',
            'download-zip'          => 'Last ned alt som ZIP',
            'per-page'              => 'Per side',
        ],
    ],
    'data-transfer' => [
        'bundle' => [
            'invalid-archive'            => 'The uploaded archive could not be opened.',
            'invalid-zip'                => 'The uploaded archive is damaged and cannot be read.',
            'no-data-file'               => 'The archive contains no CSV or Excel file to import.',
            'zip-too-many-entries'       => 'The archive contains :count entries, which is above the limit of :limit.',
            'zip-contents-too-large'     => 'The archive expands to more than :limit MB.',
            'zip-compression-suspicious' => 'The entry ":entry" is compressed far beyond a normal ratio and was rejected.',
            'asset-not-found'            => 'No asset was found at path ":path".',
            'asset-tree-misnamed'        => 'Arkivet inneholder en mappe med navnet ":name" i stedet for "assets", så ingen av ressursene kunne importeres. Gi den nytt navn til "assets" og sørg for at mappene under samsvarer med stiene i datafilen din, for eksempel "assets/Root/Seed/Tech Sheets/Cotton/". macOS lagrer en "/" skrevet i et mappenavn som ":", noe som er den vanlige årsaken.',
        ],
    ],
];
