{{--
    Datagrid asset preview modal.

    Rendered once on the DAM asset listing page. The eye icon on each gallery
    card emits `dam:open-asset-preview` with the asset id; this component
    fetches the asset's preview payload and opens a fullscreen viewer — an
    image viewer with zoom/rotate/pan, a native video/audio player, an inline
    PDF viewer, or a fallback for unsupported types.

    Self-contained by design: it does not depend on the edit-page preview
    modal, so the listing page carries no per-asset server state.
--}}
<v-dam-asset-preview></v-dam-asset-preview>

@pushOnce('scripts')
    <script type="text/x-template" id="v-dam-asset-preview-template">
        <div
            v-if="isOpen"
    ...
    <script type="module">
        app.component('v-dam-asset-preview', {
            template: '#v-dam-asset-preview-template',
            ...
            data() {
                return {
                    ...
                    previewDataUrl: @js(route('admin.dam.assets.preview_data', '__id__')),
                    placeholders:   {
            ...
            mounted() {
                this.$emitter.on('dam:open-asset-preview', this.open);
            ...
            methods: {
                ...
                open(id) {
                    ...
                    this.$axios.get(this.previewDataUrl.replace('__id__', id))
                        .then(response => {
