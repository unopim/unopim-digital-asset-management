    ...
    >
        <div class="image-card relative overflow-hidden">
            ...
            <span
                ...
                style="z-index:2;"
                :class="{
                    'bg-primary-600': asset.file_type==='video'||asset.file_type==='audio',
            ...
            <div class="absolute inset-0 flex items-center justify-center bg-black/80 dark:bg-cherry-800/90 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
                <div class="flex gap-1">
                    ...
                    <button type="button" class="icon-dam-preview text-xl p-1.5 rounded-md text-white hover:bg-primary-600 transition-colors pointer-events-none group-hover:pointer-events-auto" @click.stop="$emit('preview', asset.id)"></button>
                    @endif
                    @if (bouncer()->hasPermission('dam.asset.edit'))
                    <button type="button" class="icon-edit text-xl p-1.5 rounded-md text-white hover:bg-primary-600 transition-colors pointer-events-none group-hover:pointer-events-auto" @click.stop="$emit('edit', asset.id)"></button>
                    @endif
        ...
        <div class="flex gap-1.5 items-center py-1.5 pl-1">
            ...
            >
                <label :for="`sel-card-asset-${asset.id}`" class="flex items-center cursor-pointer">
                    ...
                    >
                    <span class="icon-checkbox-normal peer-checked:icon-checkbox-check peer-checked:text-primary-700 rounded-md text-2xl"></span>
                </label>
            </span>
            ...
            <button
                type="button"
                class="dam-ctx-trigger shrink-0 ltr:mr-1 rtl:ml-1 w-6 h-6 flex items-center justify-center rounded-md text-gray-400 dark:text-gray-300 opacity-0 group-hover:opacity-100 hover:bg-gray-100 dark:hover:bg-cherry-800 hover:text-primary-700 dark:hover:text-primary-400 transition-opacity"
