...
<v-dam-tab
    tab-id="{{ $tabId }}"
    :initial-directory-id="{{ json_encode($initialDirectoryId) }}"
    initial-search="{{ $initialSearch }}"
    initial-view-mode="{{ $initialViewMode }}"
    :initial-page="{{ $initialPage }}"
    :initial-per-page="{{ $initialPerPage }}"
    :acl-bypass="{{ $aclBypass ? 'true' : 'false' }}"
    :accessible-ids='{{ json_encode($accessibleIds) }}'
    @tab-state-change="$emit('tab-state-change', $event)"
    @tab-label-change="$emit('tab-label-change', $event)"
></v-dam-tab>
