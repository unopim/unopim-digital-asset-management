<?php

use Webkul\DAM\Helpers\AssetHelper;

/**
 * The DAM "no records found" placeholder SVG must not be uploadable as a real
 * asset. Detection is by content signature (robust to rename + re-serialization)
 * or filename stem, and only SVGs are inspected.
 */
const PLACEHOLDER_SVG = <<<'SVG'
<svg width="96" height="96" viewBox="0 0 96 96" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M76.9143 80.5715L84.1143 87.7715M61.7143 74.9715L71.3143 65.3715M61.7143 65.3715L71.3143 74.9715M80.9143 70.1715C80.9143 66.3524 79.3971 62.6897 76.6966 59.9891C73.9961 57.2886 70.3334 55.7715 66.5143 55.7715C62.6951 55.7715 59.0324 57.2886 56.3319 59.9891C53.6314 62.6897 52.1143 66.3524 52.1143 70.1715C52.1143 73.9906 53.6314 77.6533 56.3319 80.3538C59.0324 83.0543 62.6951 84.5715 66.5143 84.5715C70.3334 84.5715 73.9961 83.0543 76.6966 80.3538C79.3971 77.6533 80.9143 73.9906 80.9143 70.1715Z" stroke="#7C3AEC" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M44 88H42.908C29.868 88 23.34 88 18.812 84.808C17.5234 83.9043 16.3716 82.8199 15.392 81.588C12 77.324 12 71.188 12 58.908V48.728C12 36.876 12 30.948 13.876 26.216C16.892 18.604 23.268 12.604 31.356 9.764C36.38 8 42.672 8 55.272 8C62.464 8 66.064 8 68.936 9.008C73.556 10.632 77.2 14.06 78.924 18.408C80 21.112 80 24.5 80 31.272V48" stroke="#7C3AEC" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M12 48C12 44.4641 13.4046 41.0731 15.9049 38.5729C18.4051 36.0726 21.7961 34.668 25.332 34.668C27.996 34.668 31.136 35.132 33.724 34.44C34.8548 34.1357 35.8858 33.5394 36.7135 32.711C37.5412 31.8826 38.1366 30.8511 38.44 29.72C39.132 27.132 38.668 23.992 38.668 21.328C38.6691 17.7928 40.0741 14.4028 42.5743 11.9034C45.0744 9.40407 48.4648 8 52 8" stroke="#7C3AEC" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/>
</svg>
SVG;

function writeTempSvg(string $contents): string
{
    $path = tempnam(sys_get_temp_dir(), 'dam_svg_');
    file_put_contents($path, $contents);

    return $path;
}

it('blocks the placeholder SVG by content signature even when renamed', function () {
    $path = writeTempSvg(PLACEHOLDER_SVG);

    expect(AssetHelper::isForbiddenFile('svg', 'image/svg+xml', 'totally-renamed.svg', $path))->toBeTrue();
    expect(AssetHelper::isPlaceholderImage('svg', 'image/svg+xml', 'totally-renamed.svg', $path))->toBeTrue();

    @unlink($path);
});

it('blocks a re-serialized copy of the placeholder SVG', function () {
    // Browser/SVGO re-serialization: XML prolog + expanded tags; path data intact.
    $reSerialized = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n".str_replace('/>', '></path>', PLACEHOLDER_SVG);
    $path = writeTempSvg($reSerialized);

    expect(AssetHelper::isForbiddenFile('svg', 'image/svg+xml', 'whatever.svg', $path))->toBeTrue();

    @unlink($path);
});

it('blocks the placeholder SVG by filename stem even with a build-hash suffix', function () {
    expect(AssetHelper::isForbiddenFile('svg', 'image/svg+xml', 'no-records-found.svg'))->toBeTrue();
    expect(AssetHelper::isForbiddenFile('svg', 'image/svg+xml', 'no-records-found-Dw1Lp69h.svg'))->toBeTrue();
});

it('allows an unrelated SVG upload', function () {
    $svg = '<svg viewBox="0 0 10 10" xmlns="http://www.w3.org/2000/svg"><rect width="10" height="10" fill="#000000"/></svg>';
    $path = writeTempSvg($svg);

    expect(AssetHelper::isForbiddenFile('svg', 'image/svg+xml', 'company-logo.svg', $path))->toBeFalse();
    // Name + type only (no content read) — a normal SVG is still allowed.
    expect(AssetHelper::isForbiddenFile('svg', 'image/svg+xml', 'company-logo.svg'))->toBeFalse();

    @unlink($path);
});

it('does not treat non-SVG files as the placeholder', function () {
    // Same bytes, but a png extension/mime — not inspected as an SVG.
    $path = writeTempSvg(PLACEHOLDER_SVG);

    expect(AssetHelper::isForbiddenFile('png', 'image/png', 'no-records-found.png', $path))->toBeFalse();

    @unlink($path);
});
