...
### System dependencies for thumbnail generation

Video and PDF thumbnails are generated by external binaries. Install them on the host that runs the queue worker:
...
- `ffmpeg` is used to extract a frame from uploaded videos.
- `poppler-utils` provides `pdftoppm`, used to render the first page of a PDF as an image.

Without these binaries, video and PDF assets will upload successfully but their thumbnails will fall back to the default file-type icon.
